/* ---------- SRC-38.27 · LUMEN FILM V27 (the track is the lamp)
   Lance's DJ-set round (Nima's scene — adaptation only, coordinate
   before reworking). The film already IS a lamp behind a sheet, so
   the ears go in with the smallest possible move: when a booth feed
   is live (audioIn:true, arbitrated on inp.audio.live) the track's
   LEVEL joins the lamp itself — s.lamp = max(hand, idle, level) —
   and the apertures open in threshold-rank order with the set's
   energy: the root-stacking organ, the choir at five, the glass
   sweep all re-orchestrate themselves for free. The eases are the
   SLOWEST in the wave (~1.2/s): apertures are held notes, not
   meters. THE SHIELD: every aperture flip is a note-on/off pair
   through s.evq, so the commit test grows per-aperture HYSTERESIS
   (an OFF aperture needs th+0.03, an ON one holds to th−0.05) and
   the presence gate becomes max(hand, booth) — the booth can open
   the film with nobody at the pedestal, but a wobbling VU can never
   machine-gun the organ. KICK → the star flare (1-3 stones through
   the existing sparkle hooks, back-dated by the hit's true age,
   CAPPED so stars never stack — the 4-ray pass is the scene's most
   expensive draw); MID floors the wobble depth; TREBLE adds a slow
   spin drift UNDER the left hand's absolute dial (the gradients lag
   behind it, existing behaviour); a structural DROP showers three
   stars for 1.2s and nothing else — no lamp jump, a full-frame slam
   is the strobe the audit banned. Rest breaths treat audio as
   presence so the toll never lands over a playing track. With no
   signal this is exactly V26. (V26 notes below.) ------------------- */
/* ---------- SRC-38.26 · LUMEN FILM V26 (the diamond learns to shine)
   Lance on V25's rest: "I'm still not getting any visual tease? I
   thought you make a diamond shine or something." V25's glint was one
   small glow dot for 2.5 seconds at rest brightness, once every
   10-26s -- a blink you had to be staring at. V26 makes the tease
   THE DIAMOND ITSELF: the chosen stone's facets BLOOM with colour,
   the burst point flares a real STAR (the same four-ray flash a
   committed burst throws), and the whole shine fades on the SAME
   tail length as the sound it accompanies -- ~6s riding the shrine's
   long tail, ~5.5s under the choir chord, ~3.5s with a bass breath,
   three stones for the deep toll. Sound and light are one gesture
   now. ALSO: the toll's bell and bass hit TOGETHER (his catch:
   "the bass and the bells didn't hit at the same time... sounded
   strange") -- the bass note lands on the bell's instant and only
   its CC74 swell blooms late, instead of the note itself arriving
   0.2s behind the bell like a flam. AND THE GLITTER RESTORED ("what
   happened to the diamond sparkle noises? It doesn't feel as
   reactive"): V25's stack sent early entrances -- and their glass
   doubles -- to the bottom register. The organ keeps stacking from
   the root, but the bell doubles ring two octaves up so every
   entrance glints high again, and a fast push-in pulls the glass
   SWEEP to fire at once instead of on its own 5-9s clock -- lean in
   hard and the film answers immediately. (V25 notes below.) -------- */
/* ---------- SRC-38.25 · LUMEN FILM V25 (the wall stacks from the root)
   Lance's round on V24, four calls. THE STACK: "bring the chord in
   gradually -- pieces of the chord as you bring the right hand in, so
   it stacks into a wall of sound." The apertures always added one
   voice at a time, but in RANDOM pitch order (pitch came from frame
   height). Now threshold rank IS the chord-ladder rung: the first
   aperture to light sings the root, the next the fifth, and the wall
   builds bottom-up every time, spreading ~2.5 octaves as the right
   hand comes in -- ordered, expressive, and less muddy than the old
   clustered voicing. THE SPARKLE: every rest breath now glints a
   stone (three on the deep toll) -- the sound visibly moves the
   picture, the missing visual tease. LONGER HOLDS: the Shrine tease
   holds ~3.2s so its swell fully opens; the choir tease is a rolled
   CHORD now (root-third-fifth, 350ms stagger, 5s holds) -- Lance:
   one choir note is too subtle. REGISTER CARE: each voice's octave
   partial thins below ~350Hz so the low end of the wall stays clean.
   (V24 notes below.) ------------------------------------------------ */
/* ---------- SRC-38.24 · LUMEN FILM V24 (the rest learns to tease)
   Lance on V23's rest: "you're just hitting the bass every now and
   then. Super dry and lame. Not doing a great job teasing." And his
   own fix: "Shrine has a super long tail -- hit it for a second, one
   note, and that's a 10-second tease." V24 rebuilds the breath as a
   ROTATION: most breaths are ONE SHRINE NOTE (a second of touch, the
   patch's tail does the teasing), some are a barely-there choir root,
   some are the bass -- and the bass now rides a CC74 SWELL (ramps up
   ~1.2s before the note, falls over ~3s after) so once ch3 CC74 is
   mapped to a filter or volume, the hit BLOOMS instead of thudding.
   The 1-in-7 deep toll stays, now over a swelled bass. The lamp-based
   bass CC74 stream yields to the swell at rest.
   AND THE WOBBLE, LOCKED TO THE PICTURE (Lance: "match the rotation of
   the diamonds exactly so it feels like an instrument"): V23's LFO
   free-ran at a speed-derived rate; V24 derives the wobble's PHASE
   from the rotation itself -- the spin angle IS the LFO. Turn fast and
   it wobbles fast, turn slow and it sweeps, STOP and it holds where
   the diamonds stopped. Right hand builds the wall, left hand plays
   it. And the STEREO: each organ voice rides the circle at its own
   phase offset, so at full turn-depth the whole wall of sound swirls
   around the room with the rotation. Pad CC74 carries the same
   rotation-locked wobble. (V23 notes below.) ------------------------ */
...
const LUMEN27_STOPCACHE = new Map();
reg({
  id: 'SRC-38.27', family: 'SRC-38', ver: 27, title: 'Lumen Film V27', tech: 'GEM FILM / APERTURE ORGAN / LISTENS',
  audioIn: true,
  music: {
  ...
  fx: { bloom: 0.46 },
  tags: ['NO SMALL STONES', 'SAME LIGHT', 'THE TRACK IS THE LAMP', 'LISTENS'],
  desc: 'V26, given ears (plug the booth in and the film listens): the track\'s LEVEL is the lamp behind the sheet — the apertures open in rank order with the set\'s energy, the organ stacks from the root, the choir-at-five and the glass sweep re-orchestrate themselves for free, and nobody needs to be at the pedestal. Kicks flare one to three chromatic stars, the top end slowly turns the stones, and a structural DROP showers three stars at once. Per-aperture hysteresis keeps a wobbling meter from machine-gunning the organ. With no signal, exactly V26. — V18: the two smallest bands of stone are gone — the little ones were reading as grit against the big cut faces rather than as gems of their own. The floor of the size range lifts, the spread narrows from roughly four-to-one to under two-to-one, and because the weave is held as a fraction of the frame the count simply falls out of that: bigger stones, fewer of them, the same amount of picture under stone. — V17: the weave is now held as a fraction of the frame rather than a fixed number of stones, so it looks the same whatever shape the window or the projector is. Before this it did not: the stones are sized off the short side of the frame while the area grows with the long one, which means a set count quietly gets sparser the wider the window gets — the field that reads right in a long letterbox comes out close to twice as dense on a projector nearer square. The count is now solved from the coverage instead, so a stone stays the same size against the frame and the same fraction of the picture is stone at every shape. Everything else is V16. — V16: the stones are 40% smaller — they were reading too large once the frame went full screen. Two things had to move with them or the change would have taken the scene apart. There are more of them, 100 rather than 44, because area falls with the square of the radius and a thinned-out field both shows black between the stones and starves the lights, which only exist where two stones overlap. And the bursts keep their old size rather than shrinking along with their host, since the light was already where it wanted to be; fewer stones in a hundred are chosen to carry one, so the count of lights on screen stays about what it was. The result is a finer weave under the same lamp.',
  interact: 'R = LIGHT PRESSURE, unchanged — the lamp behind the sheet. Draw in and the film is closed, a dark faceted field and nothing else. Reach out and the bursts light one at a time, scattered anywhere in the frame, each on its own clock, each adding its held note to the chord. A burst always tells you before it commits — it leaks a little glow half a beat early, then blooms with its note on the sixteenth. L = ORIENTATION — an absolute dial: hand at the sphere and the crystals sit at rest; reach out and the whole field turns with your hand, up to a full turn at full reach, AND the gradients inside the stones lag round behind it, so the colour travels across the facets while they turn. Every burst sits on a real edge of a real stone and slides along it, so its rays swing with the geometry making them. WITH A BOOTH FEED (V27): the track\'s level joins the lamp, so the film opens with the set\'s energy on its own; hands still win any raise, and the left hand\'s absolute dial rides on top of the treble\'s slow spin drift.',
  sound: 'V26\'s organ, untouched — plus the listening layer (V27): audio joins the LAMP, so the apertures — and therefore the organ\'s own held voices — open with the track\'s level (the queue\'s per-scene OUT setting governs where that sound goes during a DJ set). Per-aperture HYSTERESIS shields the note stream: an OFF aperture needs the lamp 0.03 above its threshold, an ON one holds until 0.05 below, so a wobbling VU can never machine-gun note-on/offs. Kick = the star flare (1-3 stones, capped, back-dated to the hit); mid = a floor under the wobble depth; treble = slow spin drift; a structural DROP = a three-star shower, no lamp jump. Rest breaths treat audio as presence, so the toll never lands over a playing track. — An aperture organ. Every lit burst holds one voice — and the wall STACKS FROM THE ROOT (V25): threshold rank is the chord-ladder rung, so the chord assembles bottom-up as the right hand comes in, spanning ~2.5 octaves; stereo position by where the burst sits left to right. Voices enter rolled low-to-high like a harp and each entrance is doubled by a glass bell (MIDI role: bells) on the sixteenth it commits. Fewer, larger bursts than before means a sparser, more deliberate chord. L still opens every voice filter together and detunes their partials. Underneath: a slow air bed and a root pedal that never moves. NO PERCUSSION. Ableton: pad = the burst voices (CC74 = L, velocity by height + lamp), bells = the glass doubles, texture = air (CC74 only), bass = the held D pedal, re-struck gently each chord. NO PERCUSSION. THE WOBBLE (V24): locked to the picture - the diamonds\' spin angle IS the LFO phase (turn fast = fast wobble, stop = it holds), depth rides how far the field is turned, and at depth each voice rides the circle at its own phase so the wall of sound SWIRLS around the room; pad CC74 carries the same rotation-locked wobble. THE SWEEP (V23): under hands, every 5-9s a soft glass arpeggio rolls across exactly the lit pitches - light crossing the film, never a grid. Peak choir = two real voices. THE REST (V26): absence closes the film over ~7s; every breath makes a DIAMOND SHINE (three on the toll) - the whole stone blooms and flares a star, fading on the tease\'s own tail; the toll\'s bell and bass land on the SAME instant (only the CC74 swell blooms late); idle breathes every 10-26s in a ROTATION - one SHRINE note whose long tail does the teasing (Lance\'s call), a barely-there choir root, or a bass breath riding a CC74 swell (map ch3 CC74 and it blooms); 1-in-7 a deep glass toll over swelled bass. First touch opens the film fast.',
  ...
  init(P) {
    ...
    // THE STACK (V25): threshold rank = chord-ladder rung. The aperture
    // that lights FIRST sings the root; the wall builds bottom-up as the
    // right hand comes in, spanning ~2.5 octaves across the field.
    ap.map((p, i) => i).sort((a, b) => ap[a].th - ap[b].th)
      .forEach((pi, rank) => { ap[pi].rung = Math.round(rank / Math.max(1, ap.length - 1) * 13); });
    ap.sort((u, z) => u.th - z.th);
    ap.forEach((p, i) => { p.i = i; p.voice = i < 8 ? i : -1; });
    P.state = {
      cells, ap, pres: 0, lightP: 0, thick: 0, rotL: 0, spin: 0, openN: 0, lamp: 0, evq: [],
      // V27: the listening layer — audio liveness, slow VU eases (the
      // slowest in the wave: apertures are held notes), the treble spin
      // drift, presV = VISUAL presence (hands OR booth), event counters,
      // and the sparkle hooks the audio() rest breaths already write
      aPres: 0, aLevel: 0, aMid: 0, aTreb: 0, presV: 0, spinDrift: 0,
      _kN: 0, _dropN: 0, _aSeeded: false,
      sparkle: 0, sparkN: 0, sparkSeed: 0, sparkDur: 0.6
    };
  },
  ...
  step(P, dt, t, inp) {
    ...
    // ---- V27: THE LISTENING LAYER --------------------------------------
    const au = inp.audio;
    const aLive = au && au.live ? 1 : 0;
    s.aPres += (aLive - s.aPres) * Math.min(1, dt * 2.2);
    if (au) {
      // the slowest VU eases in the wave (~1.2/s): the lamp is a held
      // note, not a meter — only the kick moves fast
      s.aLevel += (clamp(au.level) - s.aLevel) * Math.min(1, dt * 1.2);
      s.aMid += (clamp(au.mid) - s.aMid) * Math.min(1, dt * 1.2);
      s.aTreb += (clamp(au.treble) - s.aTreb) * Math.min(1, dt * 1.2);
      // seed event counters on first sight — a scene opened mid-set must
      // not flare off a kick or drop three scenes old
      if (!s._aSeeded) { s._aSeeded = true; s._kN = au.kick ? au.kick.n : 0; s._dropN = au.drop ? au.drop.n : 0; }
      // KICK → THE STAR FLARE: 1-3 diamonds fire the full chromatic star
      // through the existing sparkle hooks, back-dated by the hit's true
      // age. CAPPED: never stack stars — the 4-ray star pass is the
      // scene's most expensive draw.
      if (au.kick && au.kick.n !== s._kN) {
        s._kN = au.kick.n;
        if (!(s.sparkle > 0.5) && s.aPres > 0.1) {
          const age = au.kick.perfClock ? 0 : clamp(au.now - au.kick.t, 0, 0.2);
          const hit = clamp(au.kick.strength) * s.aPres;
          s.sparkle = clamp(1 - (age + 0.030) / 0.6);   // back-dated on the linear tail
          s.sparkN = 1 + Math.round(2 * hit);
          s.sparkSeed = (Math.random() * 997) | 0;
          s.sparkDur = 0.6;
        }
      }
    } else { s.aLevel *= Math.pow(0.5, dt); s.aMid *= Math.pow(0.5, dt); s.aTreb *= Math.pow(0.5, dt); }
    // THE DROP: a shower of stars — three at once, on the long tail.
    // Nothing else: no lamp jump (a full-frame brightness slam is the
    // strobe the audit banned).
    if (au && au.drop && au.drop.n !== s._dropN) {
      s._dropN = au.drop.n;
      if (s.aPres > 0.3) {
        s.sparkle = 1; s.sparkN = 3; s.sparkSeed = (Math.random() * 997) | 0; s.sparkDur = 1.2;
      }
    }
    // MID → the wobble deepens with the music's body (a floor under the
    // left hand's ease — the hand always wins a raise)
    s.thick = Math.max(s.thick, 0.6 * s.aMid * s.aPres);
    // VISUAL presence: the booth can open the film with nobody at the
    // pedestal (used by the aperture commit gate below)
    ...
    // V27: LEVEL → THE LAMP. The track is the lamp behind the sheet —
    // apertures open in rank order with the set's energy, and the organ,
    // choir and sweep re-orchestrate themselves for free.
    // gamma 1.35 on the audio lamp: mid-level music opens half the wall,
    // only a true peak blazes every aperture — the full-wall star field is
    // an EARNED moment, not the groove's resting state
    s.lamp = Math.max(s.lightP, idle, Math.pow(s.aLevel, 1.35) * 0.9 * s.aPres);
    ...
    // V27: TREBLE → SPIN DRIFT. The stones turn slowly with the top end
    // and the gradients lag behind (existing behaviour); the left hand's
    // absolute dial still works on top.
    s.spinDrift += dt * 0.055 * s.aTreb * s.aPres;
    const targetSpin = (s.pres > 0.05 ? s.rotL * TAU : 0) + s.spinDrift;
    ...
    for (const c of s.cells) {
      ...
      // a shining stone fades on the tease's own tail (re-upped below
      // while the sparkle is alive)
      if (c.shine) c.shine = Math.max(0, c.shine - dt / (s.sparkDur || 3));
    }
    let open = 0;
    for (const p of s.ap) {
      const H = s.cells[p.host];
      p.x = H.x; p.y = H.y;
      // V27: THE SHIELD — per-aperture hysteresis. Every flip is a
      // note-on/off pair through s.evq, so an OFF aperture needs the lamp
      // 0.03 ABOVE its threshold and an ON one holds until 0.05 BELOW —
      // a wobbling VU can never machine-gun the organ. Presence gate is
      // max(hand, booth): the tease glows, only presence commits, and
      // the booth counts as presence now.
      const want = p.on ? (s.lamp > p.th - 0.05 && s.presV > 0.12)
                        : (s.lamp > p.th + 0.03 && s.presV > 0.12);
    ...
    s.openN = open;
    // THE SHINE (V26): each rest breath makes a DIAMOND SHINE (three on
    // the deep toll) -- the whole host stone's facets bloom, the burst
    // point flares a star, and it all fades on the tease's own tail
    // length (s.sparkDur, set with the breath). V25's 2.5s glow dot
    // was a blink nobody caught (Lance: "still not getting any visual
    // tease?") -- a tease that cannot be seen is no tease. V27 fires
    // the same hooks off the booth's kick and drop.
    if (s.sparkle > 0) {
      for (let k2 = 0; k2 < (s.sparkN || 1); k2++) {
        const sp = s.ap[((s.sparkSeed || 0) + k2 * 7) % s.ap.length];
        if (sp) {
          sp.lit = Math.max(sp.lit, s.sparkle);
          sp.glint = s.sparkle;
          const hc = s.cells[sp.host];
          if (hc) hc.shine = s.sparkle;
        }
      }
      s.sparkle = Math.max(0, s.sparkle - dt / (s.sparkDur || 3));
    }
  ...
  draw(P, g, w, h, t, inp) {
    ...
    g.fillStyle = '#010103';
    g.fillRect(0, 0, w, h);
    // V27: the booth lights the field like a body does (presV = hands OR
    // audio); the scene's own SOUND gates stay hand-only in audio()
    const bright = 0.5 + (s.presV !== undefined ? s.presV : s.pres) * 0.5;
    const gate = clamp((s.presV !== undefined ? s.presV : s.pres) * 1.15);
    ...
    const addSvgStops = (grad, a) => {
      let set = LUMEN27_STOPCACHE.get(a);
      if (!set) {
        set = SVGSTOPS.map(([t, rgb]) => [t, `rgba(${rgb},${a})`]);
        LUMEN27_STOPCACHE.set(a, set);
      }
    ...
    for (const c of s.cells) {
      ...
      for (const [inA, inB, outV] of tris) {
        ...
        // c.shine: the resting tease blooms the WHOLE stone, not a dot
        const alphaEnv = clamp(0.04 + lit * 0.5 * gate + s.lamp * 0.3 + (c.shine || 0) * 0.55);
    ...
    for (const p of s.ap) {
      const a = Math.max(p.lit * 0.35, (p.glint || 0), p.open);
      if (a < 0.01) continue;
      ...
      // a shining stone gets FULL brightness even at rest -- the whole
      // point of the tease is to be seen from across the room
      const pb = Math.max(bright, (p.glint || 0) * 0.95);
      const R = p.size * (0.13 + a * 0.4);
      const gr = g.createRadialGradient(sx, sy, 0, sx, sy, R);
      gr.addColorStop(0, `rgba(242,255,242,${(Math.min(0.92, a) * pb).toFixed(2)})`);
      gr.addColorStop(0.35, `rgba(37,85,238,${(a * 0.55 * pb).toFixed(2)})`);
      gr.addColorStop(0.7, `rgba(255,214,90,${(a * 0.3 * pb).toFixed(2)})`);
      gr.addColorStop(1, 'rgba(225,61,47,0)');
      g.fillStyle = gr;
      g.beginPath(); g.arc(sx, sy, R, 0, TAU); g.fill();

      // THE STAR: rays fire for a committed burst OR a resting shine --
      // the tease flashes the same four-ray star a played note earns,
      // briefly, then fades with the sound's tail
      const rayDrive = Math.max(p.open, (p.glint || 0) * 0.8);
      if (rayDrive > 0.2) {
        const L = R * (2.2 + rayDrive * 5);          // longer than before
        ...
        for (const dr of dirs) {
          ...
          const av = rayDrive * dr.av * pb;
          const coreW = Math.max(dr.wMin * ms, R * dr.w * rayDrive);
    ...
    g.fillText('SPOTS ' + s.openN + '/' + s.ap.length + '   LAMP ' + Math.round(s.lamp * 100) +
      ...
      (s.aPres > 0.3 ? '   AUD ' + Math.round(s.aLevel * 100) + ' m' + Math.round(s.aMid * 100) + ' t' + Math.round(s.aTreb * 100) : '') +
      ((s.presV || s.pres) < 0.3 ? '   · CLOSED' : ''), 10, h - 10);
  },
  ...
  audio(A, P) {
    ...
    for (let i = 0; i < NV; i++) {
      ...
      const o3g = v.g(0.5);
      o1.connect(f); o2.connect(f); o3.connect(o3g); o3g.connect(f); f.connect(g);
      ...
      voices.push({ o1, o2, o3, o3g, f, g, pan, freq: 220, note: -1, lvl: 0 });
    }
    const setV = (vc, freq, glide, vel) => {
      ...
      // register care: the octave partial thins on low rungs so the
      // bottom of the wall stays clean, not doubled into mud
      A.set(vc.o3g.gain, freq < 350 ? 0.14 : 0.5, 0.3);
    ...
    let brSwell = null;   // the bass breath's CC74 envelope
    // THE SWEEP: light crosses the film on its own clock (never a grid)
    let nextSweep = A.t() + 5 + Math.random() * 4;
    let sweepLamp = 0;   // slow-tracked lamp: a fast push opens a gap against it
    ...
    return {
      tick(inp, dt) {
        ...
        /* THE REST: randomized low breaths, 1-in-7 a deep glass toll --
           ...
           V27: audio counts as presence here — the toll must never land
           over a playing track (the DJ owns the room). */
        if (Math.max(s.pres, s.aPres || 0) < 0.25) {
          if (now >= nextBreath) {
            const deep = Math.random() < 1 / 7;
            const pick = Math.random();
            if (deep) {
              // the walk-toward payoff: one deep glass toll over a swelled
              // bass -- ON THE SAME INSTANT (Lance: "the bass and the bells
              // didn't hit at the same time... sounded strange"). Only the
              // CC74 swell blooms late; the note itself lands with the bell.
              A.bell(H.chordTone(0, 0), { at: now + 0.15, vol: 0.05, dur: 6, rev: 0.85 });
              A.tone(H.rootFreq(-1), { at: now + 0.15, vol: 0.05, dur: 4, attack: 0.02, type: 'triangle', rev: 0.5, role: 'bass' });
              brSwell = { t0: now, up: 1.4, dn: 4 };
              s.sparkle = 1; s.sparkN = 3; s.sparkSeed = (Math.random() * 997) | 0; s.sparkDur = 6;
            } else if (pick < 0.5) {
              // ONE SHRINE NOTE (Lance's call): a second of touch, ten
              // seconds of tail -- the patch does the teasing
              MOut.evNote('shrine', H.chordTone(Math.random() < 0.5 ? 0 : 2, 0), 0.05 + Math.random() * 0.04, now + 0.05, 3.2);
              s.sparkle = 1; s.sparkN = 1; s.sparkSeed = (Math.random() * 997) | 0; s.sparkDur = 6;
            } else if (pick < 0.7) {
              // the choir tease is a rolled CHORD (one note was too
              // subtle -- Lance): root-third-fifth, slow stagger, held
              // long enough for the patch's full swell
              [0, 1, 2].forEach((ci, i2) =>
                MOut.evNote('choir', H.chordTone(ci, 0), 0.035 + Math.random() * 0.02, now + 0.1 + i2 * 0.35, 5));
              s.sparkle = 1; s.sparkN = 1; s.sparkSeed = (Math.random() * 997) | 0; s.sparkDur = 5.5;
            } else {
              // the bass breath, on a swell: CC74 ramps up before the note
              // and falls after, so a mapped filter/volume makes it BLOOM
              A.tone(H.rootFreq(-1), { at: now + 0.35, vol: 0.028 + Math.random() * 0.028, dur: 2.2 + Math.random() * 2, attack: 1 + Math.random(), type: 'triangle', rev: 0.5, role: 'bass' });
              brSwell = { t0: now, up: 1.2, dn: 3 };
              s.sparkle = 1; s.sparkN = 1; s.sparkSeed = (Math.random() * 997) | 0; s.sparkDur = 3.5;
            }
            nextBreath = now + 10 + Math.random() * 16;
          }
          // the swell envelope (harmless until ch3 CC74 is mapped; blooms
          // the breath once it is)
          if (brSwell) {
            const e2 = now - brSwell.t0;
            const env = e2 < brSwell.up ? e2 / brSwell.up : Math.max(0, 1 - (e2 - brSwell.up) / brSwell.dn);
            MOut.expr('bass', 0.12 + 0.8 * env);
            if (e2 > brSwell.up + brSwell.dn) brSwell = null;
          }
        ...
        while ((ev = s.evq.shift()) && i < 6) {
          ...
          const vc = voices[p.voice];
          if (ev.on) {
            // THE STACK: threshold rank is the rung -- the wall builds
            // bottom-up from the root as the film opens
            const deg = p.rung || 0;
            const oct = 0;
            const up = deg / 13;
            p.deg = deg; p.oct = oct;
            ...
            vc.pan0 = clamp(p.x / P.w * 2 - 1, -1, 1) * 0.7;
            if (vc.pan) A.set(vc.pan.pan, vc.pan0, 0.2);
            const at = T.next(0.25) + p.voice * 0.012;   // low-to-high harp stagger
            A.set(vc.g.gain, 0.024 + thick * 0.012, 0.35);
            // THE GLITTER LIVES UP TOP (V26): the stack sent early
            // entrances -- and their glass doubles -- to the bottom
            // register, and the "diamond sparkle noises" went with them
            // (Lance: "it doesn't feel as reactive as it did before").
            // The organ keeps stacking from the root; the bell double
            // rings TWO octaves up, so every entrance still glints high.
            A.bell(H.chordTone(deg + 3, oct + 2), { at, vol: (0.045 + 0.03 * up) * gate, dur: 2.4, pan: clamp(p.x / P.w * 2 - 1, -1, 1) * 0.8, rev: 0.7 });
          } else {
        ...
        /* ---- THE WOBBLE, LOCKED TO THE PICTURE: the spin angle IS the
           LFO (two wobble cycles per full turn). Turn fast = fast
           wobble; stop = it holds where the diamonds stopped; depth =
           how far the field is turned. Each voice rides the circle at
           its own phase, so at depth the wall SWIRLS around the room. */
        const ph = (s.spin || 0) * 2;
        const wDepth = thick * s.pres;   // only a PRESENT turn wobbles
        const wob = Math.sin(ph);
        for (let k = 0; k < NV; k++) {
          const vc = voices[k];
          const fBase = 420 + (1 - thick) * 2600 + s.lamp * 700;
          A.set(vc.f.frequency, fBase * (1 + wob * wDepth * 0.35), 0.08);
          A.set(vc.o2.detune, 4 + (1 - thick) * 16 + Math.sin(ph + k * 0.5) * wDepth * 14, 0.08);
          A.set(vc.o3.detune, (1 - thick) * 9, 0.3);
          if (vc.pan) A.set(vc.pan.pan, clamp((vc.pan0 || 0) * (1 - wDepth * 0.8) + Math.sin(ph + k * 0.785) * wDepth * 0.85, -1, 1), 0.1);
        }

        /* ---- THE SWEEP: light crosses the lit film. A fast PUSH pulls
           the next sweep close (V26) -- lean in hard and the glass answers
           at once instead of on its own 5-9s clock. ---------------------- */
        if (s.pres > 0.3 && s.lamp - sweepLamp > 0.14) nextSweep = Math.min(nextSweep, now + 0.9);
        sweepLamp += (s.lamp - sweepLamp) * Math.min(1, (dt || 0.016) * 2);
        ...
        if (typeof MOut !== 'undefined' && MOut.expr) {
          MOut.expr('pad', clamp((1 - thick) * (1 - wDepth * 0.45) + (0.5 + 0.5 * wob) * wDepth * 0.45));
          MOut.expr('choir', Math.min(1, (s.openN || 0) / 12));
          MOut.expr('texture', s.lamp);
          MOut.expr('bells', clamp(s.openN / 8));
          if (s.pres > 0.2) MOut.expr('bass', 0.35 + 0.65 * s.lamp);   // at rest the breath swell owns this lane
        }
      },
      stop() {
