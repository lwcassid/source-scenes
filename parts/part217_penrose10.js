...
reg({
  id: 'SRC-46.10', family: 'SRC-46', ver: 10,
  title: 'Penrose Bloom V10', tech: 'PENROSE DEFLATION / INK + WASH ON PAPER / AUDIO-GROWN',
  audioIn: true,
  fx: { bloom: 0.28 },
  tags: ['AUDIO IN', 'CC1 = DEFLATION DEPTH', 'CC2 = RUBBER', 'THE CRYSTAL SUBDIVIDES', 'KITES AND DARTS', 'PAPER EVERYWHERE'],
  desc: 'V9\'s plate, with the left hand finally playing the thing the scene is named after. Spin is gone — it was the least interesting axis in the picture — and CC1 now walks the tiling\'s DEFLATION: 130 hand-sized diamonds at the bottom of the throw, 2330 fine cells at the top, eighteen times the count across one gesture. Because deflation subdivides rather than replaces, the coarse lattice is a sub-lattice of the fine one: the shared lines stand still and the new lines fade in INSIDE each rhomb, so the plate visibly divides under the hand instead of cross-fading between two pictures. One nib at every depth, the way a real plate is drawn. The music is untouched — loudness still grows the crystal from the seed out, the spectrum still paints it, and the growth front means the same thing at every depth — and the right hand is still V9\'s rubber.',
