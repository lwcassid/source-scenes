/* ---------- SRC-15.15 · FERRO BLOOM V15 (accent blooms break the mood window) ---------- */
reg({
  id: 'SRC-15.15', family: 'SRC-15', ver: 15,
  title: 'Ferro Bloom V15', tech: 'SHADER FIELD / MOTION ILLUSION',
  music: {
  ...
  desc: 'V14\'s garden with the monochrome spells broken. The shared warm↔cool mood only ever spanned a narrow slice of the palette, so the whole field would go blue together, then green together, as it breathed. Now roughly a third of the blooms are ACCENTS: they ignore the mood completely and hold a fixed red, orange, hot pink or light blue, so those four colors sit in the mix at every point of the cycle — a blue passage now carries red and hot pink flowers inside it, a green one carries orange and light blue. The palette grew to nine stops to make room, red joining at the warm end and light blue in the cool region, with the seven V12 colors keeping their order underneath. Everything else is V14: right grows the bigger half, the cluster rerolls its shape and center size each time, and the gradient runs on its own clock, untouched by the hands. Music unchanged: a three-layer soundscape — C-minor pedal drone whose color shifts, a quantized groove that only exists once the garden is thrown wide open, and a reactive layer where every willed bloom, flick, and stillness sounds back at you.',
  ...
  init(P) {
    ...
    const mat = new THREE.ShaderMaterial({
      ...
      fragmentShader: [
        ...
        '}',
        // V15: ORDERED warm→cool palette, now 9 stops — RED joins at the warm
        // end and LIGHT BLUE in the cool region. Relative order of the seven
        // V12 stops is untouched, so the sweep still reads the same.
        'vec3 paletteStop(float i){',
        '  if (i < 0.5) return vec3(1.0, 0.16, 0.13);',    // red — warmest      [accent]
        '  if (i < 1.5) return vec3(0.98, 0.62, 0.34);',   // ember orange       [accent]
        '  if (i < 2.5) return vec3(0.95, 0.55, 0.62);',   // blush pink
        '  if (i < 3.5) return vec3(1.0, 0.341, 0.831);',  // hot pink / orchid  [accent]
        '  if (i < 4.5) return vec3(0.878, 1.0, 0.161);',  // acid chartreuse — the pivot
        '  if (i < 5.5) return vec3(0.16, 0.55, 0.58);',   // teal
        '  if (i < 6.5) return vec3(0.45, 0.82, 1.0);',    // light blue         [accent]
        '  if (i < 7.5) return vec3(0.58, 0.56, 0.90);',   // lavender
        ...
        '  return mix(paletteStop(seg), paletteStop(min(seg + 1.0, 8.0)), fr);',
        '}',
        // V15: ACCENT BLOOMS. The mood window only spans ~3 stops, so when the
        // mood sits cool EVERY flower went blue, and mid-sweep everything went
        // green. A minority of blooms now ignore the mood entirely and take a
        // fixed accent hue — red, orange, hot pink or light blue — so those
        // colors are in the mix at every point of the cycle.
        'float isAccent(float seed){ return step(0.68, fract(seed * 31.7)); }',
        'float accentT(float seed){',
        '  float pick = floor(fract(seed * 47.3) * 4.0);',
        '  if (pick < 0.5) return 0.0;',    // red         (stop 0)
        '  if (pick < 1.5) return 0.125;',  // orange      (stop 1)
        '  if (pick < 2.5) return 0.375;',  // hot pink    (stop 3)
        '  return 0.75;',                    // light blue  (stop 6)
        '}',
        // mood-family hue for the majority, fixed accent hue for the rest
        'float bloomHue(float seed, float mood, float jitter){',
        '  float moodT = clamp(mood + jitter, 0.0, 1.0);',
        '  return mix(moodT, accentT(seed), isAccent(seed));',
        '}',
        ...
        // V15: unless this bloom drew an ACCENT, in which case it holds red,
        // orange, hot pink or light blue no matter where the mood sits.
        '    float hueJitter = (fract(seed * 13.7) - 0.5) * 0.44;',
        '    hueAcc += bloomHue(seed, mood, hueJitter) * w;',
        '    accAcc += isAccent(seed) * w;',
        ...
        '    hueAcc += bloomHue(h2, mood, hueJitter2) * w2;',
        '    accAcc += isAccent(h2) * w2;',
        ...
        '  float accentLevel = clamp(accAcc / max(acc, 1e-20), 0.0, 1.0);',
        ...
        // tour.
        // V15: mood blooms keep V14\'s full painterly ring drift; accent blooms
        // get a tighter one so red stays red and light blue does not smear off
        // into teal and lavender.
        '  float hueWobble = (0.20 - 0.12 * accentLevel) * sin(phase * 1.3);',
  ...
  draw(P, g, w, h, t, inp) {
    ...
    if (s.noGL || !P._three) {
      ...
      g.textAlign = 'center';
      g.fillText('FERRO BLOOM V15 · ACCENT BLOOMS', w / 2, h / 2 - 12);
