/* ---------- SRC-10.10 · WEATHER STATION V10 (attractor chaos) ---------- */
reg({
  id: 'SRC-10.10', family: 'SRC-10', ver: 11,
  title: 'Weather Station V10', tech: 'WIND + ATTRACTOR / WOVEN CHAOS',
  music: { bpm: 60, root: 43, mode: 'aeolian', prog: [0, 5], chordBars: 4 },
  fx: { bloom: 0.7, edge: true },
  tags: ['ATTRACTOR POLES', 'WOVEN MASS', 'INDEPENDENT CURL', 'WARM AT PEAK CHAOS'],
  desc: 'The same wind field, but now three poles wander inside it, and every thread near one gets pulled into orbit — spiraling, wrapping, weaving into a dense woven mass instead of just leaning downwind. Each thread also keeps a small wobble that is entirely its own, so the mass reads as a tangle, not a drawing. Cool white and violet is still home, but at the very peak of the gale the densest knot of threads flares warm — coral, gold, pink — before falling back.',
  interact: 'L = heading, same as always — the field snaps to a new direction instantly. R = everything at once now: more threads join, they move faster, the poles pull harder into tighter orbits, the independent wobble grows wilder, and at full reach the thickest knot of the weave catches fire with warm color. Low R is still just a scatter of tiny drifting marks.',
  ...
  _spawnGust(P, w, h) {
    return {
      ...
      r: Math.sqrt(w * h) * (0.16 + P.rand() * 0.2),
      life: 0, maxLife: 4.2 + P.rand() * 6,
      wander: P.rand() * TAU, wSpeed: 22 + P.rand() * 54, str: 0,
  ...
  step(P, dt, t, inp) {
    ...
    for (const m of s.mass) {
      // which pole (if any) dominates this thread, how deep inside it, and the orbital pull it exerts
      ...
      for (const gu of s.gusts) {
        ...
        if (d < gu.r) {
          ...
          const orbitR = gu.r * 0.28;
          const radial = (d - orbitR) * gb * chaos * 2.4;
          const tangential = gb * chaos * spd0 * 1.1;
      ...
      const own = clamp((base - m.pri) / 0.75); // wide band — most of the field can join in at full reach
