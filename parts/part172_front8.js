/* ---------- SRC-43.8 · CELL FRONT V8 (hands paint it) ----------
   Nima's verdict on V7: CC1/CC2 should mostly affect the COLOR of the
   clouds instead — inspired by Ferro Bloom's warm↔cool 7-stop gradient
   (ember orange / blush pink / hot orchid / acid chartreuse pivot / teal /
   lavender / electric blue) and Ridge Loom's violet↔orange↔cyan crossing.
   V5-V7 spent the hands entirely on SENSITIVITY, gating how hard audio hit
   the pockets — that's gone now, replaced outright ("instead"), because a
   hand doing two jobs (gain AND color) stops being legible as either. Audio
   ALONE drives size and reform (V6/V7 unchanged); hands ALONE drive color,
   on the house side law (L = warm country, R = violet/cool country): L's
   reach paints the BASS pocket across the palette's warm half, R's paints
   TREBLE across the cool half, and MID sits at the midpoint of wherever L
   and R currently are — so moving either hand shifts two of the three
   pockets at once, and moving both together repaints the whole field. True
   black, no shadow ring, three real pockets sized by their own band —
   everything else carries over unchanged. ------ */
const CF8_LOBES = s => [
  [-s.offBass, -0.04, s.Rbass], [0, 0.05, s.Rmid], [s.offTreble, -0.04, s.Rtreble]
];
const CF8_POOL = (s, x, y) => {
  let acc = 0;
  for (const l of CF8_LOBES(s)) {
...
reg({
  id: 'SRC-43.8', family: 'SRC-43', ver: 8, title: 'Cell Front V8', tech: 'VOLUMETRIC CELL FIELD / HAND-PAINTED AUDIO POCKETS',
  audioIn: true,
  fx: { bloom: 0.34 },
  tags: ['VOLUMETRIC', 'AUDIO IN', 'THREE POCKETS', 'HANDS = COLOR', 'SIZE REACTIVE', 'REFORMING'],
  desc: 'Same three pockets as V7 — BASS, MID and TREBLE, each their own lobe, sized by their own band, floating in true black with no shadow ring. The color is no longer fixed. Pulled from the same warm↔cool gradient Ferro Bloom uses — ember orange and blush pink at the warm end, teal, lavender and electric blue at the cool end, acid chartreuse the pivot — the BASS pocket paints from the warm half, TREBLE from the cool half, and MID sits at whatever midpoint the two currently share. A change in the sound still makes the field visibly reform (V7), and size is still the headline reaction to how loud things are (V6) — audio alone drives both now; the hands are free to just paint.',
  interact: 'This scene listens (SHOW CHECK → AUDIO IN, or MAP → Audio in) — the mic or a captured app is what the pockets react to and reform to. The hands paint instead of gating: L reaches across the warm half of the palette for the BASS pocket — ember orange near the source, out through blush pink and hot orchid toward chartreuse at full reach — R reaches across the cool half for TREBLE — chartreuse out through teal and lavender to electric blue — and MID follows the midpoint of wherever the two hands currently sit, so moving either one repaints two of the three pockets at once.',
  sound: 'Makes no sound of its own — an audio-in scene, same as V4-V7. Connect a source (mic, line-in, or CAPTURE APP AUDIO for a running app’s own output) in MAP → Audio in, then SET REST with the room quiet so silence reads as silence.',
  ...
  step(P, dt, t, inp) {
    ...
    // size is audio-only now (no hand gain) — V6's call: quiet should read
    // as nearly hidden, not a third-size resting mass.
    ...
    // FLUX (V7, unchanged): how fast bass/mid/treble are actually MOVING,
    // not how loud they are. A held level decays this to zero; a swing
    // spikes it, so the field visibly reforms exactly on a real change.
    ...
    /* ---- blooms: onset-driven only, same shape as V4-V7. A hit spawns one
    ...
    if (onsetEdge) {
      if (s.blooms.length < 6) {
        ...
        for (let k = 0; k < 12 && !ok; k++) {
          ...
          if (CF8_POOL(s, bx, by) > 0.4) ok = true;
        }
        if (ok) {
    ...
    for (let i = s.blooms.length - 1; i >= 0; i--) {
      const b = s.blooms[i];
      const inPaint = CF8_POOL(s, b.x, b.y) > 0.4;
  ...
  draw(P, g, w, h, t, inp) {
    ...
    if (s.noGL || !P._three) {
      ...
      g.fillText('CELL FRONT V8 · open on the hosted site (WebGL)', 10, h - 10);
      return;
    }
    ...
    u.uU.value = s.U;
    const ll = u.uLobe.value, LB = CF8_LOBES(s);
