/* ---------- SRC-34.4 · WHITE STUDY V4 ---------- */
reg({
  id: 'SRC-34.4', family: 'SRC-34', ver: 4, title: 'White Study V4', tech: 'DATA MINIMAL / GATED GRID',
  music: {
  ...
  tags: ['SEVERE', 'BLACK & WHITE', 'ETCHED PLATES', 'GATED GRID', 'SINE + CLICK'],
  desc: 'The machine learns to leave gaps. V3 ticked constantly — you steered it, but it never stopped talking. V4 gates the grid: sixteen steps per bar, and the right hand decides how many of them are allowed to sound, beats first, then the offbeat eighths, then the sixteenths. All the way down that is ONE tick per bar and real silence; the middle of the throw is where the grooves live — syncopated, machine-precise, with room in them; full extension is the old sixteenth-note wall. And the player finally gets a voice of their own: flick either hand outward and a stab fires on the next sixteenth — left hand low, right hand high — with a full-height white bar flashing on that side. The 30-second riser is gone. What remains between hits is silence, which was always the best sound this scene had.',
  interact: 'L = structural density (1 bar → 48 etched-plate crops) AND tone brightness — the lowpass opens with L. R = groove density: how many of the bar\'s 16 steps sound, filling beats-first (1 step → all 16), so the low end of the throw is a lone bar-length pulse, the middle is syncopated grooves, the top is the full sixteenth lattice. The lattice re-cuts only on steps that SOUND — sparse grooves hold still between hits. FLICK either hand outward to fire a stab on the next sixteenth: left = low tone, right = high ping, each with a full-height white bar flashing on that hand\'s side; flick speed = how hard it hits. The bar-locked pink window is unchanged: last bar of each 4-bar chord inverts a ~third of the frame, telegraphed a beat early. Idle: one quiet tick every two seconds, almost nothing, completely confident.',
  sound: 'Ikeda with a rhythm section\'s discipline: silence is inventory. The 16-step gate ranks steps beats-first (bit-reversed fill), so R buys groove, not just speed; every sounding step is a dry 6ms click (machine-flat, by design) plus a filtered dual-sine chord tone, now an octave higher than V3 — the pattern lives up top, the sub lives on the beat-gated pedal root, and the 110–440Hz mid band is deliberately EMPTY for whoever sits in. Scheduling moved from frame time to the audio-clock lookahead loop, so the grid is sample-tight against MIDI clock. Hand-flicks stab on the next sixteenth — left low (it borrows the empty mid band: the human gets the hole), right a bright ping — velocity scaled by flick speed. Chord changes still ring the quiet three-tone roll. The riser is gone. MIDI: pattern → arp ch4, player stabs → lead ch1, sub → bass ch3, rolls → bells ch5, clicks → perc ch10; CC74 lead = L, CC74 arp = R. In Ableton: sine patch with slap on arp, an expressive mono voice on lead — the lead channel is now literally the human\'s hands.',
  ...
  step(P, dt, t, inp) {
    ...
    s.beats = beats;
    // visual pulse: the lattice re-cuts only on steps that SOUND
  ...
  draw(P, g, w, h, t, inp) {
    ...
    // the player's stabs: a full-height white bar flashes on the striking hand's side
    for (const sb of s.stabs) {
      const age = s.beats - sb.atBeat;
      if (age < 0 || age > 0.9) continue;
      const a = 1 - age / 0.9;
      g.fillStyle = 'rgba(255,255,255,' + (a * 0.95).toFixed(3) + ')';
