/* ---------- SRC-41.8 · POUR CELLS V8 (eight pigments, one per note) ---------- */
reg({
  id: 'SRC-41.8', family: 'SRC-41', ver: 8, title: 'Pour Cells V8', tech: 'SOFT DOMES / A PIGMENT PER NOTE',
  music: {
  ...
  fx: { bloom: 0.4 },
  tags: ['A SOURCE UNDER THE FRAME', 'L = HEAT · R = THE CHORD', 'SOFT LIT DOMES', 'EVERY NOTE HAS ITS OWN PIGMENT', 'ONE BURST, ONE NOTE', 'THE QUEUE PLAYS IN ORDER'],
  ...
  /* EIGHT PIGMENTS, ONE PER RUNG. Colour used to be a function of where a
     cell sat in the frame, so the whole field only ever showed two of them at
     once: everything born left was ember, everything born right was magenta.
     Now the NOTE picks the pigment, straight off the pour — ember, scarlet,
     rose, violet, indigo, teal, mint, gold — so the arpeggio walking is what
     brings the palette out, and a cell's colour says what it will play.
     [bodyH, bodyS, bodyL, coreH, coreS, coreL]                            */
  _PAL: [
    [12, 92, 44, 38, 98, 62],    // 0 ember
    [352, 88, 46, 20, 96, 62],   // 1 scarlet
    [330, 86, 48, 344, 92, 68],  // 2 rose
    [280, 74, 46, 288, 84, 68],  // 3 violet
    [248, 72, 46, 232, 82, 68],  // 4 indigo
    [176, 76, 38, 166, 78, 60],  // 5 teal
    [158, 44, 42, 150, 46, 72],  // 6 mint-grey
    [40, 94, 46, 50, 98, 68]     // 7 gold
  ],
  // the pips at the vent wear the pigment their cell will arrive in
  _DEGC: ['#ff5a1e', '#f0203c', '#f02a86', '#9a3ae0', '#4a45e0', '#12b49c', '#9ecfc0', '#f0a41a'],
  ...
  draw(P, g, w, h, t, inp) {
    ...
    for (const c of s.cells) {
      ...
      /* THE NOTE IS THE PIGMENT. The side law survives as a nudge — the warm
         half of the frame lifts every pigment a little and the cool half cools
         it — but it no longer decides the colour, which is what was collapsing
         the whole field into two of them.                                  */
      const pal = this._PAL[c.deg % this._PAL.length];
      const sw = (warm - 0.5) * 2;
      const bodyH = (pal[0] + sw * 5 + 360) % 360, bodyS = pal[1], bodyL = pal[2] + sw * 4;
      const coreH = (pal[3] + sw * 5 + 360) % 360, coreS = pal[4], coreL = pal[5] + sw * 5;
      // shortest way round between body and core for the middle stop
      let dh = ((pal[3] - pal[0]) + 540) % 360 - 180;
      const midH = (pal[0] + dh * 0.5 + sw * 5 + 720) % 360;
      ...
      gr.addColorStop(0, `hsla(${coreH},${coreS - hot * 20}%,${coreL - 7 + hot * 26}%,${0.99 * bright})`);
      gr.addColorStop(0.38, `hsla(${midH},${(bodyS + coreS) * 0.5}%,${(bodyL + coreL) * 0.5 - 4 + c.flash * 12 + hot * 26}%,${0.99 * bright})`);
      gr.addColorStop(0.74, `hsla(${bodyH},${bodyS}%,${bodyL + c.flash * 10 + hot * 20}%,${0.99 * bright})`);
      gr.addColorStop(0.94, `hsla(${(bodyH - 12 + 360) % 360},${bodyS - 10}%,${bodyL * 0.45 + hot * 12}%,${0.99 * bright})`);
      gr.addColorStop(1, `hsla(${(bodyH - 20 + 360) % 360},${bodyS - 16}%,${bodyL * 0.3 + hot * 8}%,${0.99 * bright})`);
      g.fillStyle = gr;
      g.beginPath(); g.arc(c.x, c.y, r, 0, TAU); g.fill();
      ...
      if (r > U * 0.2) {
        ...
        hg.addColorStop(0, `rgba(255,246,232,${(0.09 + hot * 0.42) * bright})`);
