...
/* ---- the sheet ------------------------------------------------------------
   Periodic value noise: the lattice indices wrap, so the tile is seamless and
   one 384px sheet covers the show frame without a visible join. Baked once
   into a pattern and shared by every instance (the wall's thumbnail and the
   stage want the same paper). */
let PB7_PAPER = null;
const pb7Hash = (i) => {
  let n = (i * 374761393) | 0;
  n = (n ^ (n >>> 13)) * 1274126177;
...
function pb7Paper() {
  if (PB7_PAPER) return PB7_PAPER;
  const S = 512;
  const c = document.createElement('canvas');
  c.width = c.height = S;
  const x = c.getContext('2d');
  const im = x.createImageData(S, S), d = im.data;
  // separate lattice counts per axis is what buys the FIBRE: stretch the
  // noise along one axis and the grain gets a direction, like a laid sheet.
  const oct = (u, v, gx, gy, seed) => {
    const fx = u * gx, fy = v * gy;
    const i0 = Math.floor(fx) % gx, j0 = Math.floor(fy) % gy;
    const i1 = (i0 + 1) % gx, j1 = (j0 + 1) % gy;
    const su = (fx - Math.floor(fx)) * (fx - Math.floor(fx)) * (3 - 2 * (fx - Math.floor(fx)));
    const sv = (fy - Math.floor(fy)) * (fy - Math.floor(fy)) * (3 - 2 * (fy - Math.floor(fy)));
    const a = pb7Hash(i0 * 131 + j0 * 977 + seed * 7717);
    const b = pb7Hash(i1 * 131 + j0 * 977 + seed * 7717);
    const p = pb7Hash(i0 * 131 + j1 * 977 + seed * 7717);
    const q = pb7Hash(i1 * 131 + j1 * 977 + seed * 7717);
    const t0 = a + (b - a) * su, t1 = p + (q - p) * su;
    return t0 + (t1 - t0) * sv;
  };
  for (let y = 0; y < S; y++) {
    for (let px = 0; px < S; px++) {
      const u = px / S, v = y / S;
      // PAPER IS FINE AND LOW-CONTRAST. V7's first sheet clumped it into
      // 100px blobs and read as camouflage — real granulation is a per-pixel
      // tooth under a gentle mottle, with only a whisper of the laid sheet's
      // direction. The per-pixel term also hides the tile's own repeat.
      const fine = pb7Hash(px * 7919 + y * 104729 + 13);
      let n = 0.34 * fine
        + 0.30 * oct(u, v, 64, 64, 2)
        + 0.22 * oct(u, v, 24, 24, 1)
        + 0.14 * oct(u, v, 10, 10, 3);
      n += (oct(u, v, 16, 72, 4) - 0.5) * 0.14;
      n = Math.max(0, Math.min(1, (n - 0.42) * 1.5 + 0.42));
      const o = (y * S + px) * 4;
      d[o] = 0; d[o + 1] = 0; d[o + 2] = 0; d[o + 3] = (n * 118) | 0;
    }
  }
  x.putImageData(im, 0, 0);
  PB7_PAPER = x.createPattern(c, 'repeat');
  return PB7_PAPER;
}

/* ---- how much pigment each tile carries -----------------------------------
   V4 found the sun vertices and kept only the FAMILY. The wash needs the
   DISTANCE: one brushstroke across a rosette is heaviest at its heart and
   dries out toward its points, and that gradation is a property of the
   geometry, so it is computed once per deflation depth and never per frame. */
const PB7_W = {};
function pb7Wash(tiles, levels, rose) {
  if (PB7_W[levels]) return PB7_W[levels];
  const vt = new Map();
  ...
  for (const T of tiles) {
    for (let i = 0; i < 3; i++) {
      ...
      if (dd < e) e = dd;                        // the rhomb side, the unit here
    }
  }
  const d = new Float32Array(tiles.length);
  for (let i = 0; i < tiles.length; i++) {
    ...
    d[i] = rose[i]
      // the stroke: heavy at the heart, drying out through the points, and
      // never twice the same load — a brush does not meter itself
      ? Math.max(0, Math.min(1, (1 - r / 2.7) * (0.60 + 0.40 * hsh) + 0.20))
      // bare paper, except the odd diamond the brush caught anyway
      : (hsh > 0.87 ? 0.16 + 0.26 * pb7Hash(i + 7777) : 0);
  }
  d.edge = e;
  PB7_W[levels] = d;
  return d;
...
/* pigment is not a light. Pull the ramp's colour toward its own greyed
   version and a little toward the paper, so the spectrum still chooses WHICH
   pigment while never being able to choose neon. The RIM skips this — pooled
   pigment is concentrated, and that chroma step from body to edge is what
   makes a wash look like it dried. */
const PB7_SHEET = [247, 232, 206];
function pb7Pig(c, k) {
  const l = 0.30 * c[0] + 0.59 * c[1] + 0.11 * c[2];
  return [
    c[0] + ((l * 0.58 + PB7_SHEET[0] * 0.42) - c[0]) * k,
    c[1] + ((l * 0.58 + PB7_SHEET[1] * 0.42) - c[1]) * k,
    c[2] + ((l * 0.58 + PB7_SHEET[2] * 0.42) - c[2]) * k
  ];
}

reg({
  id: 'SRC-46.7', family: 'SRC-46', ver: 7,
  title: 'Penrose Bloom V7', tech: 'PENROSE DEFLATION / INK + WASH ON PAPER / AUDIO-GROWN',
  audioIn: true,
  fx: { bloom: 0.28 },
  tags: ['AUDIO IN', 'CC1 = SPIN', 'CC2 = ELASTICITY', 'WATERCOLOUR WASH', 'PAPER GRAIN'],
  desc: 'V6\'s crystal, repainted as ink and wash on paper. The geometry, the growth, the rosettes and both hands are untouched; what changes is the paint. Every washed diamond now carries pigment that pools bright at its edge and thins through its body, takes the chalky version of its colour in the middle and the undiluted one at the rim, and is granulated by a sheet of paper grain pinned to the screen — so the plate turns across the paper the way light turns across scrim. A rosette is washed as ONE stroke, heaviest at the star\'s heart and drying out toward its points, and a stray one-in-eight of the bare field catches a faint wash so the paper never reads as a clean diagram. The whole wash layer composites twice, once blurred and once sharp, which is the pigment wicking past the line it was meant to stay inside; the ink lattice stays crisp on top of it, at a line weight that varies tile by tile. Texture rides excitement with everything else: quiet is nearly bare paper and grain, loud is flooded and pooled. Makes no sound of its own.',
  ...
  init(P) {
    ...
    P.state = {
      tiles, rose, wash: pb7Wash(tiles, levels, rose), paper: pb7Paper(),
      n: 0, levels, life: 0,
  ...
  draw(P, g, w, h, t, inp) {
    ...
    wg.addColorStop(0, `rgba(18,30,92,${0.30 * bright})`);
    ...
    for (let i = 0; i < N; i++) {
      ...
      const load = wd * a * shim * fillA * (fam ? (0.55 + 0.95 * exc) : (0.26 + 0.34 * exc));
    ...
    for (let i = 0; i < N; i++) {
      ...
      g.beginPath();
      g.moveTo(PT[o], PT[o + 1]); g.lineTo(PT[o + 2], PT[o + 3]); g.lineTo(PT[o + 4], PT[o + 5]);
      g.closePath();
      if (f > 0.02) {
        const wf = f * 0.8;
        g.lineWidth = lw * jw * (1 + f * 0.9);
        g.strokeStyle = `rgba(${(r0 + (255 - r0) * wf) | 0},${(g0 + (246 - g0) * wf) | 0},${(b0 + (218 - b0) * wf) | 0},${Math.min(1, a * (ea + 0.46 * f))})`;
      } else {
        g.lineWidth = lw * jw;
    ...
    g.fillText('BASS ' + Math.round(s.bass * 100) + '  MID ' + Math.round(s.mid * 100) +
      ...
      '  WASH ' + Math.round(exc * 100) + '  RIM ' + rimW.toFixed(1) +
