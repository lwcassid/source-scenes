/* ---------- SRC-28.18 · CHLADNI COURT V18 (idle gives nothing away) ---------- */
/* Lance on V17: still giving it away. Two dials, same direction:
   - EMBER FLOOR 20% -> 8%: the resting murmur is now genuinely faint;
     the breath swell is the only time the sleeping bed speaks up.
   - NO LINES AT IDLE, EVER: the pattern-forming drive is now ZERO without
     presence (was a 2% floor), and the tease no longer gathers - the
     breath is sound plus a formless SHIVER of the grains. Nodal lines
     exist exclusively under human hands; sleep is pure scatter riding the
     slow stream. The wake surge still slams it all into geometry the
     moment someone arrives. */
reg({
  id: 'SRC-28.18', family: 'SRC-28', ver: 18, title: 'Chladni Court', tech: 'STANDING WAVES / SAND ON A PLATE',
  music: {
  ...
  tags: ['CYMATICS', 'EARNED RESONANCE', 'IDLE GIVES NOTHING AWAY', 'BREATHING SLEEP', 'WAKE SURGE'],
  desc: 'Sleep that keeps the secret. An idle court is pure strewn sand riding a whisper-slow stream - the nodal lines cannot form without hands, at all, ever - and the bed rests at a bare ember, swelling into a breath every while as the grains shiver formlessly, then settling back into space. Nothing about the resting state shows what the plate can do. Then someone arrives: the wake surge slams all that scatter into geometry, the bed steadies to full presence, and the earned detents are waiting - low modes for a slow stranger, the fine ratios for a hand that has learned the plate. The reveal belongs entirely to the first touch.',
  ...
  step(P, dt, t, inp) {
    ...
    const jit = dt * ((0.05 * (1 - s.res) + 0.03) * (0.15 + 0.85 * s.pres) + tease * 0.035);
  ...
  audio(A, P) {
    ...
    return {
      tick() {
        ...
        const amb = (s.pres || 0) + (1 - (s.pres || 0)) * (0.08 + 0.92 * (s.tease || 0));
