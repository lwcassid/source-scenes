...
reg({
  id: 'SRC-28.24', family: 'SRC-28', ver: 24, title: 'Chladni Court V24', tech: 'STANDING WAVES / SAND ON A PLATE',
  music: {
      ...
      tick() {
        ...
        if (s.beatOn && T.running) {
          ...
          while (beatNextT < bh && guard++ < 24) {
            ...
            // THE ARP RIDES THE BEAT (V24, Lance's law: rhythmic, with the
            // drums, on the grid -- never nature-timed): a gated 8th-note
            // chord-tone line on the arp channel, alive only in this window
            if (st % 2 === 0) {
              const deg = [0, 2, 4, 2, 1, 3, 4, 2][(st / 2) % 8];
              A.pluck2(H.chordTone(deg, st % 8 === 6 ? 1 : 0), {
                role: 'arp', at: beatNextT, dur: 0.22,
                vol: (st % 4 === 0 ? 0.05 : 0.032) * fade
              });
            }
