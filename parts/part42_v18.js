/* ---------- SRC-18.18 · NIGHT CIRCUIT V18 (a jam grammar: hands together = the band, reach a hand out = solo that side) ---------- */
reg({
  id: 'SRC-18.18', family: 'SRC-18', ver: 18,
  title: 'Night Circuit V18', tech: 'WEBGL / PROJECTION-READY',
  ...
  tags: ['JAM GRAMMAR', 'HANDS TOGETHER = THE DROP', 'HANDS WIDE = BREAKDOWN', 'SOLO EACH SIDE'],
  desc: 'The JAM round. The whole instrument is now built from how your two hands relate. Bring both hands TOGETHER (toward the source) and it is the drop — the drums and bass kick in and you surge forward. Throw both hands WIDE and it is the breakdown — the kit falls away, you ease off and glide, hands-off-the-bars chill, still moving but calm. Keep ONE hand tight and reach the OTHER out and you keep your speed while you SOLO that side: reach left for the orange voice, right for the violet. And you cannot miss which side you are playing — the whole edge of the road turns into a fat glowing stroke, orange on the left, violet on the right, pulsing with every note, built to read across the scrim from across the room. Everything from V17 stays (first-person camera, the manta glide, the broadside crab, the geometric tunnel with speed streaks, the tardigrade finale).',
  interact: 'HANDS TOGETHER → the drop: drums + bass in, you speed up. HANDS WIDE → the breakdown: kit out, you ease off and glide. ONE HAND TIGHT + THE OTHER OUT → keep your speed and SOLO that side (reach left = orange arps/bells, reach right = violet stabs/pad). The road-edge on the side you play becomes a fat glowing stroke that pulses with the notes — orange left, violet right — so a stranger can see, from across the room, exactly what you are playing.',
  sound: 'Its own engine — 78-ish BPM aeolian minor. Togetherness (both hands in) is the ARRANGEMENT DIAL: it brings the drums and driving bass in and out — the drop and the breakdown. Each hand reaching OUT plays its side — left = arps/bells, right = stabs/pad — independent of the beat, so you can solo over the groove or over silence. Sub under everything, hats double-time in the tunnel, the mourner lead over cave and sunset. Same rig roles and channels.',
  init(P) {
    P.state = {
    ...
    // V17 · the 3-lane grid is RETIRED (it read as a game). The music feedback
    // lives in the road-EDGE lines now — see the edge glow in step(): left burns
    // orange with the arps, right burns violet with the stabs.
  ...
  step(P, dt, t, inp) {
    ...
    // V18 · THE JAM GRAMMAR — everything is how the two hands RELATE:
    //   both hands together (in)  → the drop: drums + bass + surge
    //   both hands wide (out)     → the breakdown: kit out, ease off, glide
    //   one tight + one reached    → keep speed, SOLO the reached-out side
    const closeL = 1 - inp.L, closeR = 1 - inp.R;      // 1 = hand IN (tight/together)
    const together = clamp((closeL + closeR) / 2);     // the arrangement dial → the band
    const maxClose = Math.max(closeL, closeR);
    const playL = clamp(inp.L * s.pres);               // reach LEFT out → solo the orange voice
    const playR = clamp(inp.R * s.pres);               // reach RIGHT out → solo the violet voice
    s.together = clamp(together * s.pres); s.playL = playL; s.playR = playR;
    const thr = s.together; // general intensity read by beams/fog/tunnel build
    ...
    // SPEED: hands together surge; ONE hand tight still drives fast (so you can
    // solo the other side without losing pace); BOTH wide eases off to a glide.
    const drive01 = 0.30 + 0.52 * maxClose + 0.18 * together; // both wide→0.30, one in→~0.9, both in→1.0
    s.drive01 = clamp(drive01 * s.pres);
    s.speed += ((6 + s.drive01 * spdMax) - s.speed) * Math.min(1, dt * 1.4);
    // steering still reads the differential — reach a hand out and you lean that way
    const steerIn = clamp(inp.R - inp.L, -1, 1);
    s.steer += (steerIn - s.steer) * Math.min(1, dt * 1.6);
    s.lane += (s.steer * 2.4 - s.lane) * Math.min(1, dt * 1.4);
    s.bob += dt * (2 + s.speed * 0.3);
    s.kickPulse = Math.max(0, s.kickPulse - dt * 5);
    s.arpFlash = Math.max(0, s.arpFlash - dt * 5);
    s.stabFlash = Math.max(0, s.stabFlash - dt * 5); // V17: the right line's music pulse
    if (P.focused && typeof MOut !== 'undefined') {
      ...
      for (let i = MOut.log.length - 1; i >= 0; i--) {
        ...
        else if (ev.role === 'pad' || ev.role === 'bells') s.stabFlash = 1; // stabs/pad drive the violet line
      }
      s.lastEvP = now;
    }
    ...
    const dz = s.speed * dt;
    const act = s.act;
    // V18 · YOU PLAY A SIDE BY REACHING THAT HAND OUT. sideL/sideR are now the
    // reach of each hand (not a lane) — so the orange/violet stroke, the wash,
    // and each side's melody all answer the hand that is actually reaching.
    const sideL = playL, sideR = playR;
    s.sideL = sideL; s.sideR = sideR;
    // the side washes: your country lights up and beats with ITS music (V17:
    // stronger, and each side answers its own voice — left arps, right stabs)
    if (T3.sideLightL) {
      T3.sideLightL.intensity = sideL * (2.6 + s.kickPulse * 1.6) + s.arpFlash * (0.5 + sideL * 1.6);
      T3.sideLightR.intensity = sideR * (2.6 + s.kickPulse * 1.6) + s.stabFlash * (0.5 + sideR * 1.6);
    }
    // act visibilities
    ...
    // V17 · THE LINES SING. Each road-line carries its side's color as a standing
    // identity (orange left / violet right, even centered) and IGNITES with that
    // side's own music — the left line flashes on every arp, the right on every
    // stab — so you can see, at a glance, the music answering your hands.
    // V18 · the played side's stroke IGNITES — the color only really blooms when
    // that hand reaches out, and it fattens + flares hard so there's zero doubt
    const idL = clamp(0.22 + sideL * 0.78), idR = clamp(0.22 + sideR * 0.78);
    const pulseL = s.arpFlash * (0.8 + sideL * 2.4) + s.kickPulse * 0.3;
    const pulseR = s.stabFlash * (0.8 + sideR * 2.4) + s.kickPulse * 0.3;
    T3.edgeMats[0].color.copy(T3._cb).lerp(T3._co, idL).multiplyScalar(1 + sideL * 2.0 + pulseL);
    T3.edgeMats[1].color.copy(T3._cb).lerp(T3._cp, idR).multiplyScalar(1 + sideR * 2.0 + pulseR);
    if (T3.strEdges) { T3.strEdges[0].scale.x = 1 + sideL * 3.0; T3.strEdges[1].scale.x = 1 + sideR * 3.0; } // the stroke fattens on the side you play
    // the centre dashes carry the spine — a soft flash on the kick
    T3.dashMat.color.lerp(sideL > sideR ? T3._co : T3._cp, Math.max(sideL, sideR) * 0.4);
  ...
  draw(P, g, w, h, t, inp) {
    ...
    if (s.noGL || !P._three) {
      ...
      g.textAlign = 'center';
      g.fillText('NIGHT CIRCUIT V18 · THE JOURNEY', w / 2, h / 2 - 12);
    ...
    // V18 · SIDE-PLAY GLOW — a fat screen-edge wash you cannot miss, even across
    // the scrim from across the room: reach left and the left third of the frame
    // floods orange, right floods violet, each pulsing with its own voice.
    const pL = s.playL || 0, pR = s.playR || 0;
    const flashL = Math.min(1, (s.arpFlash || 0));
    const flashR = Math.min(1, (s.stabFlash || 0));
    g.save();
    g.globalCompositeOperation = 'lighter';
    if (pL > 0.04) {
      const a = pL * (0.28 + flashL * 0.5);
      const gr = g.createLinearGradient(0, 0, w * 0.42, 0);
      gr.addColorStop(0, `rgba(255,120,36,${a.toFixed(3)})`);
      gr.addColorStop(1, 'rgba(255,120,36,0)');
      g.fillStyle = gr; g.fillRect(0, 0, w * 0.42, h);
    }
    if (pR > 0.04) {
      const a = pR * (0.28 + flashR * 0.5);
      const gr = g.createLinearGradient(w, 0, w * 0.58, 0);
      gr.addColorStop(0, `rgba(196,92,255,${a.toFixed(3)})`);
      gr.addColorStop(1, 'rgba(196,92,255,0)');
      g.fillStyle = gr; g.fillRect(w * 0.58, 0, w * 0.42, h);
    }
    g.restore();
    const ACTS = ['THE CAVERN', 'NEON DISTRICT', 'THE TUNNEL', 'PALM SUNSET'];
    const tog = s.together || 0;
    const mode = s.pres < 0.3 ? 'WAITING IN THE DARK'
      : tog < 0.28 ? 'BREAKDOWN'
      : (pL > 0.3 && pL > pR + 0.15) ? 'PLAYING ORANGE'
      : (pR > 0.3 && pR > pL + 0.15) ? 'PLAYING VIOLET'
      : tog > 0.6 ? 'THE DROP' : 'RIDING';
    g.fillStyle = 'rgba(150,200,220,0.75)'; g.font = `${Math.round(10 * Math.max(1, Math.sqrt(areaScale(P))))}px ui-monospace,monospace`;
    g.fillText(ACTS[P.state.act] + ' · ' + Math.round(s.speed * 6) + ' KM/H · ' + mode + ' · [1-4] SKIP ACT', 10, h - 10);
  },
  audio(A, P) {
    ...
    return {
      tick(inp) {
        const s = P.state;
        // V18 · the JAM grammar in sound:
        //   togetherness (both hands in) → the BAND: drums + driving bass
        //   each hand reaching OUT       → that side's MELODY (L arps, R stabs)
        // so `lift`/drum-intensity follows the band, and the arp/stab layers
        // follow each hand's reach — you solo over the groove, or over silence.
        const band = clamp(s.together || 0);            // the drop ↔ the breakdown
        const pL = clamp(s.playL || 0), pR = clamp(s.playR || 0);
        const closeL = pL, closeR = pR;                 // "reach" drives each side's voice
        const sideL2 = pL, sideR2 = pR;
        const lift = band;                              // drums + driving bass ride the band
        const energy = clamp(Math.max(band, pL, pR));   // any activity opens the filter
        const act = s.act || 0;
        ...
        A.set(surf.frequency, 260 + Math.pow(energy, 1.4) * (act === 2 ? 2200 + build * 5600 : 5200), 0.2);
        MOut.expr('arp', pL); MOut.expr('bass', band); MOut.expr('pad', pR);
