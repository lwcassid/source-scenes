...
reg({
  id: 'SRC-42.16', family: 'SRC-42', ver: 16, title: 'Ridge Loom V16', tech: 'THE LOOM LISTENS / BOOTH = FLOOR HAND',
  music: {
    ...
    return {
      tick(inp, dt) {
        ...
        while (nextT < horizon && guard++ < 24) {
          ...
          const c5 = s.cores[5];
          if (gridOn && c5 && c5.wH > 0.8 && (st === 6 || st === 13)) {
            ...
            A.bell(H.chordTone(deg, 1), {
              at: nextT, vol: (0.02 + clamp(c5.wH / 5) * 0.016) * acc * gate, dur: 2.4, pan: 0.25, rev: 0.75
            });
          }
        ...
        if (typeof MOut !== 'undefined') {
          const c4a = s.cores[4];
          const arpOn = gridOn && c4a && c4a.wH > 0.8;
          if (arpOn) {
            const depth = clamp((c4a.wH - 0.8) / (s.wMax - 0.8));
