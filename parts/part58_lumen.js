/* ---------- SRC-38 · LUMEN FILM (light through the seams) ---------- */
reg({
  id: 'SRC-38', family: 'SRC-38', ver: 1, title: 'Lumen Film', tech: 'SOAP FILM / APERTURE ORGAN',
  music: {
  ...
  fx: { bloom: 0.5 },
  tags: ['BACKLIT SOAP FILM', 'EVERY HOLE IS A VOICE', 'THIN FILM COLOUR', 'FILLS THE FRAME'],
  desc: 'The whole frame is one sheet of soap film seen from behind, cell pressed against cell all the way to the edges. The film itself is nearly black — what you can see of it is the interference colour riding its seams, the same oil-slick spectrum a real bubble wall throws. There is a lamp behind it. Where the film thins to nothing, at the junctions where three cells meet, the lamp gets through, and every hole that opens is a sustained voice added to the chord. Filled with holes the sheet stops being a wall and becomes a lantern.',
  interact: 'R = LIGHT PRESSURE — the lamp behind the sheet. Draw in and the film is closed and the frame is nearly dark, a faint iridescent lacework and nothing else. Reach out and the junctions blow open one at a time, from the middle of the sheet outward, each one adding its own held note to the chord: the frame fills with light and the chord fills with voices, and you can hold it anywhere on the way. A hole always tells you before it commits — it leaks a little glow half a beat early, then blooms with its note on the sixteenth. L = FILM THICKNESS. Thin film is violet-blue and glassy and moves the voices up into their high partials; thick film is gold and crimson and heavy. Both hands together: thin the film and open it wide for a huge glass chord, or leave it thick and open one hole for a single deep beam.',
  ...
  draw(P, g, w, h, t, inp) {
    ...
    // ---- THE FILM: nearly black bodies, interference colour on the seams
    for (const c of s.cells) {
      const hue = (c.hue + s.thick * 210 + Math.sin(c.ph * 0.6) * 26) % 360;
      const bg = g.createRadialGradient(c.x - c.r * 0.2, c.y - c.r * 0.25, c.r * 0.1, c.x, c.y, c.r);
      bg.addColorStop(0, `hsla(${hue},60%,12%,0.9)`);
      bg.addColorStop(0.7, `hsla(${(hue + 40) % 360},55%,8%,0.85)`);
      bg.addColorStop(1, `hsla(${(hue + 80) % 360},70%,${16 + s.lamp * 16}%,0.9)`);
      g.fillStyle = bg;
      g.beginPath(); g.arc(c.x, c.y, c.r, 0, TAU); g.fill();
    }
    // the seams themselves — oil-slick rims, fattened for the mesh
    g.globalCompositeOperation = 'lighter';
    for (const c of s.cells) {
      const hue = (c.hue + s.thick * 210 + 180) % 360;
      g.lineWidth = Math.max(2.4 * ms, c.r * 0.045);
      g.strokeStyle = `hsla(${hue},92%,${44 + s.lamp * 22}%,${(0.3 + s.lamp * 0.35) * bright})`;
      g.beginPath(); g.arc(c.x, c.y, c.r * 0.995, 0, TAU); g.stroke();
      g.lineWidth = Math.max(1.2 * ms, c.r * 0.018);
      g.strokeStyle = `hsla(${(hue + 60) % 360},96%,72%,${(0.16 + s.lamp * 0.3) * bright})`;
      g.beginPath(); g.arc(c.x, c.y, c.r * 0.96, 0, TAU); g.stroke();
    }

    // ---- THE LAMP: everything the film failed to stop
    for (const p of s.ap) {
      ...
      // the shaft: light through a thin place spreads along the seam
      if (p.open > 0.2) {
        const A = s.cells[p.a], B = s.cells[p.b];
        const ang = Math.atan2(B.y - A.y, B.x - A.x) + Math.PI / 2;
        const L = R * (1.3 + p.open * 2.2);
        const lg = g.createLinearGradient(p.x - Math.cos(ang) * L, p.y - Math.sin(ang) * L,
          p.x + Math.cos(ang) * L, p.y + Math.sin(ang) * L);
        lg.addColorStop(0, 'rgba(255,255,255,0)');
        lg.addColorStop(0.5, `hsla(${hue},96%,84%,${p.open * 0.34 * bright})`);
        lg.addColorStop(1, 'rgba(255,255,255,0)');
        g.strokeStyle = lg;
        g.lineWidth = Math.max(2.5 * ms, R * 0.22 * p.open);
        g.beginPath();
        g.moveTo(p.x - Math.cos(ang) * L, p.y - Math.sin(ang) * L);
        g.lineTo(p.x + Math.cos(ang) * L, p.y + Math.sin(ang) * L);
        g.stroke();
      }
    }
