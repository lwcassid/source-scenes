/* ---------- SRC-38.8 · LUMEN FIELD (light, restored) ---------- */
reg({
  id: 'SRC-38.8', family: 'SRC-38', ver: 8, title: 'Lumen Film', tech: 'GEM FILM / APERTURE ORGAN',
  music: {
  ...
  fx: { bloom: 0.46 },
  tags: ['LIGHT, RESPONSIVE AGAIN', 'MINIMAL GRADIENT VARIATION', 'SCATTERED SPARKS', 'NO BLUR FILTER'],
  desc: 'V8: a performance pass, not a look pass. The per-pixel blur filter that softened the seams and the lamp was the single biggest cost in the frame — canvas blur is expensive per draw call, and this scene makes hundreds of them a frame — so it is gone; softness now comes only from gradients fading to nothing, same look, far cheaper. Facet gradients dropped two colour stops and a chunk of their random spread: they run close to their natural ridge-to-corner direction with only a small angle and length jitter, the restrained variation the reference file actually has, not a wide-open random spin. Everything reads a little dimmer, a little lighter. Junctions no longer open from the middle outward on a shared clock — each one\'s threshold is independent of where it sits, and each grows and fades at its own rate, so sparks turn up anywhere in the frame, edges included, on their own timeline.',
  interact: 'R = LIGHT PRESSURE, unchanged — the lamp behind the sheet. Draw in and the film is closed, a faint faceted lacework and nothing else. Reach out and junctions blow open — now scattered across the whole frame rather than radiating from the centre, each with its own grow/fade speed — each one adding its own held note to the chord. A hole always tells you before it commits — it leaks a little glow half a beat early, then blooms into a corner-flare with its note on the sixteenth. L = ORIENTATION — an absolute dial: hand at the sphere and the crystals sit at rest; reach out and the whole field turns with your hand, up to a full turn at full reach. Every open hole\'s spark and rays are pinned to one specific corner of the stones that made it. Sonically L still runs the same glassy-to-warm sweep it always has.',
  ...
  init(P) {
    ...
    const cells = [];
    for (let i = 0; i < N; i++) {
      // six facets per stone (three nested triangles per half). MINIMAL
      // variation, matching the reference file: each facet's gradient runs
      // close to its own natural ridge-to-corner line, nudged by a small
      // angle jitter and a small length jitter — not a wide random spin.
      const facets = [];
      for (let k = 0; k < 6; k++) facets.push({ angJit: (P.rand() - 0.5) * 0.5, lenMul: 0.85 + P.rand() * 0.32 });
      cells.push({
  ...
  draw(P, g, w, h, t, inp) {
    ...
    // THE PALETTE — the reference illustration's own linear gradient, six
    // stops instead of its eight (dropped the near-white peak and the
    // indigo band — cheaper per fill, and reads a touch lighter for it).
    const SVGSTOPS = [
      [0, '0,0,0'], [0.22, '37,85,238'], [0.42, '100,215,160'],
      [0.62, '255,214,90'], [0.82, '225,61,47'], [1, '0,0,0']
    ];
    ...
    // 25% of the way across (painted last, on top).
    ...
    for (const c of s.cells) {
      ...
      for (const half of halves) {
        for (const f of FRACS) {
          ...
          // true facing (for brightness) uses the facet's NATURAL angle,
          // untouched by the small gradient jitter below, so rotation
          // still sweeps the light correctly
          const ndx = Q[0] - half.apex[0], ndy = Q[1] - half.apex[1];
          const natAngle = Math.atan2(ndy, ndx), natLen = Math.hypot(ndx, ndy);
          const lit = 0.5 + 0.5 * Math.cos(natAngle - LIGHT_DIR);
          const alphaEnv = clamp(0.03 + lit * 0.34 * gate + s.lamp * 0.2);
          // gradient anchored at the facet's own inner point, running close
          // to its natural direction — only a SMALL jitter in angle and
          // length, the minimal variation the reference itself has
          const ang = natAngle + fac.angJit, len = natLen * fac.lenMul;
          const gx = cx0 + Math.cos(ang) * len, gy = cy0 + Math.sin(ang) * len;
          const fg = g.createLinearGradient(cx0, cy0, gx, gy);
