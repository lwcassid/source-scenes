/* ---------- SRC-38.4 · LUMEN FIELD (six-cut stones) ---------- */
reg({
  id: 'SRC-38.4', family: 'SRC-38', ver: 4, title: 'Lumen Film', tech: 'GEM FILM / APERTURE ORGAN',
  music: {
  ...
  fx: { bloom: 0.46 },
  tags: ['SIX-FACET STONES', 'SPARKS ONLY AT CORNERS', 'RGB HALO', 'FULL SPECTRUM PER STONE'],
  desc: 'V4: every stone is cut from exactly six triangles now — a vertical ridge line splits each diamond and each half fans into three facets, so every gem carries the whole visible spectrum across its own faces instead of one colour. Fixed sunlight crossed with each facet\'s own angle still decides which ones flare, so the spectrum shifts as the field turns. A spark never floats in the black between stones any more — it is pinned to the actual touching corner of the two diamonds that made it, tracked live as they rotate, and every one of those sparks now throws a thin red/green/blue halo at its edge, the way a bright point does through real glass.',
  ...
  init(P) {
    ...
    for (let i = 0; i < N; i++) {
      cells.push({
        ...
        hueShift: (P.rand() - 0.5) * 22,   // small per-stone spectrum offset
  ...
  draw(P, g, w, h, t, inp) {
    ...
    // six wavelengths, red through blue — never violet/pink, per spec — one
    // per facet slot, so every single stone shows the whole run at once
    const SPECTRUM = [6, 30, 50, 128, 190, 224];
    ...
    // ---- THE FILM: six facets per stone. A ridge line runs T→B through
    // two interior points (U near the top, D near the bottom, both exactly
    // on the T–B diagonal) splitting the kite into six triangles, three a
    // side. Each triangle's gradient runs from the ridge out to its own
    // outer vertex (R or L) at its own angle — that's the 3D read — and
    // each carries its own step of the spectrum, so no stone is one colour.
    // Litness (fixed overhead sun × facet's own angle) still gates how much
    // that colour shows, and is scaled by presence so the sheet stays dark
    // faceted lacework at rest.
    for (const c of s.cells) {
      ...
      const tris = [
        [T, U, R, 0], [U, D, R, 1], [D, B, R, 2],
        [T, U, L, 3], [U, D, L, 4], [D, B, L, 5]
      ];
      for (const [inA, inB, outV, hi] of tris) {
        ...
        const hue = (SPECTRUM[hi] + c.hueShift + 360) % 360;
        const Lval = 4 + lit * 34 * gate + s.lamp * 12;
        const sat = 82 - lit * 8 * gate;
        const fg = g.createLinearGradient(mx, my, outV[0], outV[1]);
        fg.addColorStop(0, `hsla(${hue.toFixed(0)},${(sat * 0.6).toFixed(0)}%,3%,0.95)`);
        fg.addColorStop(0.62, `hsla(${hue.toFixed(0)},${sat.toFixed(0)}%,${(Lval * 0.55).toFixed(0)}%,0.9)`);
        fg.addColorStop(1, `hsla(${hue.toFixed(0)},${sat.toFixed(0)}%,${Lval.toFixed(0)}%,0.87)`);
        g.fillStyle = fg;
    ...
    for (const c of s.cells) {
      ...
      g.strokeStyle = `hsla(206,60%,${28 + s.lamp * 36}%,${(0.05 + s.lamp * 0.4) * bright})`;
      strokePoly(pts);
    }
    g.filter = 'none';
    ...
    for (const p of s.ap) {
      ...
      // RGB halo — every light throws a thin red/green/blue fringe at its
      // edge, like chromatic dispersion off a bright point through glass
      const haloA = a * bright * 0.55;
      const HALO = [[2, 1.18], [130, 1.36], [222, 1.56]];
      for (const [hue, rMul] of HALO) {
        g.strokeStyle = `hsla(${hue},95%,58%,${(haloA / rMul).toFixed(2)})`;
        g.lineWidth = Math.max(1.4 * ms, R * 0.1);
        g.beginPath(); g.arc(sx, sy, R * rMul, 0, TAU); g.stroke();
      }
      ...
      if (p.open > 0.2) {
        ...
        for (const dr of dirs) {
          ...
          const lg = g.createLinearGradient(sx, sy, sx + cx, sy + cy);
          ...
          g.beginPath(); g.moveTo(sx, sy); g.lineTo(sx + cx, sy + cy); g.stroke();
        }
      }
    }
    g.filter = 'none';
