/* ---------- SRC-44 · GREEN FUSE (the plant keeps what you give it) ----------
   Every other scene in the library is a STATE: put your hands somewhere and
   the picture is somewhere. This one is an ACCUMULATOR. Your hands are the
   light; the plant grows toward them and then KEEPS what it grew. Reach up
   and the shoot follows your hand THIS FRAME — that is the one-second read,
   and it is continuous, not quantised. On the beat the shoot sets: it stops
   being a thing you are holding and becomes a thing that happened, and a new
   shoot starts from its tip. Play for two minutes and the frame you are
   standing in front of is a record of every reach you made.
   AGE is the integral of that light, and age is what turns a sprout into an
   overgrowth: it opens the eras (seed / sprout / bloom / thicket / canopy),
   each entered on a bar line, and each era branches harder, blooms wider,
   subdivides the growth clock finer and widens the hue spread — so the two
   clean colour countries of the side law bleed into the full spectrum only
   at the end, where the psychedelia has been earned. Leave and it does not
   reset: the light falls, the coronas fold shut, the understory is absorbed
   and the whole thing dies back over about three minutes to a pair of glowing
   tubers on the floor, waiting for the next pair of hands to light it again.
   MATERIAL. One fragment shader over the whole frame. Every organ — tuber,
   stem, bulb, corona — is a capsule or a spiked disc in one metaball field,
   so neighbours FUSE instead of overlapping, and the hue is accumulated as a
   vector and averaged by weight, so where two organs melt together their
   colours melt too. Airbrush, not line art: no strokes anywhere, cream cores
   in the dense middles, a luminous rim at the silhouette, black pupils at the
   bloom centres, and true black everywhere the field is empty. -------- */

const GF_NORG = 72;          // uniform budget = the organ budget
const GF_TOP = 0.82;         // above this a shoot cannot climb — it spreads instead
const GF_ERA = [0, 0.08, 0.24, 0.48, 0.74];
const GF_ERANAME = ['SEED', 'SPROUT', 'BLOOM', 'THICKET', 'CANOPY'];
...
function GF_plantHue(P, s, side) {
  const base = side === 'L' ? 0.025 : 0.600;
  const h = base + (P.rand() - 0.5) * (0.07 + s.age * 0.20);
  return h - Math.floor(h);
}
function GF_hue(P, s, tip) {
  const h = tip.hue0 + (P.rand() - 0.5) * 0.05 + tip.gen * 0.028;
  return h - Math.floor(h);
}

/* keep the array hard-capped: the shader reads exactly GF_NORG slots */
function GF_worst(s, splicing) {
  let worst = -1, ws = 1e9;
...
function GF_add(s, o) {
  ...
  o.n = ++s.born;
  if (s.org.length >= GF_NORG) {
    const worst = GF_worst(s, true);
...
function GF_target(s) { return 10 + Math.round(s.age * (GF_NORG - 22)); }
function GF_prune(s) {
  const target = GF_target(s);
  let guard = 0;
  while (s.org.filter(q => !q.dead).length > target && guard++ < 3) {
    if (!GF_absorb(s)) break;
  }
}
function GF_livePlants(s, side) {
  const seen = {};
...
const GF_STATION = [0.05, 0.225, 0.40, 0.575, 0.75];
function GF_slot(P, s, side) {
  ...
  let bx = GF_STATION[0] * dir, bd = -1;
  for (const st of GF_STATION) {
    const x = st * dir;
    let d = 9;
...
function GF_seed(P, s, side, x) {
  const hue = GF_plantHue(P, s, side);
  const pid = ++s.pid;
...
function GF_grow(P, s, side, light) {
  const alive = s.tips.filter(t => t.alive && t.side === side);
  const capTips = [1, 2, 3, 4, 5][s.era];
  if (!alive.length) {
    GF_seed(P, s, side, GF_slot(P, s, side));
    return;
  }
  ...
  if (!done && tip.segs < maxSegs && sh && tip.y < GF_TOP) {
    ...
    const spread = (tip.side === 'L' ? 1 : -1) * (tip.y / GF_TOP) * 0.30;
  ...
  GF_add(s, {
    ...
    hue: GF_hue(P, s, tip), lobes, open: 0.02, openT: 1,
...
const GF_FS = [
  ...
  'uniform vec2 uRes;',
  'uniform float uT, uPres, uAge, uSky, uDrift, uDusk;',
  ...
  '  col *= (0.40 + 0.60 * uPres) * (0.55 + 0.45 * (1.0 - uDusk));',
...
reg({
  id: 'SRC-44', family: 'SRC-44', ver: 1, title: 'Green Fuse', tech: 'ACCUMULATING GROWTH / FUSED ORGAN FIELD',
  music: {
  ...
  init(P) {
    const s = {
      ...
      age: 0, era: 0, pendEra: 0, lastBar: -1,
    ...
    P.state = s;
    s.asp = P.w / P.h;
    GF_seed(P, s, 'L', -GF_STATION[0]);
    GF_seed(P, s, 'R', GF_STATION[0]);
    if (s.noGL) return;
    ...
    const uni = {
      ...
      uO0: { value: new Float32Array(GF_NORG * 4) },
      uO1: { value: new Float32Array(GF_NORG * 4) }
    };
    T3.uni = uni;
    const mat = new THREE.ShaderMaterial({
      uniforms: uni,
      vertexShader: 'void main(){ gl_Position = vec4(position.xy, 0.0, 1.0); }',
      fragmentShader: GF_FS
    });
  ...
  step(P, dt, t, inp) {
    ...
    const idle = 0.20 + 0.13 * Math.sin(s.life * 0.23);
    s.LL = s.L * s.pres + idle * (1 - s.pres);
    s.RR = s.R * s.pres + idle * (1 - s.pres);
    ...
    let want = 0;
    for (let i = GF_ERA.length - 1; i >= 0; i--) if (s.age >= GF_ERA[i]) { want = i; break; }
    s.pendEra = want;
    ...
    for (const side of ['L', 'R']) {
      ...
      const full = s.org.filter(q => !q.dead).length >= GF_target(s);
      if (lgt > 0.11 && P.rand() < (0.16 + lgt * 0.84) * (0.22 + 0.78 * s.pres) * (full ? 0.18 : 1)) {
        GF_grow(P, s, side, lgt);
    ...
    for (const side of ['L', 'R']) {
      const n = GF_livePlants(s, side);
      const lgt = side === 'L' ? s.LL : s.RR;
      if (n < wantPlants && lgt > 0.13 && P.rand() < dt * 2.2 * s.pres) GF_seed(P, s, side, GF_slot(P, s, side));
    }
    GF_prune(s);
    ...
    for (let i = s.org.length - 1; i >= 0; i--) {
      ...
      if (o.dead) {
        ...
        o.openT = clamp(0.16 + (o.side === 'L' ? s.LL : s.RR) * 0.95);
        o.open += (o.openT - o.open) * Math.min(1, dt * 1.7);
  ...
  draw(P, g, w, h, t, inp) {
    ...
    for (let i = 0; i < GF_NORG; i++) {
      const o = s.org[i], k = i * 4;
      if (!o || o.r <= 0.0008) { a1[k] = 0; a1[k + 1] = 0; a1[k + 2] = 0; a1[k + 3] = 0; continue; }
      const sa = wind * o.ay * o.ay * 3.2 + Math.sin(s.life * 0.8 + o.ph) * 0.0022;
      const sb = wind * o.by * o.by * 3.2 + Math.sin(s.life * 0.8 + o.ph) * 0.0022;
      a0[k] = o.ax + sa; a0[k + 1] = o.ay;
      a0[k + 2] = o.bx + sb; a0[k + 3] = o.by;
      a1[k] = o.r * (1 + 0.045 * Math.sin(s.life * 0.7 + o.ph));
    ...
    g.fillText('AGE ' + Math.round(s.age * 100) + '%   ERA ' + s.era + '/4 ' + GF_ERANAME[s.era] +
      '   L ' + Math.round(s.LL * 100) + '  R ' + Math.round(s.RR * 100) +
      '   ORGANS ' + s.org.length + '/' + GF_NORG + '   BLOOMS ' + blooms +
