/* ---------- SRC-10.18 · WEATHER STATION V18 (the bells wait for hands)
   The idle-audit round: with nobody at the instrument, the ambient
   drift kept the heading-bell figure firing -- 81 bells in 40 seconds
   to an empty room -- and held the storm loop open. Both were leaks of
   the same class the audit was built to catch (idle is REST, ghosts
   must never perform). V18: the bell figure and the storm-rack hold
   are PRESENCE-gated, and the sfx CC74 stream is floored by presence so
   the storm chain sits shut at rest. The idle lure (V17's gusts, the
   1-in-7 far-off bell) is unchanged -- that is the rest state; the
   grid figure belongs to a player. (V17 notes below.) --------------- */
...
reg({
  ...
  title: 'Weather Station V18', tech: 'DENSITY SMOKE / VESPERS-STYLE PLOT',
  ...
  audio(A, P) {
    ...
    return {
      tick() {
        ...
        MOut.expr('sfx', clamp((dens - 0.25) / 0.6) * (0.2 + 0.8 * (s.pres || 0)));
        const nowA = A.t();
        if ((s.pres || 0) > 0.2 && dens > 0.3 && typeof MOut !== 'undefined') {
