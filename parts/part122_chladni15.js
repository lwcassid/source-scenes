/* ---------- SRC-28.15 · CHLADNI COURT V15 (earned detents · ambient bed · still sand) ---------- */
/* Lance's third round:
   1. EARNED RESONANCE - V14's detents made every lock walk-up easy; the
      satisfaction is in figuring it out. Windows narrowed AND scaled by
      mode number: low integers stay landable for strangers, high modes
      tighten to expert-narrow - the difficulty curve is the mastery curve.
   2. AMBIENT BED AT IDLE - rest is not silence: the chord pads + sub now
      run at full ambient level always (the drone that holds the room);
      only the INTERACTIVE voices (plate tones, blooms, bass) sleep.
   3. STILL SAND - the constant reseed (20%/s grain teleport, an
      anti-trapping mechanism) and full-rate jitter kept the idle field
      visibly boiling. Both now scale with presence: a dead plate's sand
      truly comes to rest - strewn and still - until a tease or a hand. */
reg({
  id: 'SRC-28.15', family: 'SRC-28', ver: 15, title: 'Chladni Court', tech: 'STANDING WAVES / SAND ON A PLATE',
  music: {
  ...
  tags: ['CYMATICS', 'EARNED RESONANCE', 'AMBIENT BED', 'STILL SAND', 'WAKE SURGE'],
  ...
  audio(A, P) {
    ...
    return {
      tick() {
        ...
        // THE BED NEVER SLEEPS: pads + sub hold the room at ambient level with
        // or without hands - only the interactive voices are presence-gated.
        pads.forEach(p => { p.level((0.014 + r2 * 0.045) * breath, 0.4); p.bright(260 + r2 * 1400, 0.4); });
        A.set(subG.gain, 0.012 + r2 * 0.012, 0.5);
