/* ---------- SRC-43.11 · CELL FRONT V11 (the kick lands on the sample) ----------
...
const CF11_POOL = (s, x, y) => {
  let acc = 0;
  for (const l of CF11_LOBES(s)) {
...
reg({
  id: 'SRC-43.11', family: 'SRC-43', ver: 11, title: 'Cell Front V11', tech: 'VOLUMETRIC CELL FIELD / HAND-PAINTED AUDIO POCKETS',
  audioIn: true,
  fx: { bloom: 0.34 },
  tags: ['VOLUMETRIC', 'AUDIO IN', 'THREE POCKETS', 'HANDS = COLOR', 'KICK SWELL', 'SAMPLE-ACCURATE', 'REFORMING'],
  desc: 'V10 with the lag taken out. The kick now comes from a dedicated low-passed time-domain detector in the audio engine that timestamps each hit to the sample, and the clouds swell on the very frame it is known — the swell is even back-dated by the hit’s true age so the picture is right for the moment it reaches the screen. A 16th-note bassline no longer steals kicks (the old quarter-second lockout is gone), and the swell itself is no longer filtered on the way to the radius. Same capped field, same gating, same hand-painted palette as V10.',
  ...
  step(P, dt, t, inp) {
    ...
    if (onsetEdge) {
      if (s.blooms.length < 4) {
        ...
        for (let k = 0; k < 12 && !ok; k++) {
          ...
          if (CF11_POOL(s, bx, by) > 0.4) ok = true;
        }
        if (ok) {
    ...
    for (let i = s.blooms.length - 1; i >= 0; i--) {
      const b = s.blooms[i];
      const inPaint = CF11_POOL(s, b.x, b.y) > 0.4;
  ...
  draw(P, g, w, h, t, inp) {
    ...
    u.uU.value = s.U;
    const ll = u.uLobe.value, LB = CF11_LOBES(s);
