...
const LUMEN21_STOPCACHE = new Map();
reg({
  id: 'SRC-38.21', family: 'SRC-38', ver: 21, title: 'Lumen Film V21', tech: 'GEM FILM / APERTURE ORGAN',
  music: {
  ...
  draw(P, g, w, h, t, inp) {
    ...
    const addSvgStops = (grad, a) => {
      let set = LUMEN21_STOPCACHE.get(a);
      if (!set) {
        set = SVGSTOPS.map(([t, rgb]) => [t, `rgba(${rgb},${a})`]);
        LUMEN21_STOPCACHE.set(a, set);
      }
