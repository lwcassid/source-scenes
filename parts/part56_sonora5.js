/* ---------- SRC-32.5 · SONORA V5 (stone pan · magnet hands) ---------- */
/* V4's stone bed — same grey geologies, same heat solver that warms a knock
   into a handpan — rebuilt around ONE rule so the movement finally reads:
   THE HANDS ARE MAGNETS AND THE ROCKS FOLLOW. The bed is strung between two
   magnets, one per hand. Reach a hand out and its magnet slides to that wall,
   dragging its end of the bed with it — the rocks go WHERE THE HAND GOES,
   which is what V4 (a squeeze that shoved them the OTHER way) got backwards.
   Each stone holds its own station on the line between the two hands, so the
   whole bed is one connected sheet that stretches, gathers and slides as you
   move. Fast hand = the rocks rush to catch it. The always-on outline is gone
   and the surface is higher-contrast, both as Kasia asked. */
reg({
  id: 'SRC-32.5', family: 'SRC-32', ver: 5, title: 'Sonora', tech: 'STONE PAN / MAGNET HANDS',
  music: { bpm: 60, root: 48, mode: 'ionian', prog: [0, 5, 3, 4], chordBars: 4 },
  fx: { bloom: 0.32, edge: true },
  tags: ['GREY STONE / FOUR GEOLOGIES', 'HANDS ARE MAGNETS', 'ROCKS FOLLOW THE GESTURE', 'ROCK WARMS INTO HANDPAN'],
  ...
  interact: 'Each hand is a magnet and the rocks FOLLOW it. The bed is strung between your two hands: the left hand holds its left end, the right hand its right end. The left hand draws its end INWARD as you reach out and lets it fall back to the wall as you relax; the right hand carries its end OUT to the wall as you reach. Pull a hand back and its stones come with it. Bring both hands in and the whole bed gathers to the middle, crowding and knocking; spread both out and it stretches wide across the tank. It is the MOTION that plays it — the rocks move at the speed of your hands, so a slow drift is a soft roll of contacts and a fast sweep makes them rush and clatter. Hold the bed crowded and it stops clattering and GRINDS — stone sliding on stone — and every contact feeds heat into the stones it touches, so the bed warms up: dry at first, then ringing, then singing. Let go and it cools. The slow play is the sympathy: a stone in tune with something already sounding stays warm long after you stopped working it, so the field holds its brightness in patches you did not touch.',
  ...
  init(P) {
    ...
    /* THE BED IS STRUNG BETWEEN THE TWO HANDS. Give every stone a fixed
       STATION in the bed: px is its place left-to-right (0 = the left hand's
       end, 1 = the right hand's end), py its place top-to-bottom. The stone
       keeps that station as the hands move it around — so the bed is one
       connected sheet whose left edge follows the left hand and right edge the
       right hand, not two herds. Rank by starting position so the bed keeps a
       natural, non-crossing layout. */
    const n = rocks.length;
    [...rocks].sort((a, b) => a.x - b.x).forEach((st, i) => st.px = n > 1 ? i / (n - 1) : 0.5);
    [...rocks].sort((a, b) => a.y - b.y).forEach((st, i) => st.py = n > 1 ? i / (n - 1) : 0.5);

    P.state = {
      rocks, dust: [], rings: [], energy: 0, grind: 0, heat: 0, ring: 0,
      lastRoot: H.root, retune: 0, pres: 0
    };
    // first tuning
  ...
  step(P, dt, t, inp) {
    ...
    /* ---- THE HANDS ARE MAGNETS. THE ROCKS FOLLOW. ----
       Each hand is a magnet on a rail. Reach the LEFT hand out and its magnet
       slides to the LEFT wall; reach the RIGHT out and its magnet slides to the
       RIGHT wall; relax and both magnets ease back toward a calm spread near
       the middle. The bed is STRUNG between the two magnets: each stone is
       pulled to its own station on the line from the left magnet to the right
       magnet, so the left end of the bed follows the left hand and the right
       end follows the right hand. Move a hand and its stones go WITH it — the
       thing V4 got backwards. */
    // LEFT reverted (Kasia): the left magnet's travel is flipped, so reaching
    // the left hand out pulls its end INWARD (toward centre) instead of out to
    // the wall. The end still lives in the left region — only the sense flips.
    const magL = w * (0.055 + (0.30 - 0.055) * inp.L);  // inp.L 0 → .055w (wall)  · 1 → .30w (inward)
    const magR = w * (0.70 + (0.945 - 0.70) * inp.R);   // inp.R 0 → .70w  · 1 → .945w (outward)
    const magLV = (magL - (s.pMagL !== undefined ? s.pMagL : magL)) / Math.max(dt, 1e-4);
    const magRV = (magR - (s.pMagR !== undefined ? s.pMagR : magR)) / Math.max(dt, 1e-4);
    s.pMagL = magL; s.pMagR = magR;
    s.handSpeed = (Math.abs(magLV) + Math.abs(magRV)) / w;   // for the HUD

    // presence: a hand that is actually there (calibrated, away from rest) is
    // 'live'. Gate the sound off an abandoned bed — it still gathers to centre
    // and drifts, but it must not grind or warm with nobody working it.
    const live = (chan.L.mode === 'live' || chan.R.mode === 'live') ? 1 : 0;
    s.pres += (live - s.pres) * Math.min(1, dt * 1.8);
    const actGate = s.pres;

    /* KX pulls a stone to its station; FF adds a shove in the direction the
       station is MOVING, scaled by hand speed, so a fast sweep makes the rocks
       genuinely rush to catch the hand instead of oozing. KY holds a loose
       vertical station so the bed keeps its body. */
    const KX = 9, FF = 0.42, KY = 3.4;
    for (const st of s.rocks) {
      // the stone's target: its station on the line between the two magnets
      const tx = magL + (magR - magL) * st.px;
      const txv = magLV + (magRV - magLV) * st.px;   // how fast that station moves
      const ty = h * (0.16 + 0.68 * st.py);

      // FOLLOW — spring to the moving station, plus the speed feed-forward
      st.vx += ((tx - st.x) * KX + txv * FF) * dt;
      st.vy += (ty - st.y) * KY * dt;
      // a breath of life so a held pose is not dead-still
      st.vx += Math.sin(t * 0.5 + st.ph) * dt * 4;
      st.vy += Math.cos(t * 0.4 + st.ph * 1.7) * dt * 4;
      ...
      // headroom for a fast gesture — the bed must be allowed to actually RUSH
      const sp = Math.hypot(st.vx, st.vy), CAP = 300;
    ...
    for (let pass = 0; pass < 4; pass++) {
      ...
      for (const st of s.rocks) {
        ...
        if (v > 300) { const k = 300 / v; st.vx *= k; st.vy *= k; }
      }
    }
    ...
    for (const st of s.rocks) {
      // gate the drive by presence — an abandoned bed cools back to cold stone
  ...
  draw(P, g, w, h, t, inp) {
    ...
    g.fillText('L-END ' + Math.round(inp.L * 100) + '%  R-END ' + Math.round(inp.R * 100) +
  ...
  audio(A, P) {
    ...
    return {
      tick(inp) {
        const s = P.state;
        if (!s || !s.rocks) return;
        const act = s.pres || 0;   // silence the grind/shimmer on an empty bed
