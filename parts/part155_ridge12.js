...
reg({
  id: 'SRC-42.12', family: 'SRC-42', ver: 12, title: 'Ridge Loom V12', tech: 'V10 + BREATHING REST, EARNED GRID, REAL CONVERGENCE',
  music: {
  ...
  sound: 'The playing instrument is V10\'s band with dynamics: every grid voice now derives its velocity from the gesture (core width and reach speed) with downbeat accents, the arpeggio\'s density is earned (quarters at low reach, eighths near full), and the shared-ring convergence is real — arp and bell degrees are pulled toward the long tone\'s pitch as rings enclose more cores, instead of a 2-cent detune nobody could hear. The RESTING state is rebuilt to the studio\'s idle law: the bar-clock bass root is gone; below presence the grid is silent and the scene breathes instead — randomized low swells (length, depth, voice and spacing all rolled per breath, roughly one in seven landing a deeper toll) over a loom-noise floor that undulates on two incommensurate slow LFOs, never dead, never periodic. Each breath pumps the wave field so the sound visibly moves the picture. Ableton: bass = the picture-written figures, lead = the long tone + accents only, ARP SYNTH holds the arpeggio\'s convergence note on the grid (CC74 = earned density; the patch arpeggiates), bells = the answers; the glue bed stays browser-side. Velocities carry the dynamics.',
