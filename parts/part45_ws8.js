/* ---------- SRC-10.8 · WEATHER STATION V8 (long, thin, ever-shifting) ---------- */
reg({
  id: 'SRC-10.8', family: 'SRC-10', ver: 9,
  title: 'Weather Station V8', tech: 'ONE FIELD / LONG THIN STRANDS',
  music: { bpm: 60, root: 43, mode: 'aeolian', prog: [0, 5], chordBars: 4 },
  fx: { bloom: 0.65, edge: true },
  tags: ['ONE PARTICLE FIELD', 'REACH, NOT WIDTH', 'STRANDS CAN SPAN THE SKY', 'SWARM NEVER SETTLES'],
  desc: 'Still one field, same tiny background threads at rest. Raise your right hand and three wandering streams pull threads into strands — but reach is now the whole story, not width: threads stay thin as thread no matter how far they run, and the rare few caught in the heart of a stream can uncoil clear across half the sky. Each strand carries its own faint cast of color, teal to violet, so no two read quite the same. And the tiny background swarm, left alone too long, would clump into corners like real dust in a corner draft — so every so often a few threads let go and reappear somewhere fresh.',
  interact: 'L = heading — the whole field snaps to a new direction almost instantly, and a fast turn briefly straightens everything into one aligned sweep. R = the streams: at rest, tiny marks everywhere, always quietly redistributing; raise the hand and three wandering currents pull threads into long, thin, varied strands — some brief, some spanning enormous reach — in drifting clusters of density surrounded by empty space.',
  ...
  init(P) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      mass.push({
        ...
        pri: P.rand(), lenP: Math.pow(P.rand(), 2.3), widP: P.rand(), hueJ: (P.rand() - 0.5) * 70, ph: P.rand() * TAU, boost: 0
      });
    }
    const NG = 4, gusts = [];
  ...
  draw(P, g, w, h, t) {
    ...
    for (const m of s.mass) {
      ...
      // emergent long line — thin regardless of reach; width and length are separate traits
      const lenFrac = clamp((trail.length - 5) / 260);
      const a = (0.18 + lenFrac * 0.5 + m.boost * 0.3) * pres;
      const wgt = (0.55 + m.widP * 0.55 + m.boost * 0.35) * ms;
      ...
      gr.addColorStop(0.6, `hsla(${hue},26%,76%,${a})`);
      gr.addColorStop(1, `hsla(${hue},14%,${92 + m.boost * 6}%,${Math.min(1, a * 1.3)})`);
      g.strokeStyle = gr;
      g.lineWidth = wgt;
