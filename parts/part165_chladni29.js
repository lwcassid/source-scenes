...
reg({
  id: 'SRC-28.29', family: 'SRC-28', ver: 29, title: 'Chladni Court V29', tech: 'STANDING WAVES / SAND ON A PLATE',
  music: {
    ...
    return {
      tick() {
        ...
        if (s.beatOn && T.running) {
          // THE LOOP SEAT (V29): pad 54 struck once on a bar line at the
          // first unlock, held all scene; perc CC74 does every fade
          if (typeof MOut !== 'undefined' && !s._loopHeld && MOut.wants()) {
            s._loopHeld = true;
            const barLen = T.beat * 4;
            const nextBar = T.t0 + Math.ceil((A.t() - T.t0) / barLen) * barLen;
            MOut.evNote('perc', 185.0, 0.22, nextBar, 3600);
          }
