/* ---------- SRC-43.13 · CELL FRONT V13 (white at rest, color is an event) ----------
...
const CF13_POOL = (s, x, y) => {
  let acc = 0;
  for (const l of CF13_LOBES(s)) {
...
const CF13_FS = [
  ...
  '  return mix(c, RL_VIOLET, clamp(wm * uMel, 0.0, 0.85));',
  '}',
...
reg({
  id: 'SRC-43.13', family: 'SRC-43', ver: 13, title: 'Cell Front V13', tech: 'VOLUMETRIC CELL FIELD / HAND-PAINTED AUDIO POCKETS',
  audioIn: true,
  fx: { bloom: 0.34 },
  tags: ['VOLUMETRIC', 'AUDIO IN', 'THREE POCKETS', 'WHITE AT REST', 'SPEED = COLOR', 'MELODY = VIOLET', 'KICK SWELL'],
  desc: 'V12 with white as home. When nothing much is moving the clouds are white light. Color is an event that fades: how fast the left hand moves paints orange into the left side, how fast the right hand moves paints cyan into the right, and both settle back to white over a few seconds. The melody brings the third color — when a phrase lifts the mids clearly above where they were, a violet wash blooms around the middle pocket and fades, no more often than every few seconds. Ridge Loom’s three colors, nothing else. Kick swell, capped field and gating are V11 unchanged.',
  interact: 'This scene listens (SHOW CHECK → AUDIO IN, or MAP → Audio in) — the kick swells the clouds, the bassline and melody size the field, a new melodic phrase washes the middle violet for a moment. The hands paint by MOVING: a fast reach or pull of the left hand floods the left side orange, of the right hand floods the right side cyan; stillness lets everything fade back to white.',
  ...
  init(P) {
    const s = {
      ...
      velL: 0, velR: 0, _pL: -1, _pR: -1, mel: 0, _melGap: 9, _melN: 0,
  ...
  step(P, dt, t, inp) {
    ...
    // MELODY (V13): the mid band lifting clearly above its own slow running
    // level = a phrase entering. Fires a violet wash (snap up, ~3s fade),
    // at most every 4s — an occasion, never a strobe. Stillness → white.
    s._melGap += dt;
    const melLift = clamp(inp.audio.mid) - s.mid;
    if (melLift > 0.14 && s._melGap > 4 && s.pres > 0.3) {
      s._melGap = 0; s._melN++;
      s.mel = Math.max(s.mel, clamp(0.6 + melLift * 2));
    }
    s.mel -= s.mel * Math.min(1, dt * 0.45);
    ...
    if (onsetEdge) {
      if (s.blooms.length < 4) {
        ...
        for (let k = 0; k < 12 && !ok; k++) {
          ...
          if (CF13_POOL(s, bx, by) > 0.4) ok = true;
        }
        if (ok) {
    ...
    for (let i = s.blooms.length - 1; i >= 0; i--) {
      const b = s.blooms[i];
      const inPaint = CF13_POOL(s, b.x, b.y) > 0.4;
  ...
  draw(P, g, w, h, t, inp) {
    ...
    if (s.noGL || !P._three) {
      ...
      const W = [245, 245, 255], V = [73, 0, 255], O = [243, 108, 59], C = [0, 237, 255];
      ...
      g.fillText('CELL FRONT V13 · open on the hosted site (WebGL)', 10, h - 10);
      return;
    }
    ...
    u.uU.value = s.U;
    const ll = u.uLobe.value, LB = CF13_LOBES(s);
