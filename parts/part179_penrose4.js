...
/* the sun vertices, and every tile's family around them. Cached per depth —
   the geometry is fixed, so this is computed once and never per frame. */
const PB4_ROSE = {};
function pb4Rose(tiles, levels) {
  if (PB4_ROSE[levels]) return PB4_ROSE[levels];
  const vt = new Map();
  ...
  // a SUN: ten fat corners, zero thin. Nothing else in the tiling looks
  // like this, which is why the plate's rosettes land where they land.
  const suns = [];
  for (const [k, v] of vt) {
    if (v[0] === 0 && v[1] === 10) {
      const p = k.split(',');
      suns.push([+p[0] / 1e4, +p[1] / 1e4]);
    }
  }
  let e = 1e9;
  for (const T of tiles) {
    for (let i = 0; i < 3; i++) {
      const j = (i + 1) % 3;
      const d = Math.hypot(T.x[i] - T.x[j], T.y[i] - T.y[j]);
      if (d < e) e = d;                       // the rhomb side, the unit here
    }
  }
  const rose = new Uint8Array(tiles.length);
  for (let i = 0; i < tiles.length; i++) {
    const T = tiles[i];
    let best = 1e9;
    for (const s of suns) {
      const d = Math.hypot(T.cx - s[0], T.cy - s[1]);
      if (d < best) best = d;
    }
    const r = best / (e || 1);
    rose[i] = r < 1.55 ? 1 : r < 2.2 ? 2 : 0;
  }
  PB4_ROSE[levels] = rose;
  return rose;
}
...
reg({
  id: 'SRC-46.4', family: 'SRC-46', ver: 4,
  title: 'Penrose Bloom V4', tech: 'PENROSE DEFLATION / SUN ROSETTES / AUDIO-GROWN',
  audioIn: true,
  fx: { bloom: 0.4 },
  tags: ['AUDIO IN', 'ROSETTES OUT OF BARE PAPER', 'LOUD RESOLVES THE PATTERN', 'THE BEAT IS LIGHT', 'FIVE-FOLD, NEVER REPEATS'],
  desc: 'V3 with the colour ARRANGED the way the reference plate arranges it: most of the tiling is bare paper, and the colour picks out five-fold star rosettes standing out of it. Nothing is decorated — the plate\'s pattern is a diagnosis of the tiling\'s own sun vertices, the points where five fat rhombs meet, and this scene finds them the same way (exactly ten fat triangle-corners and no thin ones, twenty-one of them at show size). The ten fat tiles at each sun are the rosette\'s heart, the ring of thin darts around it are its points, and everything else is the paper between. Heart and points take opposite ends of a stretch of Ferro Bloom\'s ramp, so a rosette is warm inside and cooler at its tips, the way the plate\'s gold centres sit inside violet ones; the paper takes Ferro Bloom\'s own cream, dim, which on black is the plate inverted into light. AND THE PATTERN EMERGES WITH LOUDNESS: quiet, paper and rosette are nearly the same brightness and the crystal reads as one even lattice; loud, the paper drops away and the stars blaze out of it. The spectrum still tilts the whole ramp warm or cool and still re-deals on a change in the harmonic balance. Makes no sound of its own.',
  ...
  init(P) {
    ...
    P.state = {
      ...
      fbase: 0.24, front: 0.24, hwm: 0.24, vel: 0,
  ...
  step(P, dt, t, inp) {
    ...
    /* ---- THE FRONT ----------------------------------------------------- */
    // V3: a narrower, higher band than V2 — loud and quiet are still clearly
    // different crystals, but they are no longer a different scene. The kick
    // keeps a small push here (0.16) because the beat is carried by LIGHT
    // now, not by the boundary.
    const baseTarget = 0.22 + 0.52 * s.field;
