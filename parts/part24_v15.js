/* ---------- SRC-18.15 · NIGHT CIRCUIT V15 (projection-ready light, one continuous camera, sides that answer) ---------- */
reg({
  id: 'SRC-18.15', family: 'SRC-18', ver: 15,
  title: 'Night Circuit V15', tech: 'WEBGL / PROJECTION-READY',
  ...
  tags: ['PROJECTION-READY LIGHT', 'ONE CONTINUOUS CAMERA', 'SIDES THAT ANSWER', 'CYBER CRAB'],
  desc: 'The feedback round, answered. The cavern keeps its mood but lifts its floor of light — built to survive a projector at night on uneven playa surfaces, with rock that reads and stalagmites rooted in the ground instead of floating over it. The fake headlight cone and its misaligned road blob are gone: the beams are real lights and the pool on the asphalt is where they actually land. There is now ONE camera: a continuous chase that eases closer and lower as you engage — never a cut, never a mode switch, and the machine rides much larger in frame. Leaning is finally legible everywhere: an orange wash rises out of the left world and a violet wash out of the right, in every act, with your side brightening and the far side falling away. The tunnel calmed down and earns its density again. And the crab is no longer a nature documentary — it crosses the highway as a flickering hologram now, which is much funnier.',
  interact: 'Same hands, one grammar: push toward the source for throttle and beam-flare, lean for steering — and the lean now ANSWERS in color, everywhere: orange country wakes on your left, violet on your right, near side up, far side down. No camera modes to rip you out of the zone; the view just breathes with your presence.',
  ...
  init(P) {
    ...
    T3.mkGlow = mkGlow;
    {
      ...
      for (let i = 0; i < 16; i++) {
        ...
        g2.position.set(sx, 10.7 - Math.abs(sx) * 0.35, -(455 + i * 13 + rnd() * 6)); // up against the ceiling rock — no more hovering teeth
        caveG.add(g2);
    ...
    const tunG = new THREE.Group(); world.add(tunG); T3.tunG = tunG;
    {
      ...
      for (let i = 0; i < 26; i++) {
        ...
        ring.position.set(0, 2.4, -i * 20 - 4); // V15: wider spacing — the bore breathes between gates
  ...
  step(P, dt, t, inp) {
    ...
    s.lane += (s.steer * (tunnel ? 2.2 : 2.6) - s.lane) * Math.min(1, dt * 1.4); // the lean moves you FURTHER toward the wall you're waking
    ...
    if (act === 0) {
      ...
      if (T3.tunHero) {
        ...
        if (T3.tunLeds) for (const ld of T3.tunLeds) {
          ld.m.opacity = ld.mag
            ? 0.06 + s.arpFlash * (0.08 + bld * 0.38) + Math.max(sideL, sideR) * 0.34 + antic * 0.35
            : 0.06 + bld * 0.2 + s.kickPulse * (0.16 + bld * 0.45) + thr * 0.1;
        }
      }
      ...
      T3.rings.forEach((r2, i2) => {
        ...
        r2.m.opacity = 0.09 + bld * 0.15 + chase * (0.2 + bld * 0.55);
  ...
  draw(P, g, w, h, t, inp) {
    ...
    if (s.noGL || !P._three) {
      ...
      g.textAlign = 'center';
      g.fillText('NIGHT CIRCUIT V15 · THE JOURNEY', w / 2, h / 2 - 12);
