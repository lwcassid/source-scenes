/* ---------- SRC-18.12 · NIGHT CIRCUIT V12 (zones, sections, sides everywhere) ---------- */
reg({
  id: 'SRC-18.12', family: 'SRC-18', ver: 12,
  title: 'Night Circuit V12', tech: 'WEBGL / CAVE ZONES / TUNNEL SECTIONS',
  ...
  step(P, dt, t, inp) {
    ...
    if (act === 0) {
      ...
      for (const tb of T3.caveTubes) {
        ...
        tb.userData.wireMat.opacity = 0.16 + s.kickPulse * 0.2 + s.arpFlash * 0.1 + thr * 0.06; // dialed UP — the surface is the drama
      }
      ...
      for (const p2 of (T3.palmModels || T3.palms)) {
        ...
        if (p2.userData.wireMat) {
          ...
          p2.userData.wireMat.opacity = 0.015 + Math.max(0, 1 - Math.abs(p2.position.z + 40) / 110) * (0.05 + nearSide * (0.1 + s.kickPulse * 0.14));
        }
      }
  ...
  draw(P, g, w, h, t, inp) {
    ...
    if (s.noGL || !P._three) {
      ...
      g.textAlign = 'center';
      g.fillText('NIGHT CIRCUIT V12 · THE JOURNEY', w / 2, h / 2 - 12);
