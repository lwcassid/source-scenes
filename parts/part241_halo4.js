/* ---------- SRC-49.4 · SPECTRUM HALO V4 (the far hand goes purple) ----------
   Lance: "change color to purple when the right hand is moved farther away."
   One addition, one subtraction, every V3 number untouched.

   THE ADDITION. The RIGHT hand's DISTANCE now owns a colour. inp.R arrives
   NEAR = HIGHER, so "farther away" is the low end of the throw: close in,
   the plate is exactly the white / slate / coral plate V3 ships; pull the
   hand back and it goes PURPLE. The purple is not a tint laid over the
   screen. It is STAMPED INTO EACH SLICE AT BIRTH (`sl.purp`), the same way a
   slice wears the spectral balance it was born with — so pulling the hand
   away turns the LIVE EDGE purple at once and the rest of the plate follows
   as the exposure ages through it, three seconds from edge to core. Push
   the hand back in and white returns from the live edge inward. That keeps
   the scene's long-exposure grammar: colour is the form's own history, and
   a hand held still cannot repaint a slice that was born before it moved.
   The mix rides on top of the spectral colour, never instead of it: slate
   strata go deep purple, coral strata go magenta, so the layers of the
   stack still differ from each other with the hand parked far.

   THE THROW. Purple starts at cc2 0.80 and is FULL by 0.15 — so the near
   quarter of the reach is V3's plate untouched, and the last 15% of the
   sensor's range (where tracking gets flaky) changes nothing, which is what
   the FIELD EDGE law asks: the maximum is not parked at the sensor's edge.
   No hand live → target 0 → the plate stays white, so an unattended scene
   is never driven purple by the wall's ghost drift. Entry eases at ~5/s
   (the live ring answers as you move); loss decays at ~1.2/s, so a hand
   drifting out of the field melts back to white instead of snapping.

   THE PRICE, PAID. Purple [178,92,255] is 136 luma against the core's 251,
   and V3 spent a whole round buying back brightness on mesh. So a purple
   stamp draws its edges with alpha x(1 + 0.30*purp) — the same payment V3
   made for a bigger figure, and for the same reason: under `lighter` it
   lands in the dense band, not the outskirts.

   THE SUBTRACTION. V1-V3's RIGHT-hand SPEED paint (a whip breathed VIOLET
   into the live edge) is gone. With the right hand's position now owning
   purple, a violet whip on the same hand is the same colour from the same
   hand for a different reason — a control that exists twice. The LEFT hand's
   orange whip stays exactly as shipped.

   Everything else — the audio engine window, the diverging slate/coral ramp,
   the sensitivity gamma, the kick path, the soft caps, the HUD — is V3 verbatim.
   ------ V3 notes follow ------
...
const H4_SLICES = 96;          // ring buffer depth — 3.0s at 32 stamps/sec
const H4_PTS = 112;            // samples around the curve (order 17 needs ~68)
const H4_CAP = 1 / 32;         // seconds between stamps — fixed, not per-frame
...
const H4_COS = new Float32Array(H4_PTS), H4_SIN = new Float32Array(H4_PTS);
for (let j = 0; j < H4_PTS; j++) {
  const a = j / H4_PTS * TAU;
  H4_COS[j] = Math.cos(a); H4_SIN[j] = Math.sin(a);
}
...
const H4_COLD = [70, 99, 158];     // bass-heavy stamp — slate
const H4_CORE = [253, 250, 250];   // mid-spectrum stamp — the white plate
const H4_WARM = [255, 127, 107];   // treble-heavy stamp — coral
...
const H4_RAMP = [0.455, 0.530, 0.560, 0.680];
// the tilt that reads as pure core white — the middle of the white plateau.
const H4_TCORE = (H4_RAMP[1] + H4_RAMP[2]) / 2;
const H4_ORANGE = [255, 162, 74];  // LEFT hand's speed
// V4: the RIGHT hand's DISTANCE. Stamped into each slice at birth, capped so
// the spectral colour underneath stays legible even with the hand parked far.
const H4_PURPLE = [178, 92, 255];
const H4_PMAX = 0.85;              // mix at full distance
const H4_PNEAR = 0.80, H4_PFAR = 0.15;  // cc2 where purple starts / is full
...
function h4Stamp(P) {
  ...
  sl.tilt = H4_TCORE + (s.tilt - H4_TCORE) * s.aud;
  sl.kick = s.kEnv;
  // V4: the right hand's distance, worn by this slice forever.
  sl.purp = s.purp;
...
reg({
  ...
  init(P) {
    ...
    const depth = as > 3.2 ? H4_SLICES : as > 1.7 ? 64 : 40;
    const sl = [];
    for (let i = 0; i < depth; i++) {
      sl.push({ out: new Float32Array(H4_PTS), inn: new Float32Array(H4_PTS), tilt: 0.5, kick: 0, purp: 0 });
    }
    P.state = {
      sl, depth, head: -1, n: 0, capT: 0, life: 0,
      pres: 0, rest: 1, expo: 0.78, sens: 1.0, velL: 0, pL: 0.5, purp: 0,
  ...
  step(P, dt, t, inp) {
    ...
    // V4: DISTANCE IS PURPLE. cc2 arrives NEAR = HIGHER, so the far end of the
    // throw is the low end. Smoothstepped from H4_PNEAR (no purple — V3's
    // plate) down to H4_PFAR (full), so the near quarter is untouched and the
    // last 15% of the sensor's reach changes nothing (field-edge law). Only a
    // LIVE right hand can ask for it; nobody there means white. Entry rides
    // the hand at ~5/s, loss melts at ~1.2/s so a hand drifting out of the
    // field never snaps the plate back.
    let purpT = 0;
    if (chan.R.mode === 'live') {
      const u = clamp((H4_PNEAR - cc2) / (H4_PNEAR - H4_PFAR));
      purpT = u * u * (3 - 2 * u);
    }
    s.purp += (purpT - s.purp) * Math.min(1, dt * (purpT > s.purp ? 5 : 1.2));
    ...
    // paints in full.
    const GATE = 0.55;
    // V4: only the LEFT hand paints by speed now; the right hand's colour is
    // its distance (above), and a violet whip on the same hand was the same
    // colour twice.
 ...
 // feel like. Measured hands-off: p50 0.785, p95 0.845, so this is a no-op
    ...
    s.capT += dt;
    let stamps = 0;
    while (s.capT >= H4_CAP && stamps < 2) { s.capT -= H4_CAP; stamps++; h4Stamp(P); }
    if (s.capT > H4_CAP * 4) s.capT = 0;   // never spiral after a hitch
  },
  ...
  draw(P, g, w, h, t, inp) {
    ...
    for (let i = vis - 1; i >= 0; i--) {
      ...
      let end = H4_CORE, k = 0;
      if (sl.tilt >= H4_RAMP[2]) {
        end = H4_WARM;
        k = clamp((sl.tilt - H4_RAMP[2]) / Math.max(1e-4, H4_RAMP[3] - H4_RAMP[2]));
      } else if (sl.tilt <= H4_RAMP[1]) {
        end = H4_COLD;
        k = clamp((H4_RAMP[1] - sl.tilt) / Math.max(1e-4, H4_RAMP[1] - H4_RAMP[0]));
      }
      const tw = k * k * (3 - 2 * k);
      let cr = H4_CORE[0] + (end[0] - H4_CORE[0]) * tw;
      let cg = H4_CORE[1] + (end[1] - H4_CORE[1]) * tw;
      let cb = H4_CORE[2] + (end[2] - H4_CORE[2]) * tw;
      ...
      // full-canvas tint).
      // V4: PURPLE IS THE SLICE'S OWN — the right hand's distance the moment
      // this slice was born, riding ON the spectral colour so slate and coral
      // strata stay distinct under it. Laid down before the orange whip so a
      // fast left hand still accents the old end of a purple plate.
      const mp = sl.purp * H4_PMAX;
      cr += (H4_PURPLE[0] - cr) * mp; cg += (H4_PURPLE[1] - cg) * mp; cb += (H4_PURPLE[2] - cb) * mp;
      const mo = clamp(wOld * s.velL * 1.15) * 0.60;
      cr += (H4_ORANGE[0] - cr) * mo; cg += (H4_ORANGE[1] - cg) * mo; cb += (H4_ORANGE[2] - cb) * mo;
      const col = (al) => `rgba(${cr | 0},${cg | 0},${cb | 0},${al})`;
      // purple is ~half the core's luma on mesh; pay for it in the edges.
      const pay = 1 + 0.30 * sl.purp;
      ...
      if (i % 12 === 0 && i > 3) {
        ...
        for (let j = 0; j < H4_PTS; j++) {
          const r = sl.out[j] * S;
          const x = cx + r * H4_COS[j], y = cy + r * H4_SIN[j];
      ...
      g.strokeStyle = col(live ? (liveA + 0.30 * s.kEnv) * bright * pay
                               : (0.066 + 0.054 * sl.kick) * fade * bright * dens * pay);
      g.beginPath();
      for (let j = 0; j < H4_PTS; j++) {
        const r = sl.out[j] * S;
        const x = cx + r * H4_COS[j], y = cy + r * H4_SIN[j];
      ...
      if (live || i % 3 === 0) {
        ...
        g.strokeStyle = col(live ? (liveA * 0.78 + 0.22 * s.kEnv) * bright * pay
                                 : (0.074 + 0.058 * sl.kick) * fade * bright * dens * pay);
        g.beginPath();
        for (let j = 0; j < H4_PTS; j++) {
          const r = sl.inn[j] * S;
          const x = cx + r * H4_COS[j], y = cy + r * H4_SIN[j];
    ...
    g.fillText('BASS ' + Math.round(s.bass * 100) + '  MID ' + Math.round(s.mid * 100) +
      ...
      '  PAINT ' + Math.round(s.velL * 100) +
      '  PURPLE ' + Math.round(s.purp * 100) +
