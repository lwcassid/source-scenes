...
reg({
  id: 'SRC-13.18', family: 'SRC-13', ver: 18, title: 'Event Horizon V18', tech: 'VERLET ROPE / ACCRETION',
  music: {
  ...
  step(P, dt, t, inp) {
    ...
    if (s.dropBeat === 0 && settled && s.pres > 0.5 && t > s.cdT && s.jamT <= 0 && inZone) {
      ...
      if (s.chg >= 1) s.dropBeat = T.running ? Math.ceil(T.beats() + 0.05) : -1;
  ...
  draw(P, g, w, h, t, inp) {
    ...
    g.fillText('TENSION ' + (s.tension * 100).toFixed(0) + '%  ·  ' + (H.label || '') +
      ...
      (s.flash > 0.4 ? '  ·  JUMP' : s.dropBeat !== 0 ? '  ·  ON THE NEXT BEAT'
  ...
  audio(A, P) {
    ...
    const bedLo = A.padVoices(v, 3, { type: 'triangle', gain: 0.0001, cutoff: 260, q: 0.7, role: 'strings' });
    const bedHi = A.padVoices(v, 3, { type: 'triangle', gain: 0.0001, cutoff: 520, q: 0.7, role: 'strings' });
    ...
    return {
      tick(inp) {
        ...
        while (nextT < horizon2 && guard++ < 24) {
          ...
          if (s.jam > 0.15) {
            // THE LOOP SEAT (V17): pad 56 struck once at the first window,
            // held all scene; perc CC74 (jam energy) does every fade
            if (typeof MOut !== 'undefined' && !s._loopHeld && MOut.wants()) {
              s._loopHeld = true;
              const blen = T.beat * 4;
              const nb = T.t0 + Math.ceil((now - T.t0) / blen) * blen;
              MOut.evNote('perc', 207.65, 0.22, nb, 3600);
            }
