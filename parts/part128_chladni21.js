/* ---------- SRC-28.21 · CHLADNI COURT V21 (the lure) ---------- */
/* Lance on V20: the idle tease was too predictable, too quiet between
   breaths, and the sand ignored its own sound. V21 = THE LURE:
   - EVERY BREATH IS DIFFERENT: random length (4-9s), depth, attack/release
     skew, spacing (10-32s), and VOICE - some bass-only, most bass + sand,
     ~1 in 7 lands a lure toll: one deep distant bell, long reverb, the
     sound you walk toward from three camps away.
   - THE SAND DOES WHAT THE SOUND DOES: each breath carries a directional
     GUST - a visible roll of grains across the frame, random heading and
     strength, on the same envelope as the wash. Hear sand = see sand.
   - THE IN-BETWEEN UNDULATES: not dead silence - a faint low floor on two
     incommensurate LFOs (never repeats, never high register), sub and low
     pair murmuring at the edge of hearing. */
reg({
  id: 'SRC-28.21', family: 'SRC-28', ver: 21, title: 'Chladni Court', tech: 'STANDING WAVES / SAND ON A PLATE',
  music: {
  ...
  tags: ['CYMATICS', 'THE LURE', 'THE FALLEN LINES', 'THE RAIL SECRET', 'KICK-REACTIVE SAND'],
  desc: 'The court becomes a lure. At rest it murmurs low - a floor that rises and sinks on its own slow tides, never twice the same - and breathes at its own irregular will: some breaths are pure bass, most roll a visible gust of sand across the frame with the wash of sound, and once in a while a single deep bell tolls into long reverb, the sound you walk toward from three camps away. The last-played lines sleep under fresh scatter as softened ghosts. Under hands the bass is structural, the detents are earned, and the rail secret still waits - charge it blue, dance it violet, every kick pulsing through the grains.',
  ...
  sound: 'Idle = the lure: an undulating low floor (bedLo + subs on two incommensurate LFOs) under irregular breaths - length 4-9s, spacing 10-32s, random depth and skew - voiced as bass-only, bass + panning sand wash with a matching visible gust, or (1 in 7) a deep lure toll (low chord-tone bell, rev 0.88) over a soft root. No high register at rest, ever. Play: modal plate tones, beat partners, JUST shimmer, structural bass. Unlock: half-time break with 32nd skitter, kick-reactive violet sand. MIDI unchanged.',
  init(P) {
    ...
    P.state = { grains, n, res: 0, wasLocked: false, motion: 0, lockT: -9, found: {}, appr: 0, resPrev: 0, pres: 0, tNow: 0, teaseT: 6, teaseUntil: -9, tease: 0, surgeUntil: -9, charge: 0, beatUntil: -9, beatOn: false, unlockCol: 0, beatCol: 0, beatPulse: 0, wasAwake: false, brLen: 5, brDepth: 1, brSkew: 0.4, gustA: 1, gustDir: 0, tollQ: false, tolled: false };
  },
  step(P, dt, t, inp) {
    const s = P.state;
    s.tNow = t;
      ...
      if (t < s.teaseUntil) {
        ...
        if (s.tollQ && !s.tolled && u >= s.brSkew) {
          ...
          P.ping(A => {
            ...
            A.bassNote(H.rootFreq(-2), { at: A.t() + 0.05, vol: 0.06, dur: 4 });
          });
        }
      }
    }
