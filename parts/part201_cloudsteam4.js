/* ---------- SRC-47.4 · CLOUD STEAM V4 (the beat is the pacing) ----------
   Nima on V3: "There is some flashing happening or something like that for
   each beat. Beats should set the pacing in which you're flying through
   space, and the rest of the musical inputs can control the shape of the
   clouds and the intensity in which they're growing/changing."

   V3's kick was doing four jobs and three of them were the flash: it
   multiplied the whole frame's brightness (+20% on every hit — that IS the
   flash), it dropped the global coverage threshold so the entire sky
   thickened and thinned in time, and it surged the churn. Only the fourth,
   the forward surge, was the one worth keeping. So V4 gives the beat exactly
   one job and takes the other three away:

   THE BEAT IS THE PACING, AND NOTHING ELSE. Every kick is a forward IMPULSE —
   the flight lurches through the field and settles, so the pulse arrives as
   travel rather than as light. Nothing about the sky's brightness, density or
   colour is allowed to move on a beat. On top of that the engine's detected
   tempo (AUDIOIN.kickBpm) sets the cruising speed, so a faster track is
   literally a faster flight and a half-time section falls back to a drift —
   the beat is the pacing at both scales, per-hit and per-track.

   THE BANDS SHAPE THE CLOUD AND DRIVE THE GROWTH, all on slow envelopes so
   nothing they do can read as a flash:
   · LOUDNESS is GROWTH — it lowers the coverage threshold, so the sky fills
     in and opens up with the arrangement over bars, not beats.
   · BASS is BODY — it drives the density gain, so a heavy track has solid,
     well-defined masses and a thin one has vapour.
   · MID is CHANGE — it drives how fast the field itself evolves, which is the
     "intensity in which they're growing/changing": on a busy section the
     clouds boil and reform, on a sparse one they hold their shape.
   · TREBLE is CARVE — it weights the fine octaves into filigree.
   Everything else — the raymarch, the lighting, the hands, the heart — is V3
   unchanged.
   ------ */
const CS4_FS = [
  'precision highp float;',
  'uniform vec2 uRes;',
  'uniform float uPres, uGlow, uWind, uHeat, uHeart;',
...
reg({
  id: 'SRC-47.4', family: 'SRC-47', ver: 4,
  title: 'Cloud Steam V4', tech: 'VOLUMETRIC RAYMARCH / THE BEAT IS THE PACING',
  audioIn: true,
  fx: { bloom: 0.28 },
  tags: ['AUDIO IN', 'VOLUMETRIC RAYMARCH', 'THE BEAT IS THE PACING', 'NO SILHOUETTE', 'TEMPO SETS THE CRUISE', 'HEART ON THE DROP'],
  desc: 'V3’s volumetric raymarch with the beat given exactly one job. A ray per pixel is stepped through a 3D noise field, gathering colour and opacity as it goes, lit by sampling the density a short step toward the sun — cloud tops catch cream light, crevices fall to deep rose, and edges are soft because a ray that clips a fringe simply gathers less. There is no outline anywhere in it. THE BEAT IS THE PACING: every kick is a forward impulse, so the flight lurches through the field and settles, and the detected tempo sets the cruising speed — a faster track is a faster flight, a half-time section is a drift. Nothing about the sky’s brightness or density is allowed to move on a beat, so the pulse arrives as travel rather than as light. The bands do the rest, all on slow envelopes: loudness is GROWTH, opening and filling the sky over bars; bass is BODY, making the masses solid or vaporous; the mids are CHANGE, setting how hard the field boils and reforms; treble is CARVE, weighting the fine octaves into filigree. On a big rise or fall the sky thins to mist and a heart-shaped mass of cloud hangs in it, a fixed distance ahead, until it lets go.',
  interact: 'This scene listens (SHOW CHECK → AUDIO IN, or MAP → Audio in) — bring it music and it flies itself, one surge forward per beat. The hands fly it, and answer instantly. LEFT HAND BANKS: bring it in toward the source and the flight yaws left, the whole sky sweeping past with real parallax, and that side warms to apricot. RIGHT HAND IS THE THROTTLE: bring it in and you fly harder and the cloud churns as it goes by, and that side cools to lilac. Reach both away and you glide, slow and level — which is also where a hand the sensor loses settles, so nothing ever slams. THE HEART arrives on its own when the music makes a big move; the house code summons one — park the left hand at the source and wiggle the right until the charge fills.',
  ...
  step(P, dt, t, inp) {
    ...
    s.speed = Math.min(0.95, (0.075 + 0.105 * s.energy) * s.tempo * (1 + 0.85 * s.heat) * (1 + 1.85 * s.kick))
      * (1 - 0.72 * s.heart);
    s.fly = (s.fly + s.speed * dt) % 4096;
    s.churn = (s.churn + (0.06 + 1.55 * s.mid + 0.75 * s.energy + 0.55 * s.heat) * dt) % 4096;
  ...
  draw(P, g, w, h, t, inp) {
    ...
    if (s.noGL || !P._three) {
      ...
      g.fillText('CLOUD STEAM V4 · open on the hosted site (WebGL)', 10, h - 10);
      return;
    }
