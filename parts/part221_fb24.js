/* ---------- SRC-15.24 · FERRO BLOOM V24 (the garden hears weather)
   Lance's DJ-set round PROPOSAL (Kasia owns this scene — this is a
   suggestion for her garden, not a rework; adopt or discard). The move:
   climate, not choreography. When a booth feed is live (audioIn:true,
   arbitrated on inp.audio.live), the music reaches the garden the way
   WEATHER does — slow fronts, not cues. A HIGH-WATER MARK of the low
   end (rises ~0.5/s, melts ~0.08/s) floors both reaches, so a bass-heavy
   stretch of the set GROWS blooms one at a time, spaced — deliberate,
   because every birth queues 3 bells + a pad note, and the newBorn drain
   is now capped at 2/tick as storm insurance. The real KICK (part2e's
   time-domain scanner, unsmoothed, back-dated) fires the existing
   s.pulse envelope — the frame already knows how to breathe with it.
   LEVEL floors the frame's zoom (spread), MID surges the band-flow
   clock (the V14 gradient that no hand may touch now runs with the
   music's body), and a structural DROP rerolls the hero and flashes
   the pulse — a new garden statement. Audio treats sleep as presence
   (Math.max(pres, aPres) in the idle test only). THE SHIELD: the
   groove/loop machinery stays strictly HAND-EARNED — flower gates on
   a pre-floor spreadH, the scatter-arp register on pre-floor eLH/eRH —
   so a loud song can never strike the dub loop or flip the arp octave;
   the hands' reach-release logic is untouched. Audio drives VISUALS
   (and the births they queue), never the groove. With no signal this
   is exactly V23. (V23 notes below.) -------------------------------- */
...
reg({
  id: 'SRC-15.24', family: 'SRC-15', ver: 24,
  title: 'Ferro Bloom V24', tech: 'SHADER FIELD / MOTION ILLUSION / LISTENS',
  audioIn: true,
  music: {
  ...
  init(P) {
    P.state = {
      ...
      newBorn: [],
      // V24: the listening layer — audio liveness, slow band eases, the
      // bass high-water mark, hand-only shadows of the reaches (the shield),
      // and event counters seeded on first sight of the stream
      aPres: 0, aBass: 0, aMid: 0, aLevel: 0, aHwm: 0,
      eLH: 0, eRH: 0, spreadH: 0,
  ...
  step(P, dt, t, inp) {
    ...
    // ---- V24: THE SHIELD — capture the HAND-ONLY reaches before any audio
    // floor touches them. The flower-power gate, the loop seat and the
    // scatter-arp register read these, so the DJ can never strike the loop.
    s.eLH = s.eL; s.eRH = s.eR;
    s.spreadH = (s.eLH + s.eRH) / 2;
    // ---- V24: THE LISTENING LAYER — the set reaches the garden as WEATHER:
    // slow fronts (band eases at 1.5/s), one fast thing (the kick), and a
    // bass high-water mark that grows blooms spaced instead of in a burst.
    const au = inp.audio;
    const aLive = au && au.live ? 1 : 0;
    s.aPres += (aLive - s.aPres) * Math.min(1, dt * 2.2);
    if (au) {
      s.aBass += (clamp(au.bass) - s.aBass) * Math.min(1, dt * 1.5);
      s.aMid += (clamp(au.mid) - s.aMid) * Math.min(1, dt * 1.5);
      s.aLevel += (clamp(au.level) - s.aLevel) * Math.min(1, dt * 1.5);
      // seed event counters on first sight — a scene opened mid-set never
      // fires a phantom kick pulse or a drop three scenes old
      if (!s._aSeeded) { s._aSeeded = true; s._kN = au.kick ? au.kick.n : 0; s._dropN = au.drop ? au.drop.n : 0; }
      // THE KICK: the real kick fires the pulse envelope the frame already
      // owns — unsmoothed, back-dated by the hit's true age.
      if (au.kick && au.kick.n !== s._kN) {
        ...
        s.pulse = Math.max(s.pulse, hit * Math.exp(-6.9 * (age + 0.030)));
      }
      // THE DROP: a structural drop rerolls the hero (the same dice the
      // rising-pres edge throws) and flashes the pulse — a new garden
    ...
    } else { s.aBass *= Math.pow(0.5, dt); s.aMid *= Math.pow(0.5, dt); s.aLevel *= Math.pow(0.5, dt); }
    // THE HIGH-WATER MARK: the low end raises a slow floor under both
    // reaches (rise ~0.5/s, melt ~0.08/s) — every bloom birth queues 3
    // bells + a pad note, so births must arrive SPACED, like a front.
    const hwmT = s.aBass * s.aPres;
    if (hwmT > s.aHwm) s.aHwm += (hwmT - s.aHwm) * Math.min(1, dt * 0.5);
    else s.aHwm = Math.max(hwmT, s.aHwm - dt * 0.08);
    s.eL = Math.max(s.eL, s.aHwm * 0.9);
    s.eR = Math.max(s.eR, s.aHwm);
    s.spread = (s.eL + s.eR) / 2;
    // LEVEL breathes the frame: the track's loudness widens the framing
    s.spread = Math.max(s.spread, 0.85 * s.aLevel * s.aPres);
    // stillness clock — the call-and-response trigger
    if (s.vel < 0.06 && s.pres > 0.3) s.still += dt; else s.still = 0;
    // V24: audio is presence for SLEEP only — a live set keeps the garden
    // awake; the !live reach-release above stays hand-only, untouched.
    if (Math.max(s.pres, s.aPres) < 0.12) { s.idleT = (s.idleT || 0) + dt; } else s.idleT = 0;
    ...
    // V24: the MID band is the one visitor this clock accepts — the flow
    // surges with the music's body. No hand term, same as ever.
    s.uph += dt * 0.44 * (0.35 + 0.65 * s.pres) * (1 + 0.8 * s.aMid * s.aPres);
    ...
    // V24: the booth counts as presence for the PICTURE — the garden calms
    // its rest-wobble and brightens for a playing DJ like it does for hands
    s.breath = 1 - Math.max(s.pres, s.aPres || 0) * 0.7;
  },
  draw(P, g, w, h, t, inp) {
    const s = P.state;
    if (s.noGL || !P._three) {
    ...
    u.uPres.value = Math.max(s.pres, s.aPres || 0); u.uPulse.value = s.pulse;
    u.uBreath.value = s.breath;
    u.uAmb.value = Math.max(0, 1 - Math.max(s.pres, s.aPres || 0) * 1.4) * 0.9;
    ...
    g.fillText('BLOOMS ' + s.count + '/17 (L ' + (s.cL || 0) + ' · R ' + (s.cR || 0) + ')' + ((s.eL > 0.66 || s.eR > 0.66) ? ' + SCATTER' : '') + '  ·  ' + (H.label || '') + '  ·  L ' + Math.round(s.eL * 100) + ' / R ' + Math.round(s.eR * 100) + (s.aPres > 0.3 ? '  ·  AUD b' + Math.round(s.aBass * 100) + ' m' + Math.round(s.aMid * 100) + ' hwm' + Math.round(s.aHwm * 100) : '') + (Math.max(s.pres, s.aPres) < 0.3 ? '  ·  SLEEPING' : ''), 10, h - 10);
  },
  audio(A, P) {
    const v = A.voice();
    ...
    return {
      tick(inp) {
        ...
        // V24: gated on the HAND-ONLY spread (the shield) — the groove, the
        // one-way loop latch and the clave scheduler stay hand-earned; a
        // loud song must never permanently strike the dub loop.
        const flower = clamp((s.spreadH - 0.55) / 0.3);
        ...
        // V24: drain at most 2 births per tick (the rest stay queued) —
        // storm insurance if the high-water mark ever wakes a crowd at once
        let casc = 0;
        while (s.newBorn.length && casc < 2) {
        ...
        while (nextT < horizon) {
          ...
          if (st % 2 === 0 && flower > 0.2 && s.pres > 0.15) {
            ...
            // V24: the octave jump reads the HAND-ONLY reaches — an audio
            // floor can never flip the arp register (the shield)
            const scatter = (s.eLH > 0.66 || s.eRH > 0.66) && arpIdx % 3 === 0;
