/* ---------- SRC-15.16 · FERRO BLOOM V16 (three color scenes: warm · green · blue) ---------- */
reg({
  id: 'SRC-15.16', family: 'SRC-15', ver: 16,
  title: 'Ferro Bloom V16', tech: 'SHADER FIELD / MOTION ILLUSION',
  music: {
  ...
  desc: 'The garden now moves between THREE COLOR SCENES rather than sliding along one warm-to-cool line. A WARM scene of hot pink, red and orange, with light blue held back as the counterpoint in a minority of its flowers. A GREEN scene of chartreuse through lime into teal. A BLUE scene of electric blue, lavender and light blue. Each scene owns the whole field while it is up — no stray colors from the other two leaking in — and the garden crossfades warm ↔ green ↔ blue and back, green sitting in the middle because it is the natural bridge, so a transition never passes through a hue that belongs to neither side. Within a scene the variety comes from the flowers spreading across that scheme\'s full range, so a warm passage carries reds next to oranges next to hot pinks. Everything else is V14: right grows the bigger half, the cluster rerolls its shape and center size each time, and the color runs on its own clock, untouched by the hands. Music unchanged: a three-layer soundscape — C-minor pedal drone whose color shifts, a quantized groove that only exists once the garden is thrown wide open, and a reactive layer where every willed bloom, flick, and stillness sounds back at you.',
  interact: 'BOTH HANDS GROW FLOWERS, NEITHER DRIVES THE COLOR: L reach summons up to 6 flowers on the left, R reach up to 10 on the right — so the right side fills faster and denser — and each new bloom lands in a fresh random spot at a fresh random size. Push a hand past ~65% and that side OVERFLOWS: small scatter blooms flood the full frame edge to edge. The gradient flowing through the petals runs on its own clock throughout — no gesture speeds it up or slows it down. Keep it small and it stays chamber-music quiet; throw both arms wide and the groove fades in. Quick flicks fire log-drum answers only in full bloom. Stillness after a gesture invites the garden to answer.',
  ...
  init(P) {
    ...
    const mat = new THREE.ShaderMaterial({
      ...
      fragmentShader: [
        ...
        '}',
        // V16: THREE COLOR SCENES instead of one warm→cool ramp. Each scheme
        // is a self-contained ramp over s (0..1) — a flower's own stable slot
        // in the current scheme — so the whole garden always reads as ONE
        // palette, with variety spread across the flowers rather than colors
        // from other schemes leaking in.
        'vec3 rampMix(vec3 a, vec3 b, float x){ return mix(a, b, smoothstep(0.0, 1.0, clamp(x, 0.0, 1.0))); }',
        // WARM: hot pink, red and orange — with light blue as the counterpoint
        // holding the top of the ramp (a minority of the flowers).
        'vec3 schemeWarm(float s){',
        '  if (s < 0.26) return rampMix(vec3(1.0, 0.16, 0.13),  vec3(0.98, 0.62, 0.34), s / 0.26);',
        '  if (s < 0.54) return rampMix(vec3(0.98, 0.62, 0.34), vec3(1.0, 0.341, 0.831), (s - 0.26) / 0.28);',
        '  if (s < 0.79) return rampMix(vec3(1.0, 0.341, 0.831), vec3(0.95, 0.55, 0.62), (s - 0.54) / 0.25);',
        '  return rampMix(vec3(0.95, 0.55, 0.62), vec3(0.45, 0.82, 1.0), (s - 0.79) / 0.21);',
        '}',
        // GREEN: chartreuse through lime into teal, light blue at the top.
        'vec3 schemeGreen(float s){',
        '  if (s < 0.34) return rampMix(vec3(0.878, 1.0, 0.161), vec3(0.55, 0.92, 0.45), s / 0.34);',
        '  if (s < 0.68) return rampMix(vec3(0.55, 0.92, 0.45),  vec3(0.16, 0.55, 0.58), (s - 0.34) / 0.34);',
        '  return rampMix(vec3(0.16, 0.55, 0.58), vec3(0.45, 0.82, 1.0), (s - 0.68) / 0.32);',
        '}',
        // BLUE: electric blue through lavender and light blue into teal.
        'vec3 schemeBlue(float s){',
        '  if (s < 0.34) return rampMix(vec3(0.0, 0.263, 1.0),  vec3(0.58, 0.56, 0.90), s / 0.34);',
        '  if (s < 0.68) return rampMix(vec3(0.58, 0.56, 0.90), vec3(0.45, 0.82, 1.0), (s - 0.34) / 0.34);',
        '  return rampMix(vec3(0.45, 0.82, 1.0), vec3(0.16, 0.55, 0.58), (s - 0.68) / 0.32);',
        '}',
        // The garden crossfades WARM <-> GREEN <-> BLUE and back. Green sits
        // in the middle because it is the natural bridge between the two, so
        // no transition ever passes through an unrelated hue.
        'vec3 sceneColor(float s, float scenePos){',
        '  if (scenePos < 0.5) return mix(schemeWarm(s), schemeGreen(s), smoothstep(0.0, 1.0, scenePos * 2.0));',
        '  return mix(schemeGreen(s), schemeBlue(s), smoothstep(0.0, 1.0, (scenePos - 0.5) * 2.0));',
        '}',
        ...
        '    float w = exp(-f / k);',
        '    acc += w;',
        // V16: a flower no longer carries a hue — it carries a stable SLOT in
        // whatever scheme is currently showing. The scheme decides the actual
        // colors, so the garden reads as one palette at all times and the
        // flowers spread across its full range instead of clustering.
        '    hueAcc += fract(seed * 13.7) * w;',
        ...
        // the ring drift now travels along the CURRENT scheme's ramp, so a
        // flower's gradient stays inside the scene it belongs to
        '  vec3 tint = sceneColor(t, mood);',
        ...
        '    float ambT = fract(fj * 0.61 + 0.2);',
        '    col += mix(deep, sceneColor(ambT, mood), aband) * ain * 0.34 * uAmb;',
  ...
  draw(P, g, w, h, t, inp) {
    ...
    if (s.noGL || !P._three) {
      ...
      g.textAlign = 'center';
      g.fillText('FERRO BLOOM V16 · WARM · GREEN · BLUE', w / 2, h / 2 - 12);
