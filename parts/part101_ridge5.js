/* ---------- SRC-42.5 · RIDGE LOOM V5 (V4's heat ramp + V3's proximity fade) ---------- */
reg({
  id: 'SRC-42.5', family: 'SRC-42', ver: 5, title: 'Ridge Loom V5', tech: 'CONTOUR HEAT / FAR FIELD FADES, MIDDLES BURN',
  music: {
  ...
  fx: { bloom: 0.45 },
  tags: ['LEFT HAND = BASS', 'RIGHT HAND = LEADS', 'HEIGHT IS THE COLOUR', 'THE MIDDLES BURN'],
  desc: 'V4\'s contour map with V3\'s depth put back. Colour still says one thing — how high the ridge is: the low ground and outermost rings are deep violet (#4900ff), and the ladder climbs through purple and magenta to hot orange (#f36c3b) at the summits. What returns from V3 is the FADE. Ridge far from every core is drawn faint and sunken; the closer a ridge sits to the middle of a whorl the harder it burns, so the tight inner rings pack in bright and fill each other while the open country stays a dim violet weave. Every ridge is still one hairline weight — the intensity does all the work. At rest two shallow knots glow in a faded sea; raise a hand and a second core surfaces on that side, then a third, each one heating violet → orange and pulling its own patch of the field up out of the dark.',
  ...
  init(P) {
    ...
    P.state = {
      ...
      oTh: [0.12, 0.4, 0.9, 1.7, 2.8], tierA: [0.52, 0.63, 0.74, 0.85, 0.93, 1],
