...
reg({
  id: 'SRC-28.27', family: 'SRC-28', ver: 27, title: 'Chladni Court V27', tech: 'STANDING WAVES / SAND ON A PLATE',
  music: {
      ...
      tick() {
        ...
        if (s.beatOn && T.running) {
          ...
          if (typeof MOut !== 'undefined' && typeof H !== 'undefined' && s._arpChord !== H.step) {
            ...
            // THE WALK (V27, Lance's F-G-D shape in lydian): the held tone
            // steps D -> F# -> G# -> D across the chord cycle, one per
            // chord, and lands home -- movement without a note-spray
            const walk = [0, 2, 3, 0];
            MOut.evNote('arp', H.scaleTone(walk[((H.step % 4) + 4) % 4], 0), 0.1, 0, hold);
          }
