/* ---------- SRC-18.14 · NIGHT CIRCUIT V14 (the tunnel, produced end-to-end) ---------- */
reg({
  id: 'SRC-18.14', family: 'SRC-18', ver: 14,
  title: 'Night Circuit V14', tech: 'WEBGL / TUNNEL PRODUCTION PASS',
  ...
  tags: ['TUNNEL PRODUCED END-TO-END', 'PHRASE-TIMED GATES', 'FPV ENTRANCE', 'LIGHT AT THE END'],
  desc: 'The tunnel gets the full production pass. Entering it drops you into first person for three breaths — the bore swallowing the camera — before the chase view returns. The music is now a composed arc across the whole act: a sparse pulse that earns its bass ostinato, the filter opening as you ride deeper, a lead motif arriving mid-build, snare rolls at every phrase turn, a riser at the top — and the gates KNOW the music: rings chase the beat down the bore in sequence, and the next gate ahead pre-glows through the last bar of every phrase so you feel the downbeat coming before it lands. Deep in, coral has started growing through the wall seams; nobody maintains this section anymore. And at the far end, a warm light grows with the build until the tunnel releases you into the sunset it was promising all along.',
  interact: 'Hands unchanged: L/R beams, sides, push-together throttle. In the tunnel the act itself is the arrangement — the further you ride, the more of the composition you unlock; back off and it thins. Wall panels still answer your lean per side; the exit only opens if you keep riding.',
  ...
  step(P, dt, t, inp) {
    ...
    if (s.pending != null) {
      ...
      if (s.trans >= 1) {
        if (s.pending === 2) s.fpv = 3.6; // V14: the bore swallows the camera — three breaths of first person
        s.act = s.pending; s.pending = null;
      }
    } else {
    ...
    if (act === 0) {
      ...
      if (T3.tunHero) {
        ...
        // LED conduits: the cyan mainline's floor RISES with the build; the
        // magenta service lines answer arps, lean, and the coming downbeat
        if (T3.tunLeds) for (const ld of T3.tunLeds) {
          ld.m.opacity = ld.mag
            ? 0.07 + s.arpFlash * (0.1 + bld * 0.45) + Math.max(sideL, sideR) * 0.3 + antic * 0.35
            : 0.08 + bld * 0.24 + s.kickPulse * (0.2 + bld * 0.5) + thr * 0.1;
        }
      }
      // the gates play the sequence: each ring belongs to a beat of the bar and
      // fires on its beat; the NEAREST gate ahead pre-glows through the last
      // bar of every phrase so the turn is felt before it lands
      let nearest = null, nearZ = -1e9;
      T3.rings.forEach((r2, i2) => {
        recyc(r2.ring, 364);
        r2.ring.scale.setScalar(1);
        const chase = (i2 % 4) === beatIdx ? s.kickPulse : 0;
        r2.m.opacity = 0.12 + bld * 0.18 + chase * (0.2 + bld * 0.6);
      ...
      for (const p2 of T3.pans) {
        ...
        p2.userData.m.opacity = 0.25 + bld * 0.1 + near4 * (0.5 + s.kickPulse * 0.35);
    ...
    const expAct = act === 2 ? 0.76 + (s.tunBuild || 0) * 0.32 : [0.8, 0.9, 1.0, 1.12][act]; // the tunnel EARNS its brightness
    T3.renderer.toneMappingExposure = (0.68 + E * 0.5 + s.kickPulse * 0.05) * expAct * (1 - s.trans * 0.45);
    ...
    // V14 · FPV entrance envelope: quick dive in, hold, ease back to the chase
    if (s.fpv > 0) s.fpv = Math.max(0, s.fpv - dt);
    const fk = s.fpv > 0 ? (s.fpv > 3.2 ? (3.6 - s.fpv) * 2.5 : s.fpv < 0.7 ? s.fpv / 0.7 : 1) : 0;
    T3.cam.rotation.z = s.camR;
    T3.cam.rotation.y = (0.05 * camK - s.camX * 0.06) * (1 - fk);
    let rotX = (-0.3 + 0.235 * camK) + (act === 2 ? Math.cos(s.tunPh) * 0.035 : 0);
    let camXv = 1.05 * (1 - camK) + s.camX;
    // near-field motion grows with intensity: bob + a shiver of kick
    let camYv = (6.0 - 3.55 * camK)
      + Math.sin(s.bob) * (0.015 + camK * 0.03) * (s.speed / 40)
      + s.kickPulse * camK * 0.045
      + T3.world.position.y * 0.4;
    if (fk > 0) { // over the tank, eyes on the bore
      rotX = rotX * (1 - fk) + -0.02 * fk;
      camXv *= 1 - fk;
      camYv = camYv * (1 - fk) + (1.66 + T3.world.position.y) * fk;
    }
    T3.cam.rotation.x = rotX;
    T3.cam.position.x = camXv;
    T3.cam.position.y = camYv;
    T3.cam.fov = 58 + camK * 5 + (tunnel ? thr * 8 + (s.tunBuild || 0) * 6 : 0) + fk * 9;
  ...
  draw(P, g, w, h, t, inp) {
    ...
    if (s.noGL || !P._three) {
      ...
      g.textAlign = 'center';
      g.fillText('NIGHT CIRCUIT V14 · THE JOURNEY', w / 2, h / 2 - 12);
