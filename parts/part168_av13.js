...
reg({
  id: 'SRC-09.13', family: 'SRC-09', ver: 13, title: 'Attractor Vespers V13', tech: 'CLIFFORD MAP / FLAME DENSITY',
  music: {
  ...
  sound: 'V10\'s engine with an ARTICULATED beam. THE MOUTH: gesture velocity kicks the beam\'s lowpass open (up to ~+750Hz) with resonance rising 2→7 and a fast wah-relax; each rung step fires a ~250ms attack transient (filter + Q + a level accent) so the walking melody has consonants; caustic sharpness sets the resting reediness; and the saw\'s share of the blend grows with brightness and the kick, hardening the voice under intensity. THE MOUTH streams on ch12 (sax) CC74 now - V13 casts the beam onto the SAX seat (any legato patch; the felt piano is silent here), and the CHOIR (ch11) holds a soft root+color double breathing with the light. Everything else as V10: rung-exact tuning (never parked between pitches), triangle-led G3–G5 register, maw on bass ch3, churn on arp CC74, dreaming idle with spurs, presence-gated heat, wake on sfx ch11, pad CC74 = the LIGHT. V12 rack fixes: the analog pad holds exactly the five-voice chord (teeth are browser growl only), and the structural sub holds the low root on SYNTH BASS all scene, breathing with the light, under the maw\'s surges.',
  init(P) {
    ...
    return {
      tick(inp, dt) {
        ...
        if (typeof MOut !== 'undefined') {
          if (bl > 0.0035) MOut.holdOn(beam.h, 'sax', bfreq, Math.round(34 + br * 66));
          else MOut.holdOff(beam.h);
          if (bl2 > 0.0035) MOut.holdOn(beam2.h, 'sax', bfreq2, Math.round(28 + br * 50));
      ...
      stop() {
        if (typeof MOut !== 'undefined') { MOut.holdOff(beam.h); MOut.holdOff(beam2.h); MOut.holdOff(mawH); MOut.holdOff(subH); MOut.holdOff(choirH1); MOut.holdOff(choirH2); }
        bv.kill(); v.kill();
      }
    };
  }
