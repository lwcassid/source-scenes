...
const LUMEN22_STOPCACHE = new Map();
reg({
  id: 'SRC-38.22', family: 'SRC-38', ver: 22, title: 'Lumen Film V22', tech: 'GEM FILM / APERTURE ORGAN',
  music: {
  ...
  draw(P, g, w, h, t, inp) {
    ...
    const addSvgStops = (grad, a) => {
      let set = LUMEN22_STOPCACHE.get(a);
      if (!set) {
        set = SVGSTOPS.map(([t, rgb]) => [t, `rgba(${rgb},${a})`]);
        LUMEN22_STOPCACHE.set(a, set);
      }
  ...
  audio(A, P) {
    ...
    const choirH = {};   // the voice through the holes (ch11), full aperture only
    ...
    return {
      tick(inp, dt) {
        ...
        if (typeof MOut !== 'undefined') {
          ...
          if ((s.openN || 0) >= 6 && s.pres > 0.2) {
            MOut.holdOn(choirH, 'choir', H.rootFreq(0), Math.round(22 + 20 * Math.min(1, s.openN / 15) + 10 * s.lamp));
          } else MOut.holdOff(choirH);
        }
      ...
      stop() {
        ...
        if (typeof MOut !== 'undefined') MOut.holdOff(choirH);
        v.kill();
      }
    };
  }
