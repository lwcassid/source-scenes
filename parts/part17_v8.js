/* ---------- SRC-18.8 · NIGHT CIRCUIT V8 (bloom + the winding road) ---------- */
reg({
  id: 'SRC-18.8', family: 'SRC-18', ver: 8,
  title: 'Night Circuit V8', tech: 'WEBGL / BLOOM / WINDING ROAD',
  ...
  init(P) {
    P.state = {
      ...
      act: 0, actT: 0, trans: 0, pending: null, curveA: 0, curvePh: 0,
    ...
    for (const pair of [[-4.5, emL], [4.5, emR]]) {
      ...
      edge.position.set(pair[0], 0.015, -150); roadStr.add(edge);
    }
    ...
    T3.mkGlow = mkGlow;
    {
      ...
      const mkCaveTube = (zoff) => {
        ...
        const wire = new THREE.Mesh(tube, new THREE.MeshBasicMaterial({ color: 0x4a2fae, wireframe: true, transparent: true, opacity: 0.04, side: THREE.BackSide, blending: THREE.AdditiveBlending, depthWrite: false }));
      ...
      T3.monos = [];
      for (let i = 0; i < 8; i++) {
        const col = [0xff7a3c, 0x8a5cff, 0x2fe89a][i % 3];
        const m = new THREE.MeshBasicMaterial({ color: col, transparent: true, opacity: 0.8 });
        const g2 = new THREE.Group();
        ...
        const halo = mkGlow(col, 4 + rnd() * 2); halo.position.y = 2.6; halo.material.opacity = 0.28; g2.add(halo);
        const side2 = i % 2 ? 1 : -1;
      ...
      for (let i = 0; i < 26; i++) {
        ...
        for (let c2 = 0; c2 < n2; c2++) {
          ...
          const stem = new THREE.Mesh(stemG, L2(0x453a63));
          const cap = new THREE.Mesh(capG, new THREE.MeshLambertMaterial({ color: 0x241738, emissive: glow, emissiveIntensity: 0.32, flatShading: true }));
  ...
  step(P, dt, t, inp) {
    ...
    if (act === 0) {
      ...
      for (const tb of T3.caveTubes) {
        ...
        tb.userData.wireMat.opacity = 0.045 + s.arpFlash * 0.1 + thr * 0.05;
      }
    ...
    T3.hemi.intensity = [0.04, 0.3, 0.22, 0.34][act]; // cave nearly unlit — the glow carries it
    ...
    T3.renderer.toneMappingExposure = (0.68 + E * 0.5 + s.kickPulse * 0.05) * [0.85, 1, 1.0, 1.15][act] * (1 - s.trans * 0.45);
    ...
    T3.rim.intensity = act === 0 ? 0.45 : 1.5; // the blue rim was washing the cave walls lavender
  ...
  draw(P, g, w, h, t, inp) {
    ...
    if (s.noGL || !P._three) {
      ...
      g.textAlign = 'center';
      g.fillText('NIGHT CIRCUIT V8 · THE JOURNEY', w / 2, h / 2 - 12);
