...
reg({
  id: 'SRC-13.21', family: 'SRC-13', ver: 21, title: 'Event Horizon V21', tech: 'VERLET ROPE / ACCRETION',
  music: {
  ...
  audio(A, P) {
      ...
      if (s2.tension > 0.3 && s2.pres > 0.4) {
        ...
        for (let i = 0; i < 3; i++) {
          A.tone(H.chordTone(i * 2, -1), {
            ...
            type: 'triangle', pan: -0.25 + i * 0.25, rev: 0.6, role: 'strings'
          });
        }
      }
    });
    ...
    const saxH = {};
    // V20: each hand HOLDS a string voice -- the continuous coupling the
    // runs alone lost. Joins past ~25% height, releases when the arm
    // drops, re-voiced per chord, velocity from the height at the strike.
    const strHL = {}, strHR = {};
    ...
    return {
      tick(inp) {
        ...
        for (let j = 0; j < 6; j++) {
          ...
          if (want > 0.5 && !was[j]) {
            ...
            A.tone(f, { at: T.next(0.5), vol: 0.045 * gate * duck, dur: 1.8, attack: 0.1, type: 'triangle', pan: j < 3 ? -0.4 : 0.4, rev: 0.55, role: 'strings' });
        ...
        if (typeof MOut !== 'undefined') {
          if (s.pres > 0.3 && s.uL > 0.25) MOut.holdOn(strHL, 'strings', H.chordTone(1, 0), Math.round(38 + 62 * s.uL));
          else if (s.pres <= 0.3 || s.uL < 0.15) MOut.holdOff(strHL);
          if (s.pres > 0.3 && s.uR > 0.25) MOut.holdOn(strHR, 'strings', H.chordTone(3, 0), Math.round(38 + 62 * s.uR));
          else if (s.pres <= 0.3 || s.uR < 0.15) MOut.holdOff(strHR);
        }
        ...
        const gateSy = s.pres > 0.35 && hi > 0.58 && T.running;
        ...
        if (typeof MOut !== 'undefined') {
          if (gateSy) {
            ...
            while (synthNext < now + 0.15 && g3++ < 4) {
              MOut.evNote('synth', H.chordTone(0, -1), 0.07 + 0.14 * hi, synthNext, barLen * 1.02);
        ...
        while ((ev = s.strumQ.shift()) && k < 2) {
          ...
          for (let i = 0; i < 5; i++) {
            ...
            A.pluck2(H.chordTone(idx, -1), {
              ...
              rev: 0.55, del: 0.2, role: 'strings'   // the improvisational voice (V19)
            });
          }
        }

        if (typeof MOut !== 'undefined' && MOut.expr) {
          MOut.expr('strings', depth);   // THE MARQUEE: gate depth on ch6 -> Augmented's gate dial
          MOut.expr('sax', clamp(0.15 + 0.85 * armAvg) * s.pres);   // the swell's breath
          MOut.expr('synth', gateSy ? clamp(0.2 + 0.8 * hi) : 0.1); // ch16 filter lane
      ...
      stop() {
        if (typeof MOut !== 'undefined') {
          MOut.holdOff(saxH); MOut.holdOff(strHL); MOut.holdOff(strHR);
