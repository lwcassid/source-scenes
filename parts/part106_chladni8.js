/* ---------- SRC-28.8 · CHLADNI COURT V8 ---------- */
/* V7's plate, shown as just TWO squares instead of three. Each square is the
   full frame height and they meet at the centre with a mirror seam, filling the
   view top-to-bottom and bleeding a little off the outer left/right edges. */
reg({
  id: 'SRC-28.8', family: 'SRC-28', ver: 8, title: 'Chladni Court', tech: 'STANDING WAVES / SAND ON A PLATE',
  ...
  draw(P, g, w, h, t, inp) {
    ...
    // sand — bold gold bands; a touch bigger than V6, on-lock swell (~1.5x)
    const sz = Math.max(2.8, m * 0.0068) * (1 + warm * 0.5);
    ...
    const startX = (w - 2 * m) / 2;       // centre the pair → equal outer bleed
    const yTile = (h - m) / 2;            // m === h, so this is 0
    for (let c = 0; c < 2; c++) {
      const xflip = (c === 0);            // left square mirrored, right = base
      g.save();
      g.translate(startX + c * m + (xflip ? m : 0), yTile);
      g.scale(xflip ? -1 : 1, 1);
