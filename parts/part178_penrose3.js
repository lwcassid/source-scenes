...
reg({
  id: 'SRC-46.3', family: 'SRC-46', ver: 3,
  title: 'Penrose Bloom V3', tech: 'PENROSE DEFLATION / AUDIO-GROWN',
  audioIn: true,
  fx: { bloom: 0.4 },
  tags: ['AUDIO IN', 'THE CRYSTAL ONLY MELTS BACK', 'SPECTRUM OWNS THE PALETTE', 'THE BEAT IS LIGHT', 'FIVE-FOLD, NEVER REPEATS'],
  desc: 'V2 with the sensitivity swapped between the two things it does. THE SIZE IS CALM NOW: the crystal only ever melts back. Growth is instant — a loud bar or a kick pushes the boundary straight out — but the retreat is a slow thaw, so the edge never flickers and a dip in the track no longer costs a whole ring of tiles. The kick has stopped throwing the boundary around and become LIGHT instead: a ring of fire that brightens on the beat and travels out through a crystal that is holding still. THE COLOUR IS LOUD NOW: the spectrum owns most of Ferro Bloom\'s palette rather than a corner of it, so bass-heavy music really is ember and blush and orchid, and a treble section really is chartreuse, teal, lavender and electric blue — opposite ends of the ramp, not two stops apart. The diamonds differ more from each other too, and a change in the harmonic balance re-deals which of the five tile orientations takes which colour, all at once. Makes no sound of its own.',
  ...
  draw(P, g, w, h, t, inp) {
    ...
    for (let i = 0; i < N; i++) {
      ...
      const c = pbCol(tiltT + tl.r * 0.26 + (dealt - 0.5) * 0.48);
      const f = s.fl[i];
      g.beginPath();
