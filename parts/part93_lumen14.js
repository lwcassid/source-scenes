/* ---------- SRC-38.14 · LUMEN FIELD (all four rays refract) ---------- */
reg({
  id: 'SRC-38.14', family: 'SRC-38', ver: 14, title: 'Lumen Film', tech: 'GEM FILM / APERTURE ORGAN',
  music: {
  ...
  fx: { bloom: 0.46 },
  tags: ['ONE BEAM PER STONE', 'ALL FOUR RAYS REFRACT', 'NO HARD ENDS', 'L TURNS THE GRADIENTS TOO'],
  desc: 'V14: the rays come back in by half — V13 was too wide once it was actually on screen. The short cross rays are no longer the poor relation: they were being drawn as one flat stroke, which is exactly why they stopped dead at a hard edge, and they had none of the colour split. Now all four rays are built the same way — a wide soft falloff underneath, the three colour channels drawn separately and nudged out of register so red rides one edge and blue the other, and a thin hot thread down the middle — with the colour running out to nothing before the tip so every ray dissolves rather than ending. The cross rays stay short, they just finish properly now.',
  ...
  draw(P, g, w, h, t, inp) {
    ...
    for (const p of s.ap) {
      ...
      if (p.open > 0.2) {
        ...
        const dirs = [
          { a: eAng, len: 1, w: 0.475, wMin: 4.5, av: 0.15 },
          { a: eAng + Math.PI, len: 1, w: 0.475, wMin: 4.5, av: 0.15 },
          { a: eAng + Math.PI / 2, len: 0.4, w: 0.22, wMin: 3, av: 0.1 },
          { a: eAng - Math.PI / 2, len: 0.4, w: 0.22, wMin: 3, av: 0.1 }
        ];
        g.lineCap = 'round';
