/* ---------- SRC-41.5 · POUR CELLS V5 (the pops are notes, and they queue) ---------- */
reg({
  id: 'SRC-41.5', family: 'SRC-41', ver: 5, title: 'Pour Cells V5', tech: 'BOILING SOURCE / PITCHED BURSTS',
  music: {
  ...
  fx: { bloom: 0.4 },
  tags: ['A SOURCE UNDER THE FRAME', 'L = HEAT · R = THE CHORD', 'ONE BURST, ONE NOTE', 'THE QUEUE PLAYS IN ORDER', 'D PHRYGIAN'],
  ...
  sound: 'ONE BURST, ONE NOTE — and it is a NOTE, not a click. The burst bends into its pitch in six to twenty milliseconds and then RINGS there, a fifth of a second for a speck and nearly a full second for a lake, with a twelfth over the top for the first instant to keep the consonant of a pop and a few milliseconds of bandpassed noise for the film tearing. There is no burst anywhere on this frame you do not hear: the rate limit is spent on the physics, so a cell that reaches its bursting radius while the room is full holds there, stretched and white-veined, and goes when there is room — about one a sixteenth at full heat. Pitch class is the rung the cell was issued at the vent; the octave is Minnaert, off its radius when it fails, so a lake and a speck can carry the same note and still sound like a lake and a speck. The ambience is made of the same material: three narrow resonators tuned live to the chord\'s own tones, a sine on the root under the floor, six sustained cell voices that are sine plus a twelfth — the pop held open instead of struck — and every burst thrown hard into the same long reverb so its tail is the pad. MIDI: pops → bells ch5 with the note length they actually ring, big-lake pops bloom bass ch3, merges → lead ch1. CC74: pad = heat, texture = fill, bass = biggest cell.',
  ...
  _pop(A, freq, { at = 0, vol = 0.12, size = 0.4, pan = 0 } = {}) {
    ...
    const ring = 0.20 + big * 0.62;               // and then it SINGS: 0.2 → 0.8 s
    ...
    // THE NOTE — bends down into pitch, then holds it all the way out
    ...
    g.gain.exponentialRampToValueAtTime(vol * 0.36, t0 + bend + 0.03);
    g.gain.exponentialRampToValueAtTime(0.0001, t0 + ring);
    o.connect(g); g.connect(out);
    o.start(t0); o.stop(t0 + ring + 0.05);

    // OCTAVE — thickens the fundamental without arguing with it
    const o3 = ctx.createOscillator(); o3.type = 'sine';
    o3.frequency.setValueAtTime(freq * 2.98, t0);
    o3.frequency.exponentialRampToValueAtTime(freq * 2, t0 + bend * 1.6);
    const g3 = ctx.createGain();
    g3.gain.setValueAtTime(0.0001, t0);
    g3.gain.linearRampToValueAtTime(vol * 0.3, t0 + 0.002);
    g3.gain.exponentialRampToValueAtTime(0.0001, t0 + ring * 0.42);
    o3.connect(g3); g3.connect(out);
    o3.start(t0); o3.stop(t0 + ring + 0.05);

    // SNAP — the consonant that makes it a POP and not a bell. Short enough
    // now that it colours the attack instead of being the whole sound.
    const o2 = ctx.createOscillator(); o2.type = 'triangle';
    o2.frequency.setValueAtTime(freq * (5.4 - big * 2.6), t0);
    o2.frequency.exponentialRampToValueAtTime(freq * 2.98, t0 + bend * 0.8);
    const g2 = ctx.createGain();
    g2.gain.setValueAtTime(0.0001, t0);
    g2.gain.linearRampToValueAtTime(vol * (0.34 - big * 0.2), t0 + 0.001);
    g2.gain.exponentialRampToValueAtTime(0.0001, t0 + 0.03 + big * 0.05);
    ...
    ng.gain.setValueAtTime(vol * 0.34, t0);
    ng.gain.exponentialRampToValueAtTime(0.0001, t0 + 0.005 + big * 0.008);
  ...
  step(P, dt, t, inp) {
    ...
    s.acc += dt * (2.6 + HE * 5.4) * 1.25 * (1 - clamp((s.readyN || 0) / 12)) * (1 - s.fill * 0.5);
  ...
  audio(A, P) {
    ...
    const octOf = () => 0;
    const notePitch = (deg) => H.chordTone(deg || 0, 0);
