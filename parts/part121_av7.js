/* ---------- SRC-09.7 · ATTRACTOR VESPERS V7 (the awakening) ---------- */
reg({
  id: 'SRC-09.7', family: 'SRC-09', ver: 7, title: 'Attractor Vespers V7', tech: 'CLIFFORD MAP / FLAME DENSITY',
  music: {
  ...
  tags: ['STRANGE ATTRACTOR', 'FLAME DENSITY', 'LIGHT = SOUND', 'THE AWAKENING'],
  desc: 'V6\'s living organism, plus the moment it NOTICES you: the first touch after stillness fires a real recorded one-shot — an electromagnetic surge (Lance\'s sample, played straight from the browser, no rig needed) — and the whole figure answers with a flush of bioluminescence rolling through its veins. Warm amber body, cold glow where the orbit works, and the synth still played by the measured light.',
  interact: 'L bends constant a, R bends constant b — the figure obeys instantly, and reach outward = more: at the sphere the map rests as condensed embers, full stretch on both sides is the torn smoke at its most chaotic. Put your hands in after the scene has sat still and it WAKES: the surge fires once and the glow flushes through every vein — then it is yours. Color is anatomy, not position: amber body = settled smoke, cold glow = where the point weaves right now, tinted by the hands (L green-gold, R violet-blue). Brightness drives the synth, sharpness rings the air, the beam voice follows the light\'s mass.',
  sound: 'ONE SYNTH, played by the picture (V5\'s engine), plus THE AWAKENING: the first engagement after ~2 seconds of stillness plays a recorded electromagnetic surge one-shot (assets/av_wake.wav, decoded and fired in WebAudio — samples in the repo need no Ableton rigging) and re-arms only after the hands leave again, so it stays a moment, never a loop. Mirrored as note 41 on the sfx channel 11 for the rig. Everything else as V6: brightness = intensity, sharpness = resonance, the beam voice rides the light\'s centroid on lead ch1, chords drift Gm(add9) → Gm9 → Cm11 → E♭maj9 behind a quiet roll.',
  init(P) {
    // CURATED SEED (as V3–V5): rich where the hands reach; rest may collapse.
    ...
    P.state = {
      ...
      bright: 0, conc: 0, cenX: 0.5, cenY: 0.5, wake: 0,
  ...
  draw(P, g, w, h, t, inp) {
    const s = P.state;
    /* ---- FLAME-DENSITY RENDER, ORGANISM PALETTE ------------------------
       TWO fields now: `den` is the slow-decay body (the smoke's history)
       and `fre` is a fast-decay trace of where the orbit is working RIGHT
       NOW. Color comes from the form itself: the body renders as one warm
       organic ramp (russet → amber → bone), and cells whose fresh trace
       runs hot are pulled toward a cold bioluminescent glow — living veins
       inside a warm mass, never a spatial filter. */
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (bx >= 0 && bx < bw && by >= 0 && by < bh) {
        const ii = by * bw + bx;
        den[ii] += 1;
        fre[ii] += 1;                     // the living trace
        if (halo) {
          // ember halos are BODY (amber dust); the cores alone stay alive
    ...
    /* palettes — BODY (one warm organic ramp, position-free) and GLOW (the
       bioluminescence, hue tinted by the hands: L green-gold, R violet). */
    if (!s.amb || Math.abs(s.peak - s.lutPeak) > s.lutPeak * 0.1 || Math.abs(s.tilt - s.lutMix) > 0.04) {
      ...
      /* the bioluminescence stays SATURATED at neutral (pure cyan-teal) and
         the hands shade it: L toward green-gold, R toward violet-blue —
         never averaged into gray */
      // glow stays SATURATED even at high tone — the veins must read teal,
      // not white (the white crown belongs to the amber ramp's peaks alone)
    ...
    // one pass: decay both fields + tone-map + occupancy + peak + light measure
    ...
    for (let i = 0, j = 0; i < den.length; i++, j += 4) {
      ...
      if (tn > 0.6) hi++;
      /* ALIVE, two sources:
         STRUCTURAL — the vein band: mid-bright caustic tissue glows while
         the hottest cores stay bone-white and the thin body stays amber
         (in a held figure the veins ARE the living part of the form).
         TEMPORAL — fresh-over-density: where the orbit has just arrived,
         fr runs far above its steady share of v, so the leading edge
         blazes and abandoned smoke cools back to amber. */
      ...
      // THE AWAKENING: the surge one-shot flushes bioluminescence through
      // every vein of the body at once, then cools back to amber
  ...
  audio(A, P) {
    ...
    /* --- ONE SYNTH, played by the picture (V5's engine, unchanged) ------- */
    ...
    /* --- THE AWAKENING: a real recorded one-shot, no rig required --------
       Samples dropped in assets/ play straight from WebAudio: fetch,
       decode once, fire on the trigger. The preview builder inlines them
       so the offline harness hears them too; MOut mirrors the moment as a
       note on the sfx channel for setups that want Ableton's own copy. */
    let wakeBuf = null;
    ...
    const playWake = () => {
      ...
      g.gain.value = 0.34;                 // sample peaks near 0dBFS — sit it in the mix
      src.connect(g); g.connect(v.group);
    ...
    let swoop = 0, wakeArmed = true, stillT = 0;
    return {
      tick(inp, dt) {
        const s = P.state;
        ...
        // the first touch after stillness wakes the organism — once, then it
        // re-arms only when the hands have actually left again
        if (s.pres < 0.08) stillT += dt; else stillT = 0;
        if (stillT > 2) wakeArmed = true;
        if (wakeArmed && s.pres > 0.3) {
          wakeArmed = false;
          s.wake = 1;
          playWake();
        }
