/* ---------- SRC-15.10 · FERRO BLOOM V10 (clear flower silhouette, seamless color, gradient dots) ---------- */
reg({
  id: 'SRC-15.10', family: 'SRC-15', ver: 10,
  title: 'Ferro Bloom V10', tech: 'SHADER FIELD / MOTION ILLUSION',
  music: {
  ...
  desc: 'The hero bloom finally reads as a FLOWER — six clean, deep-cut petals instead of a soft triangular blob — while its satellites keep their varied silhouettes, soft to spiky. Speckle grain is gone (it was pixelating on the mesh); in its place, roughly a third of the blooms wear a small polka-dot pattern, each dot tinted by a smooth gradient that sweeps through the palette from bloom center to edge. Color itself no longer breaks at the seams between flowers — hue is now a fully continuous field, so where two blooms overlap the tint melts across the boundary instead of snapping. Full seven-color palette: blush, teal, ember, lavender, acid chartreuse, electric blue, hot orchid. Music unchanged from V9: a three-layer soundscape — C-minor pedal drone whose color shifts, a quantized groove that only exists once the garden is thrown wide open, and a reactive layer where every willed bloom, flick, and stillness sounds back at you.',
  ...
  init(P) {
    ...
    const mat = new THREE.ShaderMaterial({
      ...
      fragmentShader: [
        ...
        '}',
        // V10: ~35% of blooms wear a small polka-dot pattern instead of the
        // old speckle grain — driven by the same seed that fixes their
        // shape, so it reads as a stable trait, not noise.
        'float bloomDotted(float seed){ return step(0.65, fract(seed * 21.7)); }',
        'float dotMask(vec2 q, float S, float seed){',
        '  vec2 gp = q / max(S, 1e-4) * 2.7;',
        '  vec2 cell = floor(gp);',
        '  vec2 fp = fract(gp) - 0.5;',
        '  float h = hash21(cell + seed * 19.0);',
        '  vec2 jitter = vec2(hash21(cell + seed * 11.0 + 3.0), hash21(cell + seed * 7.0 + 5.0)) - 0.5;',
        '  vec2 dpos = fp - jitter * 0.35;',
        '  float rdot = 0.20 + 0.12 * h;',
        '  float dd = length(dpos);',
        '  return 1.0 - smoothstep(rdot * 0.6, rdot, dd);',
        '}',
        ...
        '  if (h < 4.0) return vec3(0.58, 0.56, 0.90);',   // lavender-periwinkle
        '  if (h < 5.0) return vec3(0.878, 1.0, 0.161);',  // acid chartreuse  #e0ff29
        '  if (h < 6.0) return vec3(0.0, 0.263, 1.0);',    // electric blue    #0043ff
        '  return vec3(1.0, 0.341, 0.831);',                // hot orchid      #ff57d4
        '}',
        'void main(){',
        ...
        // V10: gradient-colored dot pattern, masked to this bloom's own
        // silhouette and weighted by its field contribution so it fades
        // smoothly at the union boundary instead of cutting off hard.
        '    if (bloomDotted(seed) > 0.5 && b > 0.5) {',
        '      float ownInside = 1.0 - smoothstep(0.80, 1.0, f);',
        '      float dm = dotMask(q, S, seed) * ownInside;',
        // dot hue is offset ~half the palette away from this bloom's own base
        // hue (fract(seed*5.13)) so dots read as a clear pattern against the
        // petal instead of blending into a same-family tint; the offset still
        // sweeps smoothly with radius and time, so the dots carry their own
        // gradient rather than a single flat color.
        '      float baseHuePos = fract(seed * 5.13);',
        '      float dhIdx = fract(baseHuePos + 0.5 + f * 0.35 + uT * 0.015) * 6.99;',
        '      vec3 dotColor = mix(ringTint(floor(dhIdx)), ringTint(floor(dhIdx) + 1.0), smoothstep(0.0, 1.0, fract(dhIdx)));',
        '      dotColor = mix(dotColor, vec3(1.0, 0.97, 0.92), 0.38);',
        '      dotAcc += dm * w;',
        '      dotColAcc += dotColor * dm * w;',
        '    }',
        ...
        // V10: SEAMLESS COLOR — hueOff is now a continuous field (no floor),
        // so as weight shifts from one bloom to its neighbor across a union
        // boundary, hue slides smoothly instead of snapping a full palette
        // step. Scaled across the whole 7-color set to keep full variety.
        '  float hueOff = clamp(hueAcc / max(acc, 1e-20), 0.0, 1.0) * 6.99;',
        '  float dotLevel = clamp(dotAcc / max(acc, 1e-20), 0.0, 1.0);',
        '  vec3 dotColFinal = dotColAcc / max(dotAcc, 1e-20);',
        ...
        // so luminance never flatlines at either extreme.
        '  float bandsN = 2.6;',
        ...
        // V9: full-range smoothstep, no flat plateau — hue melts continuously
        // across the whole ring instead of holding solid then snapping.
        '  vec3 tint = mix(ringTint(floor(rif)), ringTint(floor(rif) + 1.0), smoothstep(0.0, 1.0, fract(rif)));',
        ...
        '  bc = mix(bc, dotColFinal, dotLevel);',
        '  bc += dotColFinal * dotLevel * 0.14;',
  ...
  draw(P, g, w, h, t, inp) {
    ...
    if (s.noGL || !P._three) {
      ...
      g.textAlign = 'center';
      g.fillText('FERRO BLOOM V10 · FLOWER, SEAMLESS, GRADIENT DOTS', w / 2, h / 2 - 12);
