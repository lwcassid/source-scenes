...
reg({
  id: 'SRC-42.11', family: 'SRC-42', ver: 11, title: 'Ridge Loom V11', tech: 'V10 + BREATHING REST, EARNED GRID, REAL CONVERGENCE',
  music: {
  ...
  sound: 'The playing instrument is V10\'s band with dynamics: every grid voice now derives its velocity from the gesture (core width and reach speed) with downbeat accents, the arpeggio\'s density is earned (quarters at low reach, eighths near full), and the shared-ring convergence is real — arp and bell degrees are pulled toward the long tone\'s pitch as rings enclose more cores, instead of a 2-cent detune nobody could hear. The RESTING state is rebuilt to the studio\'s idle law: the bar-clock bass root is gone; below presence the grid is silent and the scene breathes instead — randomized low swells (length, depth, voice and spacing all rolled per breath, roughly one in seven landing a deeper toll) over a loom-noise floor that undulates on two incommensurate slow LFOs, never dead, never periodic. Each breath pumps the wave field so the sound visibly moves the picture. Ableton: bass ch3, lead ch1, bells ch5, pad ch2, texture ch6 — velocities now carry the dynamics.',
    ...
    return {
      tick(inp, dt) {
        ...
        while ((ev = s.evq.shift()) && k < 4) {
          ...
          if (c.side === 0) {
            // the floor moved
            A.bassNote(toneOf(ev.core, -1), { at, vol: (ev.up ? 0.09 : 0.05) * spdAcc(c) * gate, dur: ev.up ? 1.4 : 0.7 });
          } else if (ev.core !== 3) {
