...
const LUMEN18_STOPCACHE = new Map();
reg({
  id: 'SRC-38.18', family: 'SRC-38', ver: 18, title: 'Lumen Film', tech: 'GEM FILM / APERTURE ORGAN',
  music: {
  ...
  draw(P, g, w, h, t, inp) {
    ...
    const addSvgStops = (grad, a) => {
      let set = LUMEN18_STOPCACHE.get(a);
      if (!set) {
        set = SVGSTOPS.map(([t, rgb]) => [t, `rgba(${rgb},${a})`]);
        LUMEN18_STOPCACHE.set(a, set);
      }
