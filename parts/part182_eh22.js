...
reg({
  id: 'SRC-13.22', family: 'SRC-13', ver: 22, title: 'Event Horizon V22', tech: 'VERLET ROPE / ACCRETION',
  music: {
  ...
  audio(A, P) {
    ...
    return {
      tick(inp) {
        ...
        if (typeof MOut !== 'undefined') {
          if (gateSy) {
            ...
            while (synthNext < now + 0.15 && g3++ < 4) {
              MOut.evNote('synth', H.chordTone(0, -1), 0.15 + 0.11 * armAvg, synthNext, barLen * 1.02);
        ...
        A.set(syo.frequency, H.chordTone(0, -1), 0.08);
        ...
        if (typeof MOut !== 'undefined' && MOut.expr) {
          ...
          MOut.expr('synth', clamp(0.15 + 0.85 * armAvg) * s.pres); // ch16 filter: the arms play it
