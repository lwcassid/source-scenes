...
reg({
  id: 'SRC-09.12', family: 'SRC-09', ver: 12, title: 'Attractor Vespers V12', tech: 'CLIFFORD MAP / FLAME DENSITY',
  music: {
    ...
    return {
      tick(inp, dt) {
        ...
        if (typeof MOut !== 'undefined') {
          ...
          MOut.expr('pad', br);
          MOut.expr('lead', clamp((mouth - 340) / 1500));   // CC74 lead = THE MOUTH
          MOut.expr('arp', churn);
          MOut.expr('bass', clamp(mawLvl * 16));
        }
      },
      stop() {
        if (typeof MOut !== 'undefined') { MOut.holdOff(beam.h); MOut.holdOff(beam2.h); MOut.holdOff(mawH); MOut.holdOff(subH); }
        bv.kill(); v.kill();
      }
    };
  }
