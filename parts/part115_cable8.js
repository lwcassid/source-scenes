/* ---------- SRC-13.8 · EVENT HORIZON V8 — scrim pass + expressive flick ---------- */
/* Hardening for the wall + deepening the instrument. SCRIM: stars fattened
   ~2x (thin streaks vanish on mesh), the void rim gets a brightness floor
   sized for fabric that eats half the light, physics and gradient columns
   coarsen on mobile/tile. FLICK: run direction follows the gesture (up-flick
   = rising run, down = falling), sharpness picks the register — a lob is a
   low dark run, a snap is high and bright. Same one mechanic, more hand. */
reg({
  id: 'SRC-13.8', family: 'SRC-13', ver: 8, title: 'Event Horizon', tech: 'VERLET ROPE / ACCRETION',
  music: {
  ...
  draw(P, g, w, h, t, inp) {
    ...
    const rimA = 0.38 + s.tension * 0.18 + s.chg * 0.3;
