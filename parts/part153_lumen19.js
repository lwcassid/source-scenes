...
reg({
  id: 'SRC-38.19', family: 'SRC-38', ver: 19, title: 'Lumen Film V19', tech: 'GEM FILM / APERTURE ORGAN',
  music: {
  ...
  audio(A, P) {
    ...
    const bassPedal = () => {
      ...
      MOut.evNote('bass', H.rootFreq(-1), 0.06 + 0.05 * (s0.lamp || 0), 0, hold);
    };
    retune(0.05);
    bassPedal();
      ...
      tick(inp, dt) {
        ...
        while ((ev = s.evq.shift()) && i < 6) {
          ...
          if (ev.on) {
            ...
            p.deg = deg; p.oct = oct;
            setV(vc, H.chordTone(deg, oct), 0.08, 52 + 26 * s.lamp + 12 * up);
