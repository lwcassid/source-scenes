/* ---------- SRC-37.3 · IRIS ENGINE V3 (halos that melt together) ---------- */
reg({
  id: 'SRC-37.3', family: 'SRC-37', ver: 3, title: 'Iris Engine V3', tech: 'RING HALOS / MORPH = CHORD',
  music: {
    bpm: 112, root: 40, mode: 'phrygian', chordBars: 8,
    // SIX CHORDS FOR SIX SHAPES — the chord is chosen by the shape the right
    // hand is making, not by a timer.
    chords: [
      ...
      [0, 1, 8, 15, 18]     // 5 · seven lobes  E♭9♯11
    ],
    chordNames: ['Em7', 'C/E', 'Em11', 'Am11/E', 'E♭9 phryg', 'E♭9♯11']
  },
  fx: { bloom: 0.6 },
  tags: ['FIG PALETTE', 'SHAPE CHOOSES THE CHORD', 'HALOS MELT AT SPEED', 'THE ROOM CLOSES FROM THE SIDES'],
  desc: 'A tunnel of fat bands in fig colours — indigo, royal purple, amber, pale tan, fig flesh — boiling out of one eye. Each band carries a halo, and the faster the engine runs the wider those halos spread, until neighbouring rings stop being separate things and melt into one another into a single moving body of light. Every band also listens: when anything else on the wall rings the tone it is tuned to, it flares on its own. And the room has edges — the picture is brightest through the middle and gives itself up toward the left and right walls, so when the instrument is left alone it does not blink out, it closes in from the sides like a door shutting.',
  interact: 'R = MORPH, and morph is harmony. Drawn in the rings are true circles; reach out and they grow lobes — three, four, five, six, seven — and lean off-axis, and each of those six shapes has its own chord, taken the moment the shape settles: Em7 → C/E → Em11 → Am11/E → E♭9 → E♭9♯11, open to sour. The picture tells you which harmony you are in before the ear does. L = INTENSITY: a ladder of speeds from a band every two beats up to sixteenths, and with it the halos widen — at the top of the ladder the rings have bled into each other and the tunnel is one melting body rather than a set of hoops. As long as either hand is off its rest the scene stays lit. Only when both hands come home AND stop moving does the light withdraw, from the sides inward, over about ten seconds; the smallest movement brings the whole room back instantly.',
  sound: 'The arpeggio walks the chord ladder one note per band (MIDI role: arp), with the grid step taken straight from the ring rate so the note and the band arrive together. Underneath, a sawtooth pedal filtered by two formants driven directly off the raw hands (CC1/CC2) — even between notes a moving hand changes the vowel of the drone — and a tremolo on it running at exactly the ring rate, so the flicker you see is the flicker you hear. When the shape settles into a new morph the harmony JUMPS rather than waiting for the bar. Bells double the arpeggio an octave up once the tunnel is both fast and lobed; the kick appears only at the top of the intensity ladder. With both hands at rest the whole rig ducks to a bare pedal as the picture closes in. Ableton: arp ch4, texture ch6 = the vowel drone, pad ch2, bells ch5, perc ch10.',
  ...
  step(P, dt, t, inp) {
    ...
    /* ---- AWAKE = A HAND OFF ITS REST, OR A HAND MOVING ------------------
       Holding a position IS playing — a theremin player parked at 90 is
       engaged, and the scene has no business fading on them. It only lets go
       when both hands are home and nothing has moved for about ten seconds. */
    ...
    const held = clamp((Math.max(inp.L, inp.R) - 0.03) / 0.22);   // any real CC value holds it open
    ...
    // MELT: how much the halos have spread into each other
  ...
  draw(P, g, w, h, t, inp) {
    ...
    if (wake < 0.006) {
      g.fillStyle = 'rgba(235,199,154,0.3)';
    ...
    // THE ROOM CLOSES FROM THE SIDES: the walls give up their light first and
    // the middle is the last thing left, so sleep reads as a door shutting
    const edge = clamp((wake - 0.55) / 0.45);       // the far left and right
    const mid = clamp((wake - 0.18) / 0.5);         // the thirds
    const core = clamp(wake / 0.3);                 // the middle itself
    ...
    for (let k = NR - 1; k >= 0; k--) {
      ...
      // TWO PASSES: the band, and a wide soft halo around it. At speed the
      // halos of neighbouring rings overlap and the tunnel melts into one body.
      for (let pass = 0; pass < 2; pass++) {
        const halo = pass === 0;
        // the halos must BLEED into their neighbours, not flood the frame —
        // past about 40% overlap the tunnel stops reading as rings at all
        const lw = halo
          ? base * (1.4 + s.melt * 2.6)
        ...
        gr.addColorStop(0, `rgba(${c0[0]},${c0[1]},${c0[2]},${a * warm * edge})`);
        gr.addColorStop(0.26, `rgba(${c0[0]},${c0[1]},${c0[2]},${a * warm * mid})`);
        gr.addColorStop(0.5, `rgba(${c1[0]},${c1[1]},${c1[2]},${a * core})`);
        gr.addColorStop(0.74, `rgba(${c2[0]},${c2[1]},${c2[2]},${a * cool * mid})`);
        gr.addColorStop(1, `rgba(${c2[0]},${c2[1]},${c2[2]},${a * cool * edge})`);
        g.strokeStyle = gr;
        g.lineWidth = lw;
        g.beginPath();
        for (let i = 0; i <= SEG; i++) {
    ...
    eg.addColorStop(0, `rgba(255,226,186,${(0.1 + s.pulse * 0.22) * bright * core})`);
    eg.addColorStop(0.5, `rgba(255,178,68,${(0.05 + s.pulse * 0.12) * bright * core})`);
    eg.addColorStop(1, 'rgba(255,96,46,0)');
    g.fillStyle = eg;
    g.beginPath(); g.arc(cx, cy, rr * 1.15, 0, TAU); g.fill();
    g.globalCompositeOperation = 'source-over';

    g.fillStyle = 'rgba(255,226,186,0.85)';
    g.font = `${Math.round(10 * ms)}px ui-monospace,monospace`;
