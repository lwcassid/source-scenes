/* ---------- SRC-34.10 · WHITE STUDY V10 (the DJ takes the latch)
   Lance's DJ-set round. This was always a techno scene — it synthesizes
   its own kick and sidechains its own drone — so the adaptation is the
   smallest of the first wave: when a booth feed is live (audioIn:true,
   arbitrated on inp.audio.live), the REAL kick fires the existing
   s.pump envelope (frame brightness, bar width, stab width — the
   latch's whole look, played from outside), a slow bass floor under eL
   thickens the lattice with the low end, a slow treble floor under eR
   fills the 16-step groove beats-first (the DJ's hat density literally
   fills the grid), and a structural DROP (inp.audio.drop, part2e)
   opens the pink inversion window — bounded, bar-quantized, the
   scene's own drop language. A VISUAL presence (presV = max of hand
   presence and audio liveness) wakes density, luminance and the
   rhythmic recuts, so the wall dances for the room with nobody at the
   pedestal — but the scene's OWN sound stays hand-gated: idle highs
   stay dead, the machine never plays clicks over the DJ's track, and
   the crash/summon ritual, the jab detectors and the drop state
   machine read the RAW hands only (the audit's shield: audio never
   touches commit/still, so a loud section can never arm-and-abort the
   riser). With no signal this is exactly V9. (V9 notes below.) ----- */
/* ---------- SRC-34.9 · WHITE STUDY V9 (the clicks stay home)
   Pre-built rig round (Lance's head-start): one casting fix. The Ikeda
   16th clicks -- machine-flat ON PURPOSE, "they ARE the aesthetic, they
   stay browser" per the rig doc -- were auto-mirroring through A.hit
   onto kit pad 46 (the dusty OPEN hat), spraying machine 16ths over the
   R&B kit's air pad through every groove and build. midi:false now: the
   clicks are browser-only, and the kit hears exactly what it should --
   the club kick with its 119 accents, the crash on 49, nothing else.
   Everything else was already deliberate: stabs velocity-honest on
   lead, pattern + hook on arp, half-weight sub on bass, breaths on the
   bed sampler. Browser sound and visuals untouched from V8. -------- */
/* ---------- SRC-34.8 · WHITE STUDY V8 ---------- */
reg({
  id: 'SRC-34.10', family: 'SRC-34', ver: 10, title: 'White Study V10', tech: 'DATA MINIMAL / INWARD FIELD / LISTENS',
  audioIn: true,
  music: {
  ...
  tags: ['SEVERE', 'ETCHED PLATES', 'THE DJ TAKES THE LATCH', 'THE CRASH'],
  desc: 'V9, given ears (plug the booth in and the machine listens): every real kick pumps the lattice through the same envelope the latched kick owns, the DJ\'s low end thickens the plates, their hat density fills the 16-step grid beats-first, and a structural DROP opens the pink inversion window. The wall dances for the room with nobody at the pedestal — but the scene\'s own clicks stay dead and the crash ritual stays in your hands. With no signal, exactly V9. The field turned inside out: intensity lives AT the source now. Lean a hand in toward the sphere and the lattice thickens and brightens around you; withdraw and it thins to nothing — the far edge of the field is the quiet end, so a hand drifting out of tracking settles the scene instead of slamming it. Press BOTH hands deep and hold still and the crash builds — a rising wall of noise that cuts to one silent sixteenth and detonates on the next chord change, cymbal and kick together on beat one. Then the beat is YOURS: the kick latches for sixteen bars with your hands completely free, so you can actually drum — jab either hand at the source and the stab lands on the next sixteenth while the whole lattice pumps to every kick. The pink came back too: besides the two-hand summon, the inversion window now drifts through on its own every now and then while you play, rails first, one bar of hot pink, gone. And when nobody is there the scene finally RESTS — a dim lattice of slender plates breathing slowly, low bass breaths instead of clicks, the occasional ghost stab still teaching the verb.',
  ...
  sound: 'V9\'s machine, untouched — plus the listening layer: audio drives VISUALS only (pump, density, groove fill, the pink window); the scene\'s own kick, clicks and arp stay gated on hand presence, so it never drums over the DJ\'s track. V7\'s club machine with the polarity flipped and the drop rebuilt. THE RISER IS A CRASH NOW: a looping white-noise sweep (bandpass 300→5200Hz) crescendos under thin detuned sines, cuts to one dead-silent sixteenth, and a highpassed noise CRASH lands sample-locked with the first kick — and the drop is TIMED: it always falls on the next 2-bar chord boundary at least one bar out, so beat one of the drop is also a chord change. THE LATCH: the kick no longer needs held hands — it runs 16 bars free (recommit extends, absence or expiry schedules the landing bar + final accent at 119 velocity), so stabs (cooldown down to 0.3s) become an actual drum performance over the four-on-the-floor, sidechain pump and all. THE REST: idle drops every click (idle highs are dead) — the bed sits at a whisper and randomized low bass breaths (55Hz swells, 1-in-7 a deeper two-voice toll) breathe the picture visibly; grid, hook and sub wake with presence. MIDI: crash → perc 49, breaths → bed sampler, kick 36 with 119 accents; the Ikeda clicks stay BROWSER-ONLY by design (V9) - the kit never hears them.',
  ...
  init(P) {
    P.state = {
      ...
      // V10: the listening layer — audio liveness, slow band floors, event
      // counters, and presV = the VISUAL presence (hands OR booth)
      aPres: 0, aBass: 0, aTreb: 0, presV: 0, _kN: 0, _dropN: 0, _aSeeded: false
    };
  ...
  step(P, dt, t, inp) {
    ...
    if (au) {
      ...
      s.aTreb += (clamp(au.treble) - s.aTreb) * Math.min(1, dt * 1.5);
      // seed event counters on first sight — no phantom pump/window on open
      if (!s._aSeeded) { s._aSeeded = true; s._kN = au.kick ? au.kick.n : 0; s._dropN = au.drop ? au.drop.n : 0; }
      // THE DJ TAKES THE LATCH: the real kick fires the existing pump
      // envelope, unsmoothed, back-dated by the hit's true age.
      if (au.kick && au.kick.n !== s._kN) {
        ...
        s.pump = Math.max(s.pump, hit * Math.exp(-6.9 * (age + 0.030)));
      }
    } else { s.aBass *= Math.pow(0.5, dt); s.aTreb *= Math.pow(0.5, dt); }
    // slow band FLOORS under the hands (Math.max — a hand always wins a
    // raise; the jab/crash detectors read RAW inp and never see these)
    s.eL = Math.max(s.eL, s.aBass * 0.9 * s.aPres);
    s.eR = Math.max(s.eR, s.aTreb * 0.95 * s.aPres);
    // VISUAL presence: the wall dances for the room when either the hands
    // or the booth are live; the scene's own sound stays hand-gated.
    ...
    const density = Math.max(1, Math.round(9 + (liveN - 9) * s.presV));
    const n = this._nSteps(s.eR);
    s.nSteps = n;
    s.cutoff = 450 * Math.pow(13, s.eL);
    const beats = (P.focused && T.running) ? T.beats() : t * 2;
    s.beats = beats;
    // THE DROP OPENS THE PINK: a structural drop (part2e's detector) enters
    // the scene's own drop language — the inversion window, bar-quantized,
    // rails telegraphing first. Never the white flash (that stays with the
    // hand-built crash), never the state machine.
    if (au && au.drop && au.drop.n !== s._dropN) {
      s._dropN = au.drop.n;
      if (!s.win && s.mode !== 'build' && s.aPres > 0.3) {
        const startBeat = T.running ? Math.ceil(beats + 1e-3) : beats + 0.001;
        this._openWin(P, beats, startBeat, au.drop.n * 977 + 131);
      }
    }
  ...
  _pass(P, g, w, h, flipped) {
    ...
    // V10: the VISUAL presence — the booth wakes the picture like a body does
    const rest = 1 - (s.presV || s.pres);
    // resting the scene DIMS and breathes; the kick pump snaps it bright
    const lum = Math.min(1, 0.55 + 0.45 * (s.presV || s.pres) + 0.12 * rest * Math.sin(s.age * 0.23)
    ...
    g.fillText('STEPS ' + (s.nSteps || 1) + '/16  N ' + s.bars.length + '  ' +
      (s.presV < 0.15 ? 'REST' : s.mode.toUpperCase()) +
      (s.aPres > 0.3 ? '  AUD b' + Math.round(s.aBass * 100) + ' t' + Math.round(s.aTreb * 100) : '') +
          ...
          if (idx > lastIdx) {
            ...
            if (building) {
              ...
              if (s.dropAtBeat - b > 0.26) {
                ...
                A.hit({ vol: 0.05 + prog * 0.07, dur: 0.006, freq: 8000, q: 0.4, at: tt, midi: false });
                if (prog > 0.55) A.hit({ vol: 0.03 + prog * 0.05, dur: 0.005, freq: 9500, q: 0.4, at: tt + stepDur / 2, midi: false });
              }
            } else if (!resting && s.rank[st] < self._nSteps(s.eR)) {
              A.hit({ vol: 0.09, dur: 0.006, freq: 8000, q: 0.4, at: tt, midi: false }); // machine-flat, by design -- and BROWSER-ONLY by design (the rig doc: the clicks ARE the aesthetic)
              if (st === 6 || st === 14) {
