/* ---------- SRC-09.8 · ATTRACTOR VESPERS V8 (the soundscape, nailed) ---------- */
reg({
  id: 'SRC-09.8', family: 'SRC-09', ver: 8, title: 'Attractor Vespers V8', tech: 'CLIFFORD MAP / FLAME DENSITY',
  music: {
  ...
  tags: ['STRANGE ATTRACTOR', 'FLAME DENSITY', 'LIGHT = SOUND', 'IT ACCUMULATES'],
  desc: 'V7\'s living organism with the soundscape finished. The hum now OCCUPIES SPACE the way the light does — every voice sits in the stereo field and the image widens as the smoke widens. The beam voice truly bends: continuous portamento along the chord ladder, and a vibrato that only arrives when you hold the figure still, like a singer settling into a note. Minutes of bright playing accumulate HEAT that earns a deeper sub and a second beam harmonizing below; stillness with the figure blazing earns a slow upward bloom. The surge one-shot now ducks the hum so the awakening lands like a mixed event.',
  interact: 'L bends constant a, R bends constant b — reach outward = more, the wake fires on first touch after stillness, color is anatomy. New verbs to master: HOLD STILL with a bright figure and the beam starts to sing (vibrato) while the hum blooms slowly upward — restraint is a technique now. Keep the picture burning for minutes and the sound accumulates heat: the field widens, a deeper sub arrives, and a second voice joins in harmony below the beam. Walk away and the heat drains; minute nine sounds nothing like minute one.',
  sound: 'ONE SYNTH, played by the picture, now with memory and a stereo field. SPACE: bed voices and teeth fan across the field (root center, extensions outward), the cathedral airs sit hard left/right, and the whole image widens with the smoke\'s measured spread. THE BEAM: continuous portamento — its pitch springs along the chord ladder toward the light\'s centroid, bending between tones instead of stepping — with hold-earned vibrato (4.2→5.5Hz, deepening over ~6s of stillness). HEAT: brightness integrates over minutes; past the first threshold a deep sub-octave arrives (bar-latched), past the second a companion beam harmonizes two rungs below on lead ch1. STILLNESS BLOOM: ~4s of true stillness with the figure alight opens the filters and shimmers the top extension in, breaking the moment you move. The wake surge ducks the hum 45% for its 1.2s. Everything else as V5–V7: brightness = intensity, sharpness = resonance, chords drift on the 4-bar pedal. Ableton: pad ch2 CC74 = the LIGHT, beams held on lead ch1, sfx ch11 = the wake.',
  init(P) {
  ...
  draw(P, g, w, h, t, inp) {
    ...
    if (P.focused) {
      ...
      g.fillText('FIGURE ' + Math.round(clamp(s.occ / 0.09) * 100) + '   BRIGHT ' + Math.round(s.bright * 100) +
        ...
        (s.still > 0.05 ? '   BLOOM ' + Math.round(s.still * 100) : ''), 10, h - 24);
    }
  },
  audio(A, P) {
    const v = A.voice();
    ...
    return {
      tick(inp, dt) {
        ...
        /* HEAT — the arc: brightness integrates over minutes and earns
           layers; rest drains it. Gate rises are latched on the bar. */
        s.heat = clamp(s.heat + (br > 0.5 ? (br - 0.5) * dt / 75 : 0) - (br < 0.2 ? dt / 55 : 0));
        ...
        A.set(airL.g.gain, 0.010 + (1 - fig * 0.5) * 0.012 + br * 0.006, 0.4);
        A.set(airR.g.gain, 0.010 + (1 - fig * 0.5) * 0.012 + br * 0.006, 0.4);
        A.set(airC.g.gain, 0.005 + br * 0.007 + conc * 0.005 + bloom * 0.006, 0.4);
        ...
        const bl = s.pres * (0.004 + br * 0.028) * clamp(s.occ / 0.004) * (1 + bloom * 0.3);
      ...
      stop() {
        if (typeof MOut !== 'undefined') { MOut.holdOff(beam.h); MOut.holdOff(beam2.h); }
        bv.kill(); v.kill();
      }
    };
  }
