/* ---------- SRC-15.13 · FERRO BLOOM V13 (right hand grows the garden, left hand drives the flow) ---------- */
reg({
  id: 'SRC-15.13', family: 'SRC-15', ver: 13,
  title: 'Ferro Bloom V13', tech: 'SHADER FIELD / MOTION ILLUSION',
  music: {
  ...
  desc: 'The hands split their jobs cleanly now. Right grows the garden — up to 10 flowers on its side now, up from 6, so reaching out floods that half faster and fuller. Left no longer just summons its own 6; pulsing it drives the gradient itself, racing the color bands outward and cycling the palette through its warm↔cool scenarios as fast as you shake it, easing to a slow idle drift when it\'s still. The cluster grows a little wilder each time too: the center bloom rerolls its size on every fresh engagement — sometimes the hero, sometimes just one voice among equals — and every flower\'s position carries more jitter, so the fan never fills in quite the same way twice. Music unchanged: a three-layer soundscape — C-minor pedal drone whose color shifts, a quantized groove that only exists once the garden is thrown wide open, and a reactive layer where every willed bloom, flick, and stillness sounds back at you.',
  interact: 'THE HANDS DO DIFFERENT JOBS: L reach still summons up to 6 flowers on the left, same as before, but PULSING it now drives the gradient — shake it and the color bands race outward and cycle through the palette faster; hold it still and the flow eases back to a slow ambient drift. R reach summons up to 10 flowers on the right — more than L, so the right side fills faster and denser. Push a hand past ~65% and that side OVERFLOWS: small scatter blooms flood the full frame edge to edge. Keep it small and it stays chamber-music quiet; throw both arms wide and the groove fades in. Quick flicks fire log-drum answers only in full bloom. Stillness after a gesture invites the garden to answer.',
  ...
  init(P) {
    P.state = {
      ...
      eL: 0, eR: 0, vel: 0, velL: 0, spread: 0, pL: 0, pR: 0, still: 0,
      moodPh: 0, heroScale: 0.30, wasActive: false,
    ...
    const uni = {
      ...
      uMoodPh: { value: 0 }, uHeroScale: { value: 0.30 },
    ...
    const mat = new THREE.ShaderMaterial({
      ...
      fragmentShader: [
        'uniform float uT, uPh, uPres, uPulse, uBreath, uAmb, uEL, uER, uMoodPh, uHeroScale;',
        'uniform vec2 uRes;',
        ...
        '}',
        // V13: mood no longer drifts on raw time alone — it reads uMoodPh, a
        // phase JS accumulates from the LEFT hand's pulsing speed, so the
        // garden loops through warm↔cool scenarios exactly as fast as you
        // shake that hand (idle still gets a slow ambient baseline).
        'float seqMood(){ return 0.5 + 0.5 * sin(uMoodPh); }',
        'void main(){',
  ...
  step(P, dt, t, inp) {
    ...
    // V13: left-hand-only velocity — the dedicated "how hard is L pulsing"
    // signal that drives the gradient flow and mood cycling below.
    s.velL = s.velL * Math.max(0, 1 - dt * 2.2) + Math.abs(tL - s.pL) * 6;
    s.pL = tL; s.pR = tR;
    ...
    s.cL = cL; s.cR = cR;
    s.count = 1 + cL + cR;
    // V13: the LEFT hand's pulsing speed (not spread, not combined velocity)
    // drives both the outward gradient flow and how fast the color mood
    // cycles through its warm↔cool scenarios. Right hand no longer feeds
    // this at all — its job is flower count now.
    const flowDrive = 0.02 + Math.min(1.2, s.velL) * 0.55 + Math.pow(s.eL, 1.1) * 0.03;
    s.uph += dt * flowDrive * (0.35 + 0.65 * s.pres);
    s.moodPh += dt * flowDrive * 0.11 * (0.35 + 0.65 * s.pres);
  ...
  draw(P, g, w, h, t, inp) {
    ...
    if (s.noGL || !P._three) {
      ...
      g.textAlign = 'center';
      g.fillText('FERRO BLOOM V13 · SPLIT HANDS', w / 2, h / 2 - 12);
    ...
    u.uMoodPh.value = s.moodPh; u.uHeroScale.value = s.heroScale;
    for (let i = 0; i < 17; i++) {
