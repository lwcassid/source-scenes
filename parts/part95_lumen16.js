/* ---------- SRC-38.16 · LUMEN FIELD (finer stones) ---------- */
...
const LUMEN16_STOPCACHE = new Map();
reg({
  id: 'SRC-38.16', family: 'SRC-38', ver: 16, title: 'Lumen Film', tech: 'GEM FILM / APERTURE ORGAN',
  music: {
  ...
  init(P) {
    ...
    // Stones are 40% smaller than V15, so the count has to climb or the
    // film stops being a film: area falls with the square of the radius
    // (0.6^2 = 0.36), and a sparse field both shows black between the stones
    // and starves the lights, which only exist where two stones overlap.
    // 44 / 0.36 = 122 would hold V15's exact density, but measured at 10fps
    // under software rendering against V15's 27 — three times the cost, and
    // lag has bitten this scene twice. 64 buys back a good part of the weave
    // for a cost close to V15's. The weave is now the dial: raising this
    // fills the black between the stones and costs frames, roughly linearly.
    const N = areaScale(P) > 1.6 ? 64 : 26;
    const cells = [];
    for (let i = 0; i < N; i++) {
      cells.push({
        x: P.rand() * w, y: P.rand() * h,
        r: S * (0.045 + P.rand() * 0.141),   // 40% smaller than V15, same spread
  ...
  draw(P, g, w, h, t, inp) {
    ...
    const addSvgStops = (grad, a) => {
      let set = LUMEN16_STOPCACHE.get(a);
      if (!set) {
        set = SVGSTOPS.map(([t, rgb]) => [t, `rgba(${rgb},${a})`]);
        LUMEN16_STOPCACHE.set(a, set);
      }
