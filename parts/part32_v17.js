/* ---------- SRC-18.17 · NIGHT CIRCUIT V17 (no lanes — the road-lines glow with the music, left orange / right violet) ---------- */
reg({
  id: 'SRC-18.17', family: 'SRC-18', ver: 17,
  title: 'Night Circuit V17', tech: 'WEBGL / PROJECTION-READY',
  ...
  init(P) {
    P.state = {
      ...
      chomp: 0, chompHit: 0, chompDone: false, camMode: 0
    };
  ...
  step(P, dt, t, inp) {
    ...
    // lean = steer + lean into your side of the world (no discrete lanes anymore)
    ...
    const idL = clamp(0.4 + sideL * 0.6), idR = clamp(0.4 + sideR * 0.6);
    const pulseL = s.arpFlash * (0.9 + sideL * 1.6) + s.kickPulse * 0.35;
    const pulseR = s.stabFlash * (0.9 + sideR * 1.6) + s.kickPulse * 0.35;
    T3.edgeMats[0].color.copy(T3._cb).lerp(T3._co, idL).multiplyScalar(1 + sideL * 0.7 + pulseL);
    T3.edgeMats[1].color.copy(T3._cb).lerp(T3._cp, idR).multiplyScalar(1 + sideR * 0.7 + pulseR);
  ...
  draw(P, g, w, h, t, inp) {
    ...
    const lean = s.sideL > 0.15 ? 'ORANGE' : s.sideR > 0.15 ? 'VIOLET' : 'CENTER';
    g.fillStyle = 'rgba(150,200,220,0.75)'; g.font = `${Math.round(10 * Math.max(1, Math.sqrt(areaScale(P))))}px ui-monospace,monospace`;
    g.fillText(ACTS[P.state.act] + ' · ' + Math.round(s.speed * 6) + ' KM/H · ' + (s.pres < 0.3 ? 'WAITING IN THE DARK' : lean) + ' · [1-4] SKIP ACT', 10, h - 10);
  },
  audio(A, P) {
