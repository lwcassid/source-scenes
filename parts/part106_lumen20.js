/* ---------- SRC-38.20 · LUMEN FILM (the instrument warms up) ---------- */
// Colour-stop strings, memoised by the alpha they were built for — see V18.
const LUMEN20_STOPCACHE = new Map();
reg({
  id: 'SRC-38.20', family: 'SRC-38', ver: 20, title: 'Lumen Film', tech: 'GEM FILM / APERTURE ORGAN',
  music: {
  ...
  init(P) {
    ...
    P.state = { cells, ap, pres: 0, lightP: 0, thick: 0, rotL: 0, spin: 0, spinRate: 0, _pvSpin: 0, openN: 0, lamp: 0, temper: 0, evq: [] };
  },
  ...
  step(P, dt, t, inp) {
    ...
    // TEMPER — the arc. Minutes of sustained play (present, lamp actually
    // up) warm the instrument toward 1 over ~7 minutes; absence cools it in
    // ~90s. Timbre accumulates — events never do — so minute nine sounds
    // played-in without the scene getting busier.
    const playing = s.pres * clamp((s.lamp - 0.3) / 0.45);
    s.temper = clamp(s.temper + playing * dt / 420 - (1 - s.pres) * dt / 90);
  ...
  draw(P, g, w, h, t, inp) {
    ...
    g.fillText('SPOTS ' + s.openN + '/' + s.ap.length + '   LAMP ' + Math.round(s.lamp * 100) +
      ...
      '   TEMPER ' + Math.round(s.temper * 100) +
  ...
  audio(A, P) {
    ...
    // MIDI is gated on presence (midiOK, set per tick): the browser side is
    // silenced by gate already, but the mirror used to keep sending — idle
    // lamp breathing was striking pad notes on the rig all night, and notes
    // whose burst shut lingered held in Live. Now: no presence, no MIDI.
    let midiOK = false;
    const setV = (vc, freq, glide, vel) => {
      ...
      if (midiOK && typeof MOut !== 'undefined') {
    ...
    /* --- the pedal: root always; a fifth above it is EARNED by temper --- */
    const sub = A.padVoices(v, 2, { type: 'triangle', gain: 0.0001, cutoff: 220, q: 0.5 });
    const retune = glide => {
      sub[0].set(H.rootFreq(-2), glide);
      sub[1].set(H.rootFreq(-2) * 1.5, glide);   // the fifth — A under every chord, all four are D-pedal
    ...
    let prevLamp = null, lampVel = 0, lampFall = 0;
    let nextBreath = 0, breath = 0, breathFall = 1.2;
    let closes = 0, tollCool = 0;   // the closing toll: recent-shut counter + cooldown
    ...
    return {
      tick(inp, dt) {
        ...
        const rawSigned = prevLamp === null ? 0 : (s.lamp - prevLamp) / Math.max(dt, 0.008);
        prevLamp = s.lamp;
        // attack-instant, slow release, both directions: a stab's FIRST
        // commits must already carry its full weight — smoothing both ways
        // made stabs print soft. lampFall is the same idea for the retreat.
        const rawLv = Math.max(0, rawSigned), rawLf = Math.max(0, -rawSigned);
        lampVel = rawLv > lampVel ? rawLv : lampVel + (rawLv - lampVel) * Math.min(1, dt * 3);
        lampFall = rawLf > lampFall ? rawLf : lampFall + (rawLf - lampFall) * Math.min(1, dt * 3);
        // temper-earned voicing: how wide the room is, right now
        const panW = 0.62 + s.temper * 0.28;
        ...
        while ((ev = s.evq.shift()) && i < 6) {
          i++;
          const p = ev.p;
          if (!ev.on) closes += 1;    // every shutting aperture counts, voiced or not
          ...
          if (ev.on) {
            ...
            if (gate > 0.15 && now - vc.lastBell > 0.6) {
              ...
              const px = clamp(p.x / P.w * 2 - 1, -1, 1) * panW;
              A.bell(H.chordTone(p.deg + 3, p.oct + 1),
                { at, vol: (0.014 + push * 0.03) * gate, dur: 2.6, pan: px, rev: 0.8 });
              // temper-earned shimmer: a played-in organ doubles some bells
              // an octave up, faint — timbre accumulating, not more events
              if (s.temper > 0.35 && P.rand() < s.temper * 0.6) {
                A.bell(H.chordTone(p.deg + 3, p.oct + 2),
                  { at: at + 0.02, vol: (0.005 + push * 0.011) * gate * s.temper, dur: 3, pan: px, rev: 0.9 });
              }
            }
          }
        ...
        /* ---- the film closing is a statement: one low toll ------------- */
        closes *= Math.exp(-dt / 0.5);
        // several apertures shutting close together, with hands still here,
        // is a RETREAT — mark it once. Idle flicker never fires this: the
        // presence gate keeps an abandoned scene silent.
        if (closes > 2.5 && now > tollCool && gate > 0.3) {
          tollCool = now + 2.5;
          closes = 0;
          const pull = clamp(lampFall * 1.2);
          A.bell(H.chordTone(0, -1),
            { at: T.next(0.25), vol: (0.016 + pull * 0.028) * gate, dur: 3.6, rev: 0.9 });
        }
        ...
        for (const vc of voices) {
          ...
          if (p && vc.pan) A.set(vc.pan.pan, clamp(p.x / P.w * 2 - 1, -1, 1) * panW, 0.07);
          A.set(vc.f.frequency, 480 + s.lamp * 1000 + whip * 900, 0.2);
          A.set(vc.o2.detune, 4 + whip * 20, 0.15);
          // the mirror is CRISP: note-off the moment the aperture shuts or
          // presence leaves (Live's own release is the tail) — waiting for
          // the browser fade left stale notes held on the rig whenever the
          // idle lamp re-lit a burst mid-fade
          const onNow = !!(p && p.on);
          if (vc.note >= 0 && (!onNow || !midiOK)) {
            if (typeof MOut !== 'undefined' && MOut.wants() && MOut.port) {
              try { MOut.port.send([0x80 | (MOut.chFor('pad') - 1), vc.note, 0]); } catch (e) {}
            }
            vc.note = -1;
          }
          // presence (re)arrived with this burst already lit → re-strike at
          // the weight it was played at
          if (onNow && midiOK && vc.note < 0) {
            setV(vc, H.chordTone(p.deg || 0, p.oct || 0), 0.08, Math.round((vc.vel || 44) * 0.9));
          }
          if (p && !p.on && env < 0.05) vc.burst = null;
        }
        ...
        // the earned fifth: silent until the organ is played-in, never loud
        sub[1].level(0.0001 + s.temper * 0.006 * (0.4 + 0.6 * gate), 0.6);
