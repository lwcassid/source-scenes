/* ---------- SRC-43.7 · CELL FRONT V7 (reforming) ----------
   Nima's verdict on V6: love the size reaction, but when the audio CHANGES
   the material should look like it's reforming — not just the same drifting
   texture scaled up or down. V6's cell drift ran at one speed set by how
   LOUD things are (uEnergy); a sudden swing from quiet to loud (or the
   reverse) grew/shrank the pockets but the internal texture kept ambling
   along at its usual pace, so the change itself wasn't LEGIBLE as motion.
   V7 adds FLUX: the scene now tracks how fast bass/mid/treble are actually
   MOVING (not just their level), and spends that on a burst of extra drift
   speed and turbulence exactly when they're moving — a genuine reform, cells
   sliding and reshuffling into new positions — while a held, unchanging
   level (loud or quiet) settles back to V6's calm ambient drift. Everything
   else is unchanged from V6: size as the headline reaction, three pockets on
   their own band, hands as sensitivity gain, true black, no shadow ring.
   ------ */
const CF7_LOBES = s => [
  [-s.offBass, -0.04, s.Rbass], [0, 0.05, s.Rmid], [s.offTreble, -0.04, s.Rtreble]
];
const CF7_POOL = (s, x, y) => {
  let acc = 0;
  for (const l of CF7_LOBES(s)) {
...
const CF7_FS = [
  ...
  'uniform float uT, uPres, uEnergy, uFlux, uBass, uMid, uTreble, uU;',
  'uniform vec2 uRes;',
  ...
  // FLUX: how fast the audio is actually changing right now, not how loud
  // it is. Spent on extra drift speed AND extra swirl amplitude, so a
  // transition genuinely reshuffles the cell field instead of just resizing
  // it — the "reforming" Nima asked for. Settles back to V6's calm ambient
  // drift the moment the level holds steady, loud or quiet.
  ...
  // individual cells swell/shrink with overall intensity, PLUS a momentary
  // pop from flux itself — the material visibly churns as it reforms.
  '  float cellSc = 0.68 + 0.60 * uEnergy + 0.22 * uFlux;',
...
reg({
  id: 'SRC-43.7', family: 'SRC-43', ver: 7, title: 'Cell Front V7', tech: 'VOLUMETRIC CELL FIELD / REFORMING AUDIO POCKETS',
  audioIn: true,
  fx: { bloom: 0.34 },
  tags: ['VOLUMETRIC', 'AUDIO IN', 'THREE POCKETS', 'HANDS = SENSITIVITY', 'SIZE REACTIVE', 'REFORMING'],
  desc: 'Same three glowing pockets as V6 — BASS, MID and TREBLE, sized by their own band, ember-orange / warm gold / cool violet, floating in true black. Here a CHANGE in the sound is its own event: the moment a band swings up or down, the material inside the pocket visibly reforms — cells sliding, swirling and reshuffling into a new arrangement — instead of just the same lazy drift scaled bigger or smaller. Hold a level, loud or quiet, and the field settles back to a calm ambient drift; the churn is reserved for the transition itself.',
  interact: 'This scene listens (SHOW CHECK → AUDIO IN, or MAP → Audio in) — the mic or a captured app is what the pockets respond to and reform to. Hands still don’t fight the audio for control: L turns up how sensitive the BASS pocket is, R turns up TREBLE’s — reach out to make quiet sound read as loud, pull back to calm a loud room down. At rest the pockets sit small enough to nearly vanish, so real intensity — and real change — has somewhere dramatic to go.',
  sound: 'Makes no sound of its own — an audio-in scene, same as V4/V5/V6. Connect a source (mic, line-in, or CAPTURE APP AUDIO for a running app’s own output) in MAP → Audio in, then SET REST with the room quiet so silence reads as silence.',
  ...
  init(P) {
    ...
    const uni = {
      ...
      uPres: { value: 0 }, uEnergy: { value: 0 }, uFlux: { value: 0 },
  ...
  step(P, dt, t, inp) {
    ...
    // SENSITIVITY, not competition (Nima's call, carried over from V5/V6).
    ...
    // V6's call: the idle floor was doing too much — quiet should read as
    // nearly hidden, not a third-size resting mass. Keep only a whisper.
    ...
    // FLUX (Nima's V7 call): how fast bass/mid/treble are actually MOVING,
    // not how loud they are. A held level — loud or quiet — decays this back
    // to zero; a swing in either direction spikes it. Fast rise so a hit
    // registers as reform almost at once, slower fall so the churn reads as
    // a genuine settle rather than a flicker.
    ...
    const fluxTarget = clamp(fluxRaw * 0.30);
    s.flux += (fluxTarget - s.flux) * Math.min(1, dt * (fluxTarget > s.flux ? 11.0 : 2.4));

    const axc = clamp(s.ax, 0.6, 1.7);
    s.offBass = (0.10 + 0.40 * s.bass) * axc;
    s.offTreble = (0.10 + 0.40 * s.treble) * axc;
    // size IS the reaction: near-invisible at rest, ballooning at peak —
    // roughly 40x the resting footprint at full intensity.
    ...
    /* ---- blooms: onset-driven only, same shape as V4/V5/V6. A hit spawns
       one sized by how loud it was AND by the current hand sensitivity, and
    ...
    if (onsetEdge) {
      if (s.blooms.length < 6) {
        ...
        for (let k = 0; k < 12 && !ok; k++) {
          ...
          if (CF7_POOL(s, bx, by) > 0.4) ok = true;
        }
        if (ok) {
    ...
    for (let i = s.blooms.length - 1; i >= 0; i--) {
      const b = s.blooms[i];
      const inPaint = CF7_POOL(s, b.x, b.y) > 0.4;
  ...
  draw(P, g, w, h, t, inp) {
    ...
    if (s.noGL || !P._three) {
      ...
      g.fillText('CELL FRONT V7 · open on the hosted site (WebGL)', 10, h - 10);
      return;
    }
    const T3 = P._three, u = T3.uni;
    u.uT.value = s.life; u.uPres.value = s.pres; u.uEnergy.value = s.energy; u.uFlux.value = s.flux;
    u.uBass.value = s.bass; u.uMid.value = s.mid; u.uTreble.value = s.treble;
    u.uU.value = s.U;
    const ll = u.uLobe.value, LB = CF7_LOBES(s);
