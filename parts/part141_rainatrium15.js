/* ---------- SRC-16.15 · RAIN ATRIUM V15 (lightning in two acts) ----------
...
reg({
  id: 'SRC-16.15', family: 'SRC-16', ver: 15, title: 'Rain Atrium V15', tech: 'TWO-FIELD WATER / SUMMONED BAND',
  music: {
  ...
  step(P, dt, t, inp) {
    ...
    if (s.charge >= 1 && !s.primed) {
      ...
      if (typeof MOut !== 'undefined') MOut.sfxNote(37, 0.7, 4);
    }
    if (s.primed) {
      s.strikeTimer += dt;
  ...
  audio(A, P) {
    ...
    v.fadeIn(1, 1.2);
    let nextT = 0, lastIdx = -1, sfxHeld = false;
    return {
      tick(inp) {
        ...
        if (typeof MOut !== 'undefined') {
          ...
          if (!sfxHeld) { sfxHeld = true; MOut.evNote('sfx', 78.7, 0.2, 0, 3600); MOut.evNote('sfx', 83.4, 0.22, 0, 3600); }
        }
        ...
        // TWO KEYS to the band (Lance): the master code opens the full
        // ~45s window; the PUBLIC key is deep closeness — press right into
        // the source with both hands and the groove FADES in and out with
        // that commitment. Continuous gates only, nothing abrupt.
        const sum = (typeof SUMMON !== 'undefined' && SUMMON.active) ? 1 : 0;
        const deep = clamp((wet - 0.7) / 0.22);
        const gate = Math.max(sum ? Math.max(0.3, clamp((wet - 0.38) / 0.35)) : 0, deep);
