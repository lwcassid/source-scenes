...
const LUMEN25_STOPCACHE = new Map();
reg({
  id: 'SRC-38.25', family: 'SRC-38', ver: 25, title: 'Lumen Film V25', tech: 'GEM FILM / APERTURE ORGAN',
  music: {
  ...
  step(P, dt, t, inp) {
    ...
    s.openN = open;
    // THE SPARKLE (V25): each rest breath glints a stone (three on the
    // deep toll) -- the sound visibly moves the picture
    if (s.sparkle > 0) {
      for (let k2 = 0; k2 < (s.sparkN || 1); k2++) {
        const sp = s.ap[((s.sparkSeed || 0) + k2 * 7) % s.ap.length];
        // glint is its OWN brightness lane -- the telegraph's 0.35 leak
        // reads as a dim blob at rest, and a tease that cannot be seen
        // is no tease (the choir lesson, applied to light)
        if (sp) { sp.lit = Math.max(sp.lit, s.sparkle); sp.glint = s.sparkle; }
      }
      s.sparkle = Math.max(0, s.sparkle - dt / 2.5);
    }
  ...
  draw(P, g, w, h, t, inp) {
    ...
    const addSvgStops = (grad, a) => {
      let set = LUMEN25_STOPCACHE.get(a);
      if (!set) {
        set = SVGSTOPS.map(([t, rgb]) => [t, `rgba(${rgb},${a})`]);
        LUMEN25_STOPCACHE.set(a, set);
      }
    ...
    for (const p of s.ap) {
      const a = Math.max(p.lit * 0.35, (p.glint || 0) * 0.7, p.open);
      if (a < 0.01) continue;
      ...
      tick(inp, dt) {
        ...
        if (s.pres < 0.25) {
          if (now >= nextBreath) {
            ...
            if (deep) {
              // the walk-toward payoff: one deep glass toll over a swelled bass
              A.bell(H.chordTone(0, 0), { at: now + 0.15, vol: 0.05, dur: 6, rev: 0.85 });
              A.tone(H.rootFreq(-1), { at: now + 0.35, vol: 0.05, dur: 4, attack: 1.4, type: 'triangle', rev: 0.5, role: 'bass' });
              brSwell = { t0: now, up: 1.4, dn: 4 };
              s.sparkle = 1; s.sparkN = 3; s.sparkSeed = (Math.random() * 997) | 0;
            } else if (pick < 0.5) {
              ...
              s.sparkle = 1; s.sparkN = 1; s.sparkSeed = (Math.random() * 997) | 0;
            } else if (pick < 0.7) {
