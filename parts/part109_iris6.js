/* ---------- SRC-37.6 · IRIS ENGINE V6 (the morph goes all the way to sharp) ---------- */
reg({
  id: 'SRC-37.6', family: 'SRC-37', ver: 6, title: 'Iris Engine V6', tech: 'GARAGE LADDER / MORPH = TEETH',
  music: {
  ...
  fx: { bloom: 0.6 },
  tags: ['LEFT HAND = THE BREAKBEAT', 'RIGHT HAND = THE CHORD, THE COLOUR, AND THE TEETH', 'THE TUNNEL ANSWERS THE DRUMS', 'G LYDIAN AT 132'],
  desc: 'A tunnel of fat bands boiling out of one eye, in one colour at a time — a gradient running from the middle of the frame outward, pale and hot at the eye, deepening as it travels. The rings still walk a ladder of six lobed shapes, but now the reach itself has teeth: near rest the bands are soft, round, barely rippled circles, and the further the right hand reaches, the more the ripple bites — by the top of its throw every ring has curdled into a hard, pointed gear of spikes. The left hand runs the same six-step drum ladder as before, wired straight to the picture — kick shoves the tunnel outward, snare lights the walls, hats put a shimmer on the halos.',
  interact: 'L = THE BEAT, in six steps. Drawn in there is no kit at all, just the drone and a band every two beats. Lift and a two-step skeleton arrives (kick, snare, off-beat sub); lift again and it shuffles — swung sixteenth hats, ghost snares, the bassline landing on the ands; keep going and the kit breaks apart into a chopped break, and at the top it is a full break with sixteenth rolls at the end of the bar. Every step up also drives the tunnel harder: more bands per beat, and every drum in the pattern is visible — a kick is a thump that pushes the whole tunnel outward, a snare throws light onto the left and right walls, hats sparkle in the halos. R = MORPH: the shape of the rings (2 to 7 lobes), the chord, and the colour of the entire room, crossfading over about a second — AND how hard that shape bites. Near rest the lobes are a gentle wave; reach all the way out and the same lobes turn into sharp, pointed teeth, gear-like and unmistakable. Left hand for the groove, right hand for the harmony and the bite, and the frame stays lit as long as either hand is off its rest.',
  ...
  step(P, dt, t, inp) {
    ...
    // ---- R: morph. Six shapes; each owns a chord AND a gradient. The
    // reach itself also owns the BITE — how sharp the lobes get (see draw).
  ...
  draw(P, g, w, h, t, inp) {
    ...
    // ---- THE BITE: how far the right hand reaches decides how sharp the
    // lobes go. Near rest the modulation is a soft, low-amplitude sine — a
    // gentle wave on an almost-circle. At full reach it curdles into a
    // triangle wave (a real corner at every peak, not a rounded crest) at
    // much higher amplitude, so the same lobe count reads as a hard-edged
    // gear of teeth instead of a wobble. asin(sin(x))*2/pi is the standard
    // closed-form triangle wave — cheap, and it shares zero-crossings with
    // sin so the blend never pops.
    const bite = Math.pow(s.warp, 1.6);
    const wave = ang => {
      const sn = Math.sin(ang);
      if (bite < 0.004) return sn;
      const tri = Math.asin(Math.max(-1, Math.min(1, sn))) * (2 / Math.PI);
      return sn + (tri - sn) * bite;
    };
    ...
    for (let k = NR - 1; k >= 0; k--) {
      ...
      // amplitude climbs hard with reach — 0.015 at rest, up past a third
      // of the ring's own radius at full stretch, so the teeth actually bite
      const amp = r0 * (0.015 + s.warp * 0.34) * Math.pow(u, 0.7);
      ...
      for (let pass = 0; pass < 2; pass++) {
        ...
        for (let i = 0; i <= SEG; i++) {
          ...
          const rr = r0 + wave(an * lobes + s.spin * 1.7 + k * 0.6) * amp
            + wave(an * (lobes + 2) - s.spin * 1.1 + k * 0.31) * amp * 0.4 + jit;
