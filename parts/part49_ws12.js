/* ---------- SRC-10.12 · WEATHER STATION V12 (one form, one wind) ---------- */
reg({
  id: 'SRC-10.12', family: 'SRC-10', ver: 13,
  title: 'Weather Station V12', tech: 'ONE POLE / SHARED RINGS / LEANING WIND',
  music: { bpm: 60, root: 43, mode: 'aeolian', prog: [0, 5], chordBars: 4 },
  fx: { bloom: 0.7, edge: true },
  tags: ['ONE POLE', 'SHARED CONCENTRIC RINGS', 'HEADING LEANS THE ORBIT', 'WARM AT PEAK CHAOS'],
  desc: 'One pole now, not several — a single grand force the whole field answers to. Four out of five threads settle onto one of a small handful of shared rings around it, tracing nearly the same path as their neighbors, so the mass reads as one object wrapped in layers, not a cloud of individuals. The fifth thread keeps its own wobble, fraying the edges just enough to keep it alive. And the wind never lets go: heading leans the whole orbit downwind, so turning your left hand visibly tips the entire form, even at the peak of the gale.',
  interact: 'L = heading — always visible now, even inside the weave: turn your hand and the whole wrapped form leans with it. R = the pole: small at rest, ballooning hard as you raise the hand, until at full reach it owns the sky and the shared rings pull in nearly everything. Low R is still just a scatter of tiny drifting marks.',
  ...
  init(P) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      mass.push({
        ...
        conform, orbitFrac, orbitDir: P.rand() < 0.5 ? 1 : -1,
    ...
    const pole = {
      ...
      wander: P.rand() * TAU, wSpeed: 10 + P.rand() * 18,
      wobPh: P.rand() * TAU, wobFr: 0.07 + P.rand() * 0.12, wobAmp: 0.12 + P.rand() * 0.14
    };
  ...
  step(P, dt, t, inp) {
    ...
    // one pole, drifting gently, leaning downwind as it grows
    pole.wobPh += dt * pole.wobFr;
    pole.r = pole.baseR * (1 + growT * 6.5);
    const driftAng = heading * 0.75 + pole.wander * 0.25 + Math.sin(pole.wobPh) * pole.wobAmp;
    pole.x += Math.cos(driftAng) * pole.wSpeed * dt;
    pole.y += Math.sin(driftAng) * pole.wSpeed * dt;
    if (pole.x < w * 0.15) pole.x = w * 0.15; if (pole.x > w * 0.85) pole.x = w * 0.85;
    if (pole.y < h * 0.15) pole.y = h * 0.15; if (pole.y > h * 0.85) pole.y = h * 0.85;
    ...
    for (const m of s.mass) {
      const dx = pole.x - m.x, dy = pole.y - m.y, d = Math.sqrt(dx * dx + dy * dy) || 1;
      const boost = d < pole.r ? clamp(1 - d / pole.r) : 0;
      m.boost = boost;
      ...
      if (boost > 0.01) {
        ...
        const tanx = -ny * m.orbitDir, tany = nx * m.orbitDir;
        // the orbit direction leans toward the wind, so the whole ring visibly tips as heading changes
        let dirx = tanx * (1 - leanBlend) + windx * leanBlend;
        let diry = tany * (1 - leanBlend) + windy * leanBlend;
        const dl = Math.hypot(dirx, diry) || 1; dirx /= dl; diry /= dl;
        const orbitR = pole.r * m.orbitFrac;
        const radialTerm = (d - orbitR) * 1.1;
        const orbSpeed = spd0 * (0.65 + boost * 0.85);
        const orbVx = dirx * orbSpeed + nx * radialTerm;
        const orbVy = diry * orbSpeed + ny * radialTerm;
