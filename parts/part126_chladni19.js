/* ---------- SRC-28.19 · CHLADNI COURT V19 (starfield idle · bass breath · the rail secret) ---------- */
/* Lance on V18: the high sine pair still droned at idle (grating) - the
   plate tones kept a constant floor. V19:
   - HIGHS DIE AT REST: plate-tone floor rides presence (zero idle), and
     the mid bed only really exists under hands.
   - STARFIELD IDLE: the resting voice is the SAND - a soft noise-wash
     slowly panning with the drifting stream.
   - BASS BREATH: the periodic idle swell is now a low event (sub + low
     pair) - low frequencies carry across the playa and pull people in.
   - THE RAIL SECRET: hold either hand at its extreme for 5s - the sand
     bunches tighter and goes BLUE - and beats unlock for 45s: a broken
     half-time groove (kick / cracked backbeat / ghost hats / bass on the
     kicks) under the still-playable plate. Re-hold to relight. Nothing
     announces it; the charge readout appears only once you are doing it. */
reg({
  id: 'SRC-28.19', family: 'SRC-28', ver: 19, title: 'Chladni Court', tech: 'STANDING WAVES / SAND ON A PLATE',
  music: {
  ...
  tags: ['CYMATICS', 'EARNED RESONANCE', 'SAND STARFIELD', 'BASS BREATH', 'THE RAIL SECRET'],
  desc: 'The court goes quiet for real. At rest there is no drone at all: the sound is the SAND - a soft starfield wash drifting slowly across the stereo field with the stream - and every while a low BASS breath swells and recedes, the kind of sound that reaches across the playa and turns heads. The plate tones, the bed, the lines: all of it exists only under hands. And for those who know: hold one hand at its extreme - all the way close or all the way far - and keep holding. The sand bunches tighter, turns blue, and the plate unlocks a broken beat for 45 seconds, yours to jam over while the clock runs. Re-hold to relight it. Nobody is told; some will find it.',
  ...
  sound: 'Idle: NO tones - a panning sand-noise wash (the starfield) plus a periodic bass breath (sub + low pair, the far-field attractor). Under hands: modal plate tones with beat partners throbbing toward stillness, bass under the lower locked mode, JUST-ratio shimmer, rolled velocity-earned lock blooms. THE UNLOCK: rail-hold 5s charges (sand noise brightens with it), then a 45s broken groove at the 116 feel - kick pattern with bass doubling, cracked backbeat at 1900Hz, ghost hats - fading over its last 4s, ducking if the player leaves. MIDI: drums auto-bucket to ch10, perc CC74 = charge/beat state, tones on texture, blooms ch5, resonance lead CC74, sand sfx CC74.',
  init(P) {
    ...
    P.state = { grains, n, res: 0, wasLocked: false, motion: 0, lockT: -9, found: {}, appr: 0, resPrev: 0, pres: 0, tNow: 0, teaseT: 6, teaseUntil: -9, tease: 0, surgeUntil: -9, charge: 0, beatUntil: -9, beatOn: false, unlockCol: 0 };
  },
  step(P, dt, t, inp) {
    const s = P.state;
    s.tNow = t;
    ...
    const colT = s.beatOn ? 1 : s.charge;
    s.unlockCol += (colT - s.unlockCol) * Math.min(1, dt * 2);
    ...
    const drive = s.pres * surge * (1 + s.charge * 1.4);
    ...
    const jit = dt * ((0.05 * (1 - s.res) + 0.03) * (0.15 + 0.85 * s.pres) * (1 - s.charge * 0.6) + tease * 0.035);
  ...
  draw(P, g, w, h, t, inp) {
    ...
    const uc = s.unlockCol || 0;
    const cb0 = 130 + warm * 55;
    tg.fillStyle = `rgba(${(240 - 130 * uc) | 0},${((205 + warm * 25) - 20 * uc) | 0},${(cb0 + (255 - cb0) * uc) | 0},${0.58 + warm * 0.32})`;
  ...
  audio(A, P) {
    ...
    return {
      tick() {
        ...
        // the tones exist only under hands — no high drone can grate at rest
        const tgtN = (0.004 + 0.011 * pres + r2 * 0.09) * gate, tgtM = tgtN;
        ...
        A.set(subG.gain, (0.022 + r2 * 0.012) * amb, 0.6);
        A.set(sub2G.gain, (0.011 + r2 * 0.006) * amb, 0.6);
        // STARFIELD: at rest the sand itself is the voice — a soft wash
        // drifting in stereo with the stream
        const wash = (1 - pres) * (0.011 + 0.006 * Math.sin((s.tNow || 0) * 0.11));
