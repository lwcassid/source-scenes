/* ---------- SRC-13.11 · EVENT HORIZON V11 — the warp is the window ---------- */
/* Lance on V10's window: visually not different enough, not an instrument,
   drums too slow. V11: THE OTHER SIDE is a different WORLD — the sky washes
   deep-space indigo and the ejecta becomes a WARP FIELD (per-star depth,
   long blue-white streaks) rushing past while the line keeps burning ember.
   The warp is PLAYED: arm height = warp speed, the kick pulses the starfield
   and the line's glow, flicks still write runs. The pocket runs DOUBLE-TIME
   (128-feel four-on-the-floor on the 16th grid, crack backbeat, hats ranked
   by tension, bass 8ths always on inside the window) — dance tempo even
   though the transport holds 64. */
reg({
  id: 'SRC-13.11', family: 'SRC-13', ver: 11, title: 'Event Horizon', tech: 'VERLET ROPE / ACCRETION',
  music: {
  ...
  desc: 'The cable is the event horizon: one burning line between the sky and the void. Stars drift down, accelerate, and are eaten; nothing below the line ever comes back. Raise your arms and an organ-dark string section assembles CHAIR BY CHAIR, the whole wall swelling under your hands. Hold both arms high and the void RISES and swallows the room — and gives back a different sky: THE OTHER SIDE, where space itself washes deep indigo and the stars become a warp field rushing past you, faster the higher you hold the wall, pulsing with a driving beat for most of a minute. Each swallow leaves a scar — fewer stars, a darker, heavier wall, forever.',
  ...
  draw(P, g, w, h, t, inp) {
    ...
    // THE OTHER SIDE is a different WORLD: the space itself washes deep
    // indigo while the window is open (the line and rim stay ember — you
    // stand at the burning horizon while cold space rushes past)
    const jw = s.jam;
    g.fillStyle = `rgba(${4 + Math.round(3 * jw)},${3 + Math.round(5 * jw)},${7 + Math.round(16 * jw)},0.45)`;
    g.fillRect(0, 0, w, h);
    ...
    for (const st of s.stars) {
      ...
      // warm ember sky normally; cold blue-violet in the warp
      const r = lerp(200 + sp * 55, 140 + st.z * 40, jw);
      const gg = lerp(205 - sp * 60, 185, jw);
      const bb = lerp(255 - sp * 120, 255, jw);
    ...
    return {
      tick(inp) {
        ...
        while (nextT < horizon2 && guard++ < 24) {
          ...
          if ((s.tension > 0.5 || s.jam > 0.15) && st % 2 === 0 && duck > 0.4) {
            const acc = st === 0 ? 1.35 : st % 4 === 0 ? 1.1 : 1;
            A.bassNote(H.chordTone(0, -1), {
              at: nextT, vol: (0.018 + Math.max(0, s.tension - 0.5) * 0.05 + s.jam * 0.02) * acc * gate * duck, dur: 0.24
            });
          }
