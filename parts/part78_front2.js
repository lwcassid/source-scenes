/* ---------- SRC-43.2 · CELL FRONT V2 (the film, not the mosaic) ----------
   V1 got the colour idea right and the MATERIAL wrong. It drew a weighted
   Voronoi: cells meeting along straight walls at angular junctions, every
   site with a birth/death phase so the whole field blinked. A pour does
   neither, and — the thing that took the longest to see — a pour is NOT a
   tessellation. Look at one closely and it is islands in a ground: rounded
   blobs of the under-paint sitting in a slate film, each in its own black
   shadow, with a bright rim, an inner ring and often a pupil, and pairs that
   landed close run together into peanuts and clovers. Between them the ground
   is a material in its own right, speckled with pinheads too small to have
   rings. Tessellate that and you get stained glass, which is what the first
   three passes of this file were.
   So: discs on three jittered lattices, radius skewed hard toward small by a
   cube so a few fat cells sit in a crowd of tiny ones, unioned with a SMOOTH
   min (sparse, so it fuses neighbours instead of flattening the canvas).
   Edges are a constant width in world units — a fat cell gets the same
   hairline as a pinhead — while the rings inside are measured in units of the
   cell's own radius, so both carry the same ring count.
   Nothing has a lifecycle. Cells do not appear or vanish; the film drifts,
   shears and stretches with the liquid under it, and that is all the motion
   there is. Colour is still sampled ONCE at the owning cell. ---------- */
const CF2_LOBES = s => [
  [-s.offW, -0.05, s.Rw], [0, 0.02, s.Rm], [s.offC, 0.05, s.Rc]
];
const CF2_POOL = (s, x, y) => {
  let acc = 0;
  for (const l of CF2_LOBES(s)) {
...
const CF2_FS = [
  ...
  'void lay(vec2 p, float U, float salt, float rmin, float rspan, float wgt, float km,',
  ...
  // no birth, no death: a cell that has broken through STAYS. It only breathes.
  '    float R = U * (rmin + rspan * h.z * h.z * h.z) * wgt * (1.0 + 0.07 * sin(uT * 0.09 + h.x * 6.283));',
  '    float d = length(q - site) - R;',
  '    dS = smin(dS, d, km);',
  '    if (d < dH){ dH = d; SP = site - off; SH = h; SR = R; }',
  '  }}',
  '}',
  'void main(){',
  '  vec2 p = (gl_FragCoord.xy - 0.5 * uRes) / min(uRes.x, uRes.y);',
  '  vec2 pa = p + flowW(p * 2.1) * (0.013 + uHeat * 0.019);',
  '  float acc = poolF(pa);',
  ...
  '  lay(pa, U0, 1.0,  0.15, 0.44, w0, U0 * 0.22, dS, dH, SP, SH, SR);',
  '  lay(pa, U1, 17.0, 0.20, 0.58, w1, U1 * 0.22, dS, dH, SP, SH, SR);',
  '  lay(pa, U2, 31.0, 0.24, 0.54, w2, U2 * 0.24, dS, dH, SP, SH, SR);',
  ...
  '  vec2 gd = floor((pa + dof) / Ud);',
  '  vec3 hd = rnd3(gd.x, gd.y, 53.0);',
  '  vec2 sd = (gd + vec2(0.5) + (hd.xy - 0.5) * 0.9) * Ud - dof;',
...
reg({
  id: 'SRC-43.2', family: 'SRC-43', ver: 2, title: 'Cell Front V2', tech: 'FUSED CELL FIELD / COLOUR AT CELL RESOLUTION',
  music: {
  ...
  fx: { bloom: 0.30 },
  tags: ['POURED PAINT', 'FUSED CELLS', 'RINGS', 'TWO BANKS MEET'],
  desc: 'A macro shot of poured paint at the moment the heavy top film has split and let the colours underneath through. The thing that makes a pour look like a pour is that it is not a tessellation: it is islands in a ground. Rounded blobs of the under-paint sit in a slate film, each in its own black shadow, each with a bright rim and an inner ring and often a pupil; pairs that land close run together into peanuts and clovers; and the ground between them is a material in its own right, speckled with pinheads too small to carry rings. A few fat cells sit in a crowd of tiny ones — the size range is most of the texture. Edges are a hairline whatever the cell\'s size, rings are measured in units of its own radius, so a pinhead and a thumbprint read the same. Nothing here has a lifecycle: cells never blink in or out, the whole film simply drifts and shears with the liquid under it. Colour is sampled once per CELL, never per pixel, so two paints meeting make a mosaic of flat colours and never a gradient.',
  ...
  step(P, dt, t, inp) {
    ...
    while (s.spawnAcc >= 1 && s.blooms.length < 6) {
      ...
      for (let k = 0; k < 12 && !ok; k++) {
        ...
        if (CF2_POOL(s, bx, by) > 0.62) ok = true;
      }
      if (!ok) break;
      const big = P.rand();
    ...
    for (let i = s.blooms.length - 1; i >= 0; i--) {
      const b = s.blooms[i];
      const inPaint = CF2_POOL(s, b.x, b.y) > 0.44;
  ...
  draw(P, g, w, h, t, inp) {
    ...
    const ll = u.uLobe.value, LB = CF2_LOBES(s);
