/* ---------- SRC-09.10 · ATTRACTOR VESPERS V10 (the beam learns to sing in tune) ---------- */
reg({
  id: 'SRC-09.10', family: 'SRC-09', ver: 10, title: 'Attractor Vespers V10', tech: 'CLIFFORD MAP / FLAME DENSITY',
  music: {
  ...
  tags: ['STRANGE ATTRACTOR', 'FLAME DENSITY', 'LIGHT = SOUND', 'IN TUNE'],
  desc: 'V9 with the beam FIXED. The old spring math parked the voice between chord tones — a permanently sour quarter-tone against the pedal — and it sat an octave too high, saw-first, blasting. Now it walks the ladder rung by rung: every landing is EXACTLY a chord tone, the portamento lives only in the glide between them, and the voice dropped an octave into a warm triangle-led register that sits inside the hum instead of on top of it. Consonant by default; the Annihilation weight stays in the maw where it belongs. Everything else as V9: the dreaming idle, the spurs, the churn, the heat.',
  ...
  sound: 'V9\'s engine with the beam RETUNED. The beam now steps the chord ladder one rung at a time toward the light\'s centroid (at most ~11 rungs/s), gliding ~90ms between rungs — so it bends audibly in transit but every landing is exactly a chord tone: consonant by construction, never parked between pitches. It lives an octave lower (G3–G5), triangle-led with the saw tucked underneath at a third of the level, filter capped near 1.7kHz with the light\'s height tilting it instead of jumping octaves. Level curve softened so it swells rather than blasts. The resonant airs\' Q ceiling roughly halved — filament states shimmer instead of whistling. Companion beam (heat-earned) harmonizes two rungs below on the same rules. All of V9 stands: the maw on bass ch3 (the deep Annihilation weight lives THERE), the churn on arp CC74, the dreaming idle with randomized spurs, presence-gated heat, the wake on sfx ch11, pad ch2 CC74 = the LIGHT.',
  init(P) {
  ...
  audio(A, P) {
    ...
    bv._noHold = true;
    const mkBeam = () => {
      // triangle-led, saw tucked a third underneath: warm, sits IN the hum
      const o1 = bv.osc('triangle', 220), o2 = bv.osc('sawtooth', 220.6);
      const sawG = bv.g(0.33);
    ...
    let brSlow = 0, mawT = -9, mawBend = false, mawLvl = 0, rung = 3, rungT = 0;
    return {
      tick(inp, dt) {
        const s = P.state;
        ...
        // the light's HEIGHT tilts the filter instead of jumping octaves
        A.set(beam.f.frequency, Math.min(1700, 380 + br * 1000 + (1 - s.cenY) * 450 + bloom * 350), 0.15);
        // during a spur the beam whispers one quiet note to itself
        const bl = (s.pres * (0.003 + Math.pow(br, 1.3) * 0.017) + s.spur * 0.005 * (0.3 + br)) * clamp(s.occ / 0.004) * (1 + bloom * 0.3);
        ...
        A.set(beam2.f.frequency, Math.min(1300, 280 + br * 900), 0.2);
