/* ---------- SRC-28.3 · CHLADNI COURT V3 ---------- */
/* Triptych: one plate, mirrored across three edge-to-edge squares that fill
   the wall. No plate outline. Grains are bigger and never thin out — the
   nodal lines render as bold bands. A slow phase drift lets the pattern sweep
   toward a corner and breathe back to center. The mode-lock hunt (both hands
   on whole numbers → bell + brighten + swell) is kept intact. */
reg({
  id: 'SRC-28.3', family: 'SRC-28', ver: 3, title: 'Chladni Court', tech: 'STANDING WAVES / SAND ON A PLATE',
  ...
  draw(P, g, w, h, t, inp) {
    ...
    // three edge-to-edge square panels filling the width, mirrored across
    // the center panel so the whole wall reads as one symmetric frieze.
    const m = w / 3;
    const y0 = (h - m) / 2;
    ...
    const sz = Math.max(2.4, m * 0.006) * (1 + warm * 0.9);
    ...
    for (let p = 0; p < 3; p++) {
      ...
      if (warm > 0.45) {
        ...
        g.fillStyle = `rgba(255,235,180,${(warm - 0.45) * 0.2})`;
        for (let i = 0; i < s.n; i += 3) {
          const gx = mx + mir * g2[i * 2];
          g.fillRect(x0 + gx * m + rj * mir - 1, y0 + g2[i * 2 + 1] * m - 1, sz + 2, sz + 2);
        }
        g.globalCompositeOperation = 'source-over';
      }
      // the driver — the source at plate center (subtle, per panel)
      const cx = x0 + m / 2, cy = h / 2;
      const pulse = 0.5 + 0.5 * Math.sin(t * (2 + s.res * 6));
      g.fillStyle = 'rgba(10,10,12,1)';
      g.beginPath(); g.arc(cx, cy, m * 0.03, 0, TAU); g.fill();
      g.strokeStyle = `rgba(200,255,180,${0.2 + pulse * 0.35 * (0.4 + s.res)})`;
      g.lineWidth = 1.5;
      g.beginPath(); g.arc(cx, cy, m * 0.03 + 3 + pulse * 3, 0, TAU); g.stroke();
    }
