/* ---------- SRC-70.3 · LIGAM À TERRA V3 — THE SOIL BREATHES, THE RIGHT ARM TURNS ----------
   Edson, Sep 25, after V2 ("this looks way better"):
   · "give a little bit more life to the particles on the field, some subtle
     random" — every mote of soil now wanders on its own slow path (three
     sines per axis at its own frequencies and phases, so no two move alike
     and none repeat soon), drifts faintly downward with the fall, and
     twinkles at its own slow rate. Still dim, still almost all bokeh.
   · "add a gentle rotation of the camera to the instrument right arm" — the
     RIGHT hand now also turns the camera around the falling roots: nearly
     still at the Source, a slow orbit when the arm is wide (≤ ~0.25 rad/s),
     eased so the turn starts and stops like a body turning, never a motor.
     The right arm opens the current AND turns you with it.
   Everything else is V2.
   ------------------------------------------------------------------
   V2's notes: SRC-70.2 · LIGAM À TERRA V2 — FALLING WITH THE ROOTS
...
(() => {
  ...
  const RIB_VS = GLSL_COMMON + `
    ...
    void main(){
      ...
      // a root that passes close to the lens goes as a ghost, never as a fat seamed tube
      vA = uFade * clamp(base / wpx, 0.04, 1.0) * smoothstep(0.25, 1.0, z) * (1.0 - 0.7 * smoothstep(30.0, 80.0, base));
      vSide = aC.y;
    }`;
  const RIB_FS = `
    varying vec3 vCol; varying float vA; varying float vSide;
  ...
  const PT_VS = GLSL_COMMON + `
    ...
    void main(){
      ...
      if (dust) {
        ...
        // each mote wanders on its own path: three sines per axis, own rates and phases
        float j1 = fract(aJ * 13.7), j2 = fract(aJ * 29.3), j3 = fract(aJ * 47.9);
        vec3 f = vec3(0.11 + 0.23 * j1, 0.09 + 0.19 * j2, 0.13 + 0.21 * j3);
        vec3 wander = vec3(
          sin(uT * f.x + aJ * 40.0) + 0.5 * sin(uT * f.y * 2.3 + j2 * 17.0) + 0.25 * sin(uT * f.z * 5.1 + j3 * 9.0),
          sin(uT * f.y + aJ * 23.0) + 0.5 * sin(uT * f.z * 1.9 + j1 * 11.0) + 0.25 * sin(uT * f.x * 4.7 + j2 * 5.0),
          sin(uT * f.z + aJ * 31.0) + 0.5 * sin(uT * f.x * 2.7 + j3 * 13.0) + 0.25 * sin(uT * f.y * 5.3 + j1 * 7.0));
        p.y -= uT * (0.004 + 0.01 * j2);                 // and settles, slowly, with the fall
        p.y = uCamY + mod(p.y - uCamY + 3.0, 6.0) - 3.0;
        p += wander * (0.035 + 0.05 * j1);
      }
      ...
      if (dust) {
        float tw = 0.65 + 0.35 * sin(uT * (0.4 + 1.3 * fract(aJ * 7.1)) + aJ * 60.0);   // its own slow twinkle
        vCol = vec3(0.85, 0.45 + 0.08 * fract(aJ * 3.3), 0.20) * (0.30 + 0.35 * uCur) * tw;
  ...
  reg({
    id: 'SRC-70.3', family: 'SRC-70', ver: 3,
    title: 'BoT · Ligam à Terra V3', tech: 'WEBGL / ENDLESS ROOTS IN TILES + A HAND-HELD FALL + LIVING SOIL + SCATTER DOF',
    ...
    tags: ['TEMPLE SET', 'ORBITAL WITNESS 6', 'THE ONE THAT POINTS DOWN', 'FALLING WITH THE ROOTS', 'OPEN L = THE FALL', 'OPEN R = THE CURRENT + THE TURN'],
    desc: '"Das coisas que me ligam à terra, meu filho e essa dor nas costas." V2 is a descent with no bottom. The roots never stop growing, and the camera falls with them, held in someone\'s hands: it rides just behind the growing tip, breathes, shakes as the fall quickens, rolls and steadies itself, and leans where the roots turn. The two roots braid around one axis all the way down — gold for the son, crimson for the ache — and the current runs down them past you into the dark. Laterals and soil pass close to the lens and blur into bokeh; every mote of soil wanders on its own and twinkles. Everything above has aged to dark wire; only the current lights it.',
    interact: 'THE SOURCE LAW, and the last position holds. LEFT IS THE FALL: at the Source it is nearly a hover, the roots inching down; open it and you fall faster, and the hand-held camera shakes and rolls more as it tries to keep up. RIGHT IS THE CURRENT AND THE TURN: one slow pulse and a camera that barely moves at the Source; open it and the current thickens, quickens and glints, the ache throbs hard, and the camera slowly turns around the falling roots. A pulse that reaches the growing tip is a sound.',
    ...
    init(P) {
      const s = {
        ...
        pres: 0, fall: 0.2, cur: 0.2, orb: 0, orbV: 0, g: 0.3, v: 0, phA: 0, phB: 0, _nA: 0, _nB: 0,
    ...
    step(P, dt, t, inp) {
      ...
      // THE TURN (right arm): a gentle orbit, eased like a body turning
      const orbWant = 0.015 + 0.24 * Math.pow(clamp(SOURCE(inp.R) * s.pres + idleR * (1 - s.pres)), 1.3);
      s.orbV += (orbWant - s.orbV) * Math.min(1, dt * 0.8);
      s.orb += s.orbV * dt;
