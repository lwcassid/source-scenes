/* ---------- SRC-10.17 · WEATHER STATION V17 (THE SINGING FORM) ---------- */
/* Lance on V16: not integrated - the music concept didn't match the
   picture. The census (what the eye sees vs what the ear hears) found the
   bells had no visual counterpart and the picture's protagonists (the
   pole-form, the thread-rings) made no sound. V17 rebuilds the instrument
   around them:
   - THE FORM IS THE VOICE: one continuous singing tone. Its height in the
     sky IS its pitch on the chord ladder; its left-right position is its
     pan. The left hand plays melody by flying the form.
   - RINGS ARE HARMONY: each thread-ring tier that fills adds a chord
     voice with a rolled, announced entrance. Count the rings on screen,
     count the voices in the air.
   - THE WIND IS THE BOW: noise filter/pan strictly follow flow speed and
     heading, under the voice. Orbit speed puts a slow undulation on the
     form's tone.
   - A HARD TURN COMBS: as the field visibly straightens, the harmony
     voices pull briefly into the lead's pitch - coherence made audible -
     with the air-crack on top.
   - The BELLS ARE GONE (scheduled hits on a continuous verb = bolted-on).
     Pinned Gm pedal, low floor, gale roar and lure idle stay from V16. */
reg({
  id: 'SRC-10.17', family: 'SRC-10', ver: 18,
  title: 'Weather Station V17', tech: 'DENSITY SMOKE / THE SINGING FORM',
  music: {
  ...
  audio(A, P) {
    ...
    return {
      tick() {
        ...
        const und = 1 + 0.16 * Math.sin(tt * (0.6 + (s.gust || 34) * 0.012));
        const vLvl = (0.008 + dens * 0.05) * pres * und;
        ...
        for (let i = 0; i < 3; i++) {
          ...
          if (want && !was[i]) {
            ...
            const rf0 = H.chordTone(deg + 2 + i * 2, 0);
          ...
          // combing pulls each ring toward the lead's pitch as the field aligns
          const target = H.chordTone(deg + 2 + i * 2, 0);
          rings[i].set(target * (1 - comb * 0.5) + vf0 * (comb * 0.5), comb > 0 ? 0.15 : 0.6);
