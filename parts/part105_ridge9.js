/* ---------- SRC-42.9 · RIDGE LOOM V9 (breath stays on the three colours) ---------- */
reg({
  id: 'SRC-42.9', family: 'SRC-42', ver: 9, title: 'Ridge Loom V9', tech: 'CONTOUR HEAT / THREE COLOURS, 6S BREATH',
  music: {
  ...
  fx: { bloom: 0.45 },
  tags: ['LEFT HAND = BASS', 'RIGHT HAND = LEADS', 'HEIGHT IS THE COLOUR', 'THREE COLOURS ONLY'],
  desc: 'V8\'s six-second breath, but the transition now stays inside the three colours instead of touring the spectrum. Two states: violet (#4900ff) floor with orange (#f36c3b) summits, breathing into an orange floor with cyan (#00edff) summits, and back, forever. Between them every colour on screen sits on a straight gradient between two of those three — the floor walks violet, purple, magenta, pink, orange; the summits walk orange, salmon, pale, cyan. V8 rotated hue to dodge the grey that orange and cyan make when you blend them, which bought saturation at the price of yellow and green appearing mid-breath. Those were not colours anyone chose, so they are gone; the pale band the summits pass through is simply the middle of the orange-cyan gradient, and on black it reads as heat. Everything else is V7: one fine stroke, height picks the hue, proximity picks the burn, an empty frame stays dark.',
  ...
  init(P) {
    ...
    const s = P.state;
    // TWO PALETTE STATES, six seconds apart. Endpoints blend STRAIGHT between
    // the anchors — same gamma blend the ramp itself uses — so every colour the
    // scene can show lies on a line between two of the three chosen colours.
    // V8 rotated hue here to dodge the pale middle of the orange-cyan blend;
    // that dodge is what put yellow and green on screen, and nobody chose those.
    //   state 1: floor #4900ff  ->  peaks #f36c3b
    //   state 2: floor #f36c3b  ->  peaks #00edff
    s.stLo = [[0x49, 0x00, 0xff], [0xf3, 0x6c, 0x3b]];
    s.stHi = [[0xf3, 0x6c, 0x3b], [0x00, 0xed, 0xff]];
  ...
  step(P, dt, t, inp) {
    ...
    for (let e = 0; e < 2; e++) {
      const A = (e ? s.stHi : s.stLo)[0], B = (e ? s.stHi : s.stLo)[1], d = e ? s.curHi : s.curLo;
      for (let i = 0; i < 3; i++)
        d[i] = Math.sqrt(A[i] * A[i] * (1 - s.mix) + B[i] * B[i] * s.mix) | 0;
    }
