/* ---------- SRC-18.11 · NIGHT CIRCUIT V11 (the instrument of the street) ---------- */
reg({
  id: 'SRC-18.11', family: 'SRC-18', ver: 11,
  title: 'Night Circuit V11', tech: 'WEBGL / GEMS / LANE INSTRUMENT / WHALES',
  ...
  init(P) {
    ...
    T3.mkGlow = mkGlow;
    {
      ...
      for (let i = 0; i < 14; i++) {
        ...
        g2.position.set(side * (5.4 + rnd() * 2.0), 0, -(i * 48 + rnd() * 20) - 6);
        caveG.add(g2);
      ...
      for (let i = 0; i < 16; i++) {
        ...
        g2.position.set(sx, 8.6 - Math.abs(sx) * 0.28, -(i * 42 + rnd() * 18) - 10);
        caveG.add(g2);
    ...
    if (typeof THREE.GLTFLoader === 'function') {
      ...
      loader.load('models/palm_trees.glb', gl2 => {
        try {
          ...
          src2.traverse(o => {
            if (o.isMesh && o.material) {
              (Array.isArray(o.material) ? o.material : [o.material]).forEach(mm => {
                if (mm.color) mm.color.setRGB(0.52, 0.44, 0.66); // dusk tint — the handpainted texture stays readable
              });
            }
          });
          ...
          for (let i = 0; i < 8; i++) {
            ...
            const wm = new THREE.MeshBasicMaterial({ color: 0xb01aff, wireframe: true, transparent: true, opacity: 0.02, blending: THREE.AdditiveBlending, depthWrite: false });
            const meshes2 = [];
      ...
      loader.load('models/gems.glb', gl2 => { try {
        ...
        for (let i = 0; i < 18; i++) mkGem(i, 1.4 + ((i * 31) % 10) / 10 * 2.2, i % 2 ? 1 : -1, -(i * 37 + (i * 17) % 15) - 10);
  ...
  step(P, dt, t, inp) {
    ...
    if (act === 0) {
      ...
      for (const m of T3.shrooms) {
        ...
        const sc4 = 1 + near2 * 0.22 + s.arpFlash * 0.05;
      ...
      if (T3.gems) for (const gm of T3.gems) {
        ...
        gm.userData.halo.material.opacity = 0.1 + near3 * 0.42 + s.arpFlash * 0.1 + s.kickPulse * 0.1;
      }
      // drips
      ...
      for (const p2 of (T3.palmModels || T3.palms)) {
        ...
        if (p2.userData.wireMat) p2.userData.wireMat.opacity = 0.015 + Math.max(0, 1 - Math.abs(p2.position.z + 40) / 110) * 0.05;
      }
    ...
    T3.hemi.intensity = [0.04, 0.22, 0.2, 0.3][act]; // dark base everywhere — light is a spend
  ...
  draw(P, g, w, h, t, inp) {
    ...
    if (s.noGL || !P._three) {
      ...
      g.textAlign = 'center';
      g.fillText('NIGHT CIRCUIT V11 · THE JOURNEY', w / 2, h / 2 - 12);
