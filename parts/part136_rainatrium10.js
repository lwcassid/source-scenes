/* ---------- SRC-16.10 · RAIN ATRIUM V10 (the bass learns to rest) --------
   Lance on V9's bass: the full-bar legato erased the space he liked, and
   the color-walk's B-flat poked out high. V10: root-only D, sustaining
   ~2/3 of the bar (air at the end), a touch quieter, and every fourth bar
   the bass goes fully silent — a breath where the ambient reads. --------
   Lance found it by hand on the Auto Filter: the piano closed off a touch
   in light rain, opening as the storm drives, reads beautifully. V9 streams
   wetness as LEAD CC74 so the show performs that sweep itself — and the
   RANGE is his to own in Live: the MIDI mapping row's Min/Max (his ear
   said 640 Hz → 8.72 kHz) defines what "closed" and "open" mean. Everything
   else is V8. ----------------------------------------------------------
   Lance's ears on V7: the motif rain and thinner piano were right, but the
   double-time pocket over-corrected — "studio ghibli lo-fi beat", V6's
   sleepier skeleton was better — and the bass color-hop to the fifth read
   dopey. V8: the groove reverts to V6's bones (kick on 1, quarter ticks)
   keeping only V7's polish (lazy drag, breathing velocities, bar-legato
   bass); the color bar leans on the minor third instead of the fifth, no
   pickup. And his TWO recordings layer by intensity: forest rainfall =
   held note 39 all scene (volume rides sfx CC74), thunderstorm = note 40
   held only past ~55% wetness (hysteresis, the Simpler's own envelope
   fades it) — the storm walks in when the room drives the rain hard. */
reg({
  id: 'SRC-16.10', family: 'SRC-16', ver: 10, title: 'Rain Atrium V10', tech: 'TWO-FIELD WATER / MOTIF RAIN',
  music: {
  ...
  desc: 'V7\'s melody, V6\'s sleep. The motif rain stays — each chord\'s three-note cell, a note a second at most, the register warm and low — but the pocket goes back to the lo-fi bones Lance preferred: kick breathing on the one, lazy quarter ticks, nothing double-time, everything gone in the lulls. The bass keeps its whole-bar breath and leans on the minor third for its color bar instead of hopping the fifth. And the rain is REAL, twice over: a forest rainfall carries the scene, and past half-storm a real thunderstorm walks in underneath it, both field recordings riding the weather from Ableton.',
  ...
  sound: 'MOTIF RAIN kept from V7 (3-tone voice-led cell per chord, ~1 note/0.7s, big drops anchor low, only fundamentals mirror to MIDI). POCKET reverted to V6\'s lo-fi bones on Lance\'s verdict — V7\'s double-time ghost map over-corrected ("studio ghibli lo-fi beat"): kick on 1 breathing, soft 8 every other bar, lazy dragged quarter ticks, rare open hat; polish kept (drag, velocity spread). BASS (V10): root-only D, ~2/3-bar sustain with air at the end, TACET every fourth bar — the rest is where the ambient reads; the color walk is gone (its B-flat poked out). ABLETON RAIN ×2: note 39 = forest rainfall held all scene (volume rides sfx CC74), note 40 = thunderstorm held past ~55% wetness with hysteresis (give its Simpler a slow amp attack/release — it walks in, never pops). Cast OUT: MIDI; browser patter is the offline fallback. LEAD CC74 = wetness (V9): the felt piano\'s filter closes off in light rain and opens with the storm — set the mapping\'s Min/Max in Live to taste (Lance\'s ear: 640 Hz → 8.72 kHz). Pinned key, empty mids, D pedal underneath.',
      ...
      tick(inp) {
        ...
        while (nextT < horizon) {
          ...
          if (idx > lastIdx) {
            ...
            if (gate > 0.03) {
              // V6's lo-fi bones, kept on Lance's verdict — V7's double-time
              // ghost map over-corrected. Only the polish survives: the lazy
              // drag, breathing velocities, and the bar-legato bass.
              const drag = T.beat * 0.055;
              if (st === 0) {
                A.kick(nextT, (0.06 + 0.13 * gate) * (0.9 + Math.random() * 0.2));
                // root only, ~2/3 of the bar, and every fourth bar the bass
                // RESTS — the space is the point (Lance, V10); the color
                // walk's B-flat poked out and is gone
                if (bar % 4 !== 3) A.bassNote(H.chordTone(0, -1), { at: nextT, vol: 0.04 + 0.07 * gate, dur: T.beat * 2.6 });
              }
              if (st === 8 && bar % 2 === 0) A.kick(nextT + drag, (0.06 + 0.12 * gate) * 0.45);
              if (st === 4 || st === 12) A.hat(nextT + drag, { vol: (0.015 + 0.026 * gate) * (0.8 + Math.random() * 0.4) });
              if (gate > 0.6 && st === 14 && bar % 4 === 3) A.hat(nextT, { vol: 0.014 });
