/* ---------- SRC-15.9 · FERRO BLOOM V9 (varied shapes, speckled texture, softer gradient, wider palette) ---------- */
reg({
  id: 'SRC-15.9', family: 'SRC-15', ver: 9,
  title: 'Ferro Bloom V9', tech: 'SHADER FIELD / MOTION ILLUSION',
  music: {
  ...
  desc: 'The willed garden, now with more species in it. Every bloom draws its own petal shape — soft and round, deeply scalloped, or fine-crenellated — from a per-flower seed, and roughly half wear a stippled, speckled skin instead of a smooth one. The color gradient is fully continuous now: hue melts into hue with no flat plateau, so nothing reads as a ring or a stripe. Palette widened with acid chartreuse, electric blue and hot orchid alongside the blush, teal, ember and lavender. Music unchanged from V8: a three-layer soundscape — C-minor pedal drone whose color shifts, a quantized groove that only exists once the garden is thrown wide open, and a reactive layer where every willed bloom, flick, and stillness sounds back at you.',
  ...
  init(P) {
    ...
    const mat = new THREE.ShaderMaterial({
      ...
      fragmentShader: [
        ...
        // V9: SHAPE VARIATION — each bloom blends two petal harmonics (broad
        // lobes + fine crenellation) with its own mix ratio and lobe depth,
        // so the garden holds soft round blooms, spiky stars and scalloped
        // in-betweens instead of one repeated silhouette.
        'float bloomLobes(int i, float seed, float th, float wob){',
        ...
        '  return mix(lobe1, lobe2, mixK);',
        '}',
        'float bloomAmp(int i, float seed, float wob){',
        '  float amp = i == 0 ? 0.12 : (0.09 + 0.30 * fract(seed * 8.3));',
        '  return amp + wob * 0.5;',
        '}',
        // V9: ~45% of blooms carry a speckled skin — driven by the same seed
        // that fixes their shape, so it reads as a stable trait, not noise.
        'float bloomSpeckled(float seed){ return step(0.55, fract(seed * 17.3)); }',
        ...
        // V9: two-harmonic lobe blend replaces the single cosine frequency
        '    float lobes = bloomLobes(i, seed, th, wob);',
        '    float R = S * (1.0 + bloomAmp(i, seed, wob) * lobes);',
        '    float f = d / max(R, 1e-4);',
        '    float w = exp(-f / k);',
        '    acc += w;',
        '    hueAcc += fract(seed * 5.13) * w;',
        '    speckAcc += bloomSpeckled(seed) * w;',
        ...
        // V9: wider petal-count and lobe-depth range for scatter blooms too
        ...
        '    acc += w2;',
        '    hueAcc += h3 * w2;',
        '    speckAcc += step(0.55, fract(h2 * 17.3)) * w2;',
        ...
        '  float speckLevel = speckAcc / max(acc, 1e-20);',
        ...
        // so luminance never flatlines at either extreme (that flatlining —
        // half of every ring holding one flat hue — was the "strips" look).
        '  float bandsN = 2.6;',
        '  float phase = f * bandsN - mod(uPh, 4.0) * dir;',
        '  float band = 0.5 + 0.28 * cos(TAU * phase);',
        '  float ri = floor(phase + 0.5) + hueOff;',
        ...
        // V9: speckled skin — fine hash flecks, dark + light, only where the
        // locally dominant blooms are speckled; smooth blooms stay smooth.
        '  float speckN = hash21(floor(p * 60.0));',
        '  float dots = smoothstep(0.58, 0.92, speckN);',
        '  bc *= 1.0 - dots * speckLevel * 0.42;',
        '  bc += cream * dots * speckLevel * 0.22;',
  ...
  draw(P, g, w, h, t, inp) {
    ...
    if (s.noGL || !P._three) {
      ...
      g.textAlign = 'center';
      g.fillText('FERRO BLOOM V9 · SPECIES & SPECKLE', w / 2, h / 2 - 12);
