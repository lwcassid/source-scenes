/* ---------- SRC-43.9 · CELL FRONT V9 (the kick owns the swell) ----------
...
const CF9_POOL = (s, x, y) => {
  let acc = 0;
  for (const l of CF9_LOBES(s)) {
...
reg({
  id: 'SRC-43.9', family: 'SRC-43', ver: 9, title: 'Cell Front V9', tech: 'VOLUMETRIC CELL FIELD / HAND-PAINTED AUDIO POCKETS',
  audioIn: true,
  fx: { bloom: 0.34 },
  tags: ['VOLUMETRIC', 'AUDIO IN', 'THREE POCKETS', 'HANDS = COLOR', 'KICK SWELL', 'REFORMING'],
  desc: 'Same three hand-painted pockets as V8, retimed for techno and house. The KICK is the only thing that makes a cloud swell hard: every kick balloons all three pockets together — bass hardest — and they breathe back down over the beat. The bassline and the melody do something slower: they set the size of the whole field, the ensemble growing and spreading as a track builds and settling as it thins, with each pocket only gently favoured by its own band. The field reforms on a real change in the music — a drop, a lead coming in — not on every note. Hands still paint the color.',
  ...
  init(P) {
    const s = {
      ...
      field: 0, kick: 0, kBass: 0, kMid: 0, kTreble: 0,
  ...
  step(P, dt, t, inp) {
    ...
    s.bass += (bassTarget - s.bass) * Math.min(1, dt * (bassTarget > s.bass ? 2.2 : 1.3));
    s.mid += (midTarget - s.mid) * Math.min(1, dt * (midTarget > s.mid ? 2.2 : 1.3));
    s.treble += (trebleTarget - s.treble) * Math.min(1, dt * (trebleTarget > s.treble ? 2.2 : 1.3));
    s.energy += (((s.bass + s.mid + s.treble) / 3) - s.energy) * Math.min(1, dt * 3);
    // the field: bass-weighted mix of the slow bands — how big the whole
    // picture is right now.
    const fieldTarget = clamp(0.45 * s.bass + 0.35 * s.mid + 0.30 * s.treble);
    ...
    if (onsetEdge) {
      const hit = clamp(0.55 + inp.audio.level * 0.6);
    ...
    const fluxTarget = clamp(fluxRaw * 0.9);
    s.flux += (fluxTarget - s.flux) * Math.min(1, dt * (fluxTarget > s.flux ? 6.0 : 1.8));

    const axc = clamp(s.ax, 0.6, 1.7);
    // spread follows the field, with a small outward push on the kick
    s.offBass = (0.10 + 0.40 * s.field + 0.05 * s.kBass) * axc;
    s.offTreble = (0.10 + 0.40 * s.field + 0.05 * s.kTreble) * axc;
    // each pocket's share of the field bends only gently toward its own
    // slow band (0.75..1.25); the kick multiplies on top — up to ~2x.
    const share = (band, mean) => 1 + 0.5 * clamp(band - mean, -0.5, 0.5);
    const mean = (s.bass + s.mid + s.treble) / 3;
    const base = 0.018 + s.field * 0.50;
    const Rbt = base * share(s.bass, mean) * (1 + 0.95 * s.kBass);
    const Rmt = base * 0.85 * share(s.mid, mean) * (1 + 0.85 * s.kMid);
    const Rtt = base * share(s.treble, mean) * (1 + 0.75 * s.kTreble);
    // radius smoothing: instant enough to carry the kick's snap, soft enough
    // to hide any residual band motion.
    s.Rbass += (Rbt - s.Rbass) * Math.min(1, dt * (Rbt > s.Rbass ? 14 : 5));
    s.Rmid += (Rmt - s.Rmid) * Math.min(1, dt * (Rmt > s.Rmid ? 13 : 5));
    s.Rtreble += (Rtt - s.Rtreble) * Math.min(1, dt * (Rtt > s.Rtreble ? 14 : 5));
    ...
    if (onsetEdge) {
      if (s.blooms.length < 6) {
        ...
        for (let k = 0; k < 12 && !ok; k++) {
          ...
          if (CF9_POOL(s, bx, by) > 0.4) ok = true;
        }
        if (ok) {
    ...
    for (let i = s.blooms.length - 1; i >= 0; i--) {
      const b = s.blooms[i];
      const inPaint = CF9_POOL(s, b.x, b.y) > 0.4;
  ...
  draw(P, g, w, h, t, inp) {
    ...
    if (s.noGL || !P._three) {
      ...
      g.fillText('CELL FRONT V9 · open on the hosted site (WebGL)', 10, h - 10);
      return;
    }
    ...
    u.uU.value = s.U;
    const ll = u.uLobe.value, LB = CF9_LOBES(s);
