/* ---------- SRC-15.7 · FERRO BLOOM V7 (tasteful: intermesh back, filigree seam, beats only in full bloom) ---------- */
reg({
  id: 'SRC-15.7', family: 'SRC-15', ver: 7,
  title: 'Ferro Bloom V7', tech: 'SHADER FIELD / MOTION ILLUSION',
  music: {
  ...
  desc: 'The willed garden, tasteful. The flowers intermesh into one breathing mass again, gradients melt hue into hue instead of striping, and the converging filigree at the flow seam is back — slim, bounded, alive. The music is a three-layer soundscape: a C-minor PEDAL DRONE whose color shifts instead of its root — Cm7♭13, the G/A♭ blur of Cm9♭13, Cm11, Cm7 — each transition rung in by a gentle low-to-high roll; a quantized groove that does not exist until the garden is thrown wide open; and a reactive layer where every willed bloom, flick, and stillness sounds back at you. Silence has room again.',
  interact: 'EACH HAND WILLS ITS OWN SIDE: L reach dials up to 6 flowers into the left half, R up to 6 into the right — instantly, one per notch, gone as fast when you pull back. Keep it small and it stays chamber-music quiet; throw both arms wide (past ~55%) and the groove fades in. Quick flicks fire log-drum answers only in full bloom. Stillness after a gesture invites the garden to answer.',
  ...
  init(P) {
    ...
    const mat = new THREE.ShaderMaterial({
      ...
      fragmentShader: [
        ...
        // V7 GRADIENTS, NOT STRIPES: fewer/wider bands, pure-cosine profile,
        // and ring hues MELT into each other instead of switching per ring.
        '  float bandsN = 3.3;',
        ...
        '  vec3 tint = mix(ringTint(floor(rif)), ringTint(floor(rif) + 1.0), smoothstep(0.25, 0.75, fract(rif)));',
        '  tint = mix(tint, vec3(0.98, 0.55, 0.38), warmMix);',
        '  vec3 bc = mix(deep, tint, smoothstep(0.02, 0.92, band));',
        '  bc = mix(bc, cream, 0.40 * smoothstep(0.82, 1.0, band));',
  ...
  draw(P, g, w, h, t, inp) {
    ...
    if (s.noGL || !P._three) {
      ...
      g.textAlign = 'center';
      g.fillText('FERRO BLOOM V7 · THE WILLED GARDEN, TASTEFUL', w / 2, h / 2 - 12);
    ...
    return {
      tick(inp) {
        ...
        while (nextT < horizon) {
          ...
          // arp is garnish, and only in full bloom
