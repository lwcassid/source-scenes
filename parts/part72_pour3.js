/* ---------- SRC-41.3 · POUR CELLS V3 (every cell carries a note) ---------- */
reg({
  id: 'SRC-41.3', family: 'SRC-41', ver: 3, title: 'Pour Cells V3', tech: 'BOILING SOURCE / EVERY CELL IS A NOTE',
  music: {
    // SIX CHORDS ACROSS THE RIGHT HAND. chordBars is long on purpose: nothing
    // here changes on a timer, only when the hand moves.
    bpm: 72, root: 38, mode: 'aeolian', chordBars: 16,
    chords: [
      [0, 7, 15, 22, 26],   // 0  Dm9
      [0, 5, 12, 17, 22],   // 1  Gsus/D
      [0, 8, 15, 19, 24],   // 2  B♭maj7/D
      [0, 10, 15, 19, 24],  // 3  Dm7
      [0, 7, 14, 17, 22],   // 4  Dm11
      [0, 9, 12, 15, 20]    // 5  Dm6      — the dorian sixth, the brightest
    ],
    chordNames: ['Dm9', 'Gsus/D', 'B♭maj7/D', 'Dm7', 'Dm11', 'Dm6']
  },
  fx: { bloom: 0.4 },
  tags: ['A SOURCE UNDER THE FRAME', 'L = HEAT · R = THE CHORD', 'EVERY CELL CARRIES ITS OWN NOTE', 'YOU CAN SEE THE NOTE BEFORE YOU HEAR IT'],
  ...
  step(P, dt, t, inp) {
    ...
    for (let i = C.length - 1; i >= 0; i--) {
      ...
      if (c.r >= c.bur) {
        ...
        if (aud) s.evq.push({ r: rU, x: c.x / w, y: c.y / h, m: c.merges, deg: c.deg });
      }
    }
