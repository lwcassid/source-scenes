/* ---------- SRC-38.15 · LUMEN FIELD (rays with weight) ---------- */
reg({
  id: 'SRC-38.15', family: 'SRC-38', ver: 15, title: 'Lumen Film', tech: 'GEM FILM / APERTURE ORGAN',
  music: {
  ...
  fx: { bloom: 0.46 },
  tags: ['ONE BEAM PER STONE', 'ALL FOUR RAYS REFRACT', 'BRIGHTER CORES', 'L TURNS THE GRADIENTS TOO'],
  desc: 'V15: same size as V14, more weight behind it. The rays had gone quiet — the passes that make up the beam were carrying too little to hold their own against the stones. The colour along each ray runs stronger and holds longer before it fades, the body of every ray is close to twice what it was, and the thread down the middle is brighter and slightly thicker so each one has a definite core rather than a haze. The one pass deliberately left alone is the broad outer falloff: it covers more ground than everything else combined, so lifting it does not make the rays read as light, it just fogs the whole frame.',
