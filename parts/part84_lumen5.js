/* ---------- SRC-38.5 · LUMEN FIELD (the reference palette) ---------- */
reg({
  id: 'SRC-38.5', family: 'SRC-38', ver: 5, title: 'Lumen Film', tech: 'GEM FILM / APERTURE ORGAN',
  music: {
  ...
  fx: { bloom: 0.46 },
  tags: ['REFERENCE-PALETTE STONES', 'NARROW BEAMS ONLY', 'RGB FRINGE ON THE EDGE', 'NO RADIAL HALO'],
  desc: 'V5: recut to a supplied reference — every colour in the scene now comes from one eight-stop gradient (black, blue, mint, near-white, gold, red, indigo, black), the same run used on the source illustration, and nothing else. Each stone\'s six facets share that run but each stone tilts its own gradient axis at a different angle, so no two diamonds catch it quite the same way. The radial halo is gone. In its place the light beams themselves are narrow, and carry a thin red/blue fringe riding just off their edges — chromatic separation on the beam, not a ring around the spark.',
  ...
  init(P) {
    ...
    for (let i = 0; i < N; i++) {
      cells.push({
        ...
        gradRot: (P.rand() - 0.5) * 1.3,   // each stone tilts its gradient axis differently
  ...
  draw(P, g, w, h, t, inp) {
    ...
    // side. Every facet shares the SAME reference gradient, but each STONE
    // rotates its own gradient axis by a random amount (c.gradRot) — the
    // same trick the reference file uses (its six triangles all reuse one
    // gradient at six different vector angles). Litness (fixed overhead sun
    // × facet's own true angle, unrelated to the randomised gradient tilt)
    // still gates brightness, scaled by presence so the sheet stays dark
    ...
    // floating over the black gap between stones. No radial halo — the
    // chromatic separation lives on the beams themselves now.
    ...
    for (const p of s.ap) {
      ...
      if (p.open > 0.2) {
        ...
        for (const dr of dirs) {
          ...
          const coreW = Math.max(0.9 * ms, R * (dr.len > 0.9 ? 0.07 : 0.045) * p.open);
          g.strokeStyle = core;
          g.lineWidth = coreW;
