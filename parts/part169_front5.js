/* ---------- SRC-43.5 · CELL FRONT V5 (three pockets, no shadow) ----------
   Nima's verdict on V4: keep the blobs — the drifting, layered cell field is
   the right material — but everything else about how they're SHOWN and what
   drives them changes.
   NO BLACK BLOBS. V4 (and every version before it) sat each cell in its own
   near-black shadow ring, an island in a grey ground. That's gone. This is
   volumetric now: pure black behind the field, and every cell is a soft
   glowing density — bright core, luminous falling-off edge, nothing dark
   drawn ON TOP of it. The "ground" material and its pinhead speckle are
   gone too; there is no ground, only glow and true black.
   THREE REAL POCKETS, not two hands and a synthetic blend. V4's warm/cool
   banks are gone — BASS, MID and TREBLE each get their own lobe, sized by
   their own band directly, colored ember-orange / warm gold / cool violet.
   Where two or three overlap, the overlap itself reads brighter — a weld
   that emerges from the density summing, not a fourth hardcoded value.
   HANDS ARE SENSITIVITY, not competition. V4 took whichever was louder,
   hand or mic — which meant a hand that had merely drifted near a wall's
   ghost-drift snapshot could sit there silently overriding real audio
   forever. Now the mic sets the SHAPE (what the room actually sounds like)
   and the hands set the GAIN on it: L turns up how loud bass reads, R turns
   up treble, so a performer can visibly turn the instrument's ear up and
   down and make it dance with what it hears, instead of arm-wrestling it
   for control. Nothing is ever silent — a floor under the gain keeps the
   room breathing even at rest. ------ */
const CF5_LOBES = s => [
  [-s.offBass, -0.04, s.Rbass], [0, 0.05, s.Rmid], [s.offTreble, -0.04, s.Rtreble]
];
const CF5_POOL = (s, x, y) => {
  let acc = 0;
  for (const l of CF5_LOBES(s)) {
...
const CF5_FS = [
  ...
  '  lay(pa, dr0, rot, U0, 1.0,  0.15, 0.44, 0.90, U0 * 0.22, dS, dH, SP, SH, SR);',
  '  lay(pa, dr1, rot, U1, 17.0, 0.20, 0.58, 0.95, U1 * 0.22, dS, dH, SP, SH, SR);',
  '  lay(pa, dr2, rot, U2, 31.0, 0.24, 0.54, 0.85, U2 * 0.24, dS, dH, SP, SH, SR);',
  '  for (int i = 0; i < 8; i++){',
  '    if (i >= uNB) break;',
  '    vec4 b = uBloom[i];',
...
reg({
  id: 'SRC-43.5', family: 'SRC-43', ver: 5, title: 'Cell Front V5', tech: 'VOLUMETRIC CELL FIELD / THREE-BAND AUDIO POCKETS',
  audioIn: true,
  fx: { bloom: 0.34 },
  tags: ['VOLUMETRIC', 'AUDIO IN', 'THREE POCKETS', 'HANDS = SENSITIVITY'],
  desc: 'The drifting cell field every Cell Front has had, rebuilt from the shading up. No black — every cell is a soft glowing density, bright at its core and luminous at its edge, floating in true black; there is no ground, no shadow, nothing dark drawn on top of the glow. Three real pockets now, not two hands and a blend: BASS, MID and TREBLE each get their own lobe, sized by their own band and colored ember-orange / warm gold / cool violet, drifting and breathing exactly like the material always has. Where two or three pockets genuinely overlap the picture reads brighter on its own — a weld that emerges from the glow summing rather than a fourth number pretending to be one. A detected hit still pops a bloom the way it has since V4, now tinted by whichever pocket it lands nearest.',
  interact: 'This scene listens (SHOW CHECK → AUDIO IN, or MAP → Audio in) — the mic or a captured app is what the pockets actually respond to. Hands do not fight the audio for control anymore: L turns up how sensitive the BASS pocket is to what it hears, R turns up TREBLE’s — reach out to turn the instrument’s ear up, pull back to calm it, and dance the two together. Nothing here goes silent: there is a floor under both hands’ gain, so the room keeps breathing even resting at the source.',
  sound: 'Makes no sound of its own — an audio-in scene, same as V4. Connect a source (mic, line-in, or CAPTURE APP AUDIO for a running app’s own output) in MAP → Audio in, then SET REST with the room quiet so silence reads as silence.',
  ...
  step(P, dt, t, inp) {
    ...
    // SENSITIVITY, not competition (Nima's call). Audio sets the SHAPE —
    // what the room actually sounds like; hands set the GAIN on top of it.
    // A floor under the gain means the instrument is never silenced by
    // hands sitting at rest, only turned down.
    ...
    // silence still breathes — the same idle-tease every still scene gets
    const idle = 0.12 + 0.05 * Math.sin(s.life * 0.19);
    const bassTarget = Math.max(idle * (1 - s.pres), clamp(inp.audio.bass * sensL));
    const midTarget = Math.max(idle * 0.75 * (1 - s.pres), clamp(inp.audio.mid * sensMid));
    const trebleTarget = Math.max(idle * (1 - s.pres), clamp(inp.audio.treble * sensR));
    s.bass += (bassTarget - s.bass) * Math.min(1, dt * 6);
    s.mid += (midTarget - s.mid) * Math.min(1, dt * 6);
    s.treble += (trebleTarget - s.treble) * Math.min(1, dt * 6);
    s.energy += (((s.bass + s.mid + s.treble) / 3) - s.energy) * Math.min(1, dt * 4);

    const axc = clamp(s.ax, 0.6, 1.7);
    s.offBass = (0.10 + 0.44 * s.bass) * axc;
    s.offTreble = (0.10 + 0.44 * s.treble) * axc;
    const Rbt = 0.07 + s.bass * 0.52, Rmt = 0.06 + s.mid * 0.40, Rtt = 0.07 + s.treble * 0.52;
    s.Rbass += (Rbt - s.Rbass) * Math.min(1, dt * (Rbt > s.Rbass ? 0.9 : 1.3));
    s.Rmid += (Rmt - s.Rmid) * Math.min(1, dt * (Rmt > s.Rmid ? 0.9 : 1.3));
    s.Rtreble += (Rtt - s.Rtreble) * Math.min(1, dt * (Rtt > s.Rtreble ? 0.9 : 1.3));

    /* ---- blooms: onset-driven only, same shape as V4. A hit spawns one
    ...
    if (onsetEdge) {
      if (s.blooms.length < 6) {
        ...
        for (let k = 0; k < 12 && !ok; k++) {
          ...
          if (CF5_POOL(s, bx, by) > 0.4) ok = true;
        }
        if (ok) {
    ...
    for (let i = s.blooms.length - 1; i >= 0; i--) {
      const b = s.blooms[i];
      const inPaint = CF5_POOL(s, b.x, b.y) > 0.4;
  ...
  draw(P, g, w, h, t, inp) {
    ...
    if (s.noGL || !P._three) {
      ...
      g.fillText('CELL FRONT V5 · open on the hosted site (WebGL)', 10, h - 10);
      return;
    }
    ...
    u.uU.value = s.U;
    const ll = u.uLobe.value, LB = CF5_LOBES(s);
