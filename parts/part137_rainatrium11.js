/* ---------- SRC-16.11 · RAIN ATRIUM V11 (Lance's groove, thinned) --------
...
reg({
  id: 'SRC-16.11', family: 'SRC-16', ver: 11, title: 'Rain Atrium V11', tech: 'TWO-FIELD WATER / MOTIF RAIN',
  music: {
        ...
        while (nextT < horizon) {
          ...
          if (idx > lastIdx) {
            ...
            if (gate > 0.03) {
              ...
              if (weak ? gate > 0.5 : gate > 0.12)
                A.hat(at2, { vol: (0.010 + 0.02 * gate) * HATV[st] * 2.2 * (0.85 + Math.random() * 0.3) });
              if (st === 0 && bar % 4 !== 3)
                A.bassNote(H.chordTone(0, -1), { at: nextT, vol: 0.04 + 0.07 * gate, dur: T.beat * 2.6 });
              // TURNAROUND (Lance): chill on D for three bars, then bar four
              // walks UP — B♭, then G — and falls home to D on the next
              // downbeat. Lands diatonic on both chords it meets (bar 4 of
              // the cycle is B♭maj9, bar 8 is Gm9).
              if (bar % 4 === 3) {
                if (st === 0) A.bassNote(H.rootFreq(-1) * Math.pow(2, 8 / 12), { at: nextT, vol: 0.045 + 0.075 * gate, dur: T.beat * 1.8 });
                if (st === 8) A.bassNote(H.rootFreq(-1) * Math.pow(2, 5 / 12), { at: nextT, vol: 0.045 + 0.075 * gate, dur: T.beat * 1.7 });
              }
            }
          }
          nextT += stepDur;
