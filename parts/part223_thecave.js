...
reg({
  id: 'SRC-50.1', family: 'SRC-50', ver: 1,
  title: 'The Cave V1', tech: 'NIGHT CIRCUIT CAVERN / PROCESSION',
  audioIn: true,
  ...
  init(P) {
    ...
    T3.ground = ground;
    // ember guides — V17's cave-dirt dashes, dimmed to a hint. They never flash.
    T3.dashes = [];
    const dashG = new THREE.BoxGeometry(0.3, 0.02, 2.2), dashM = B(0x3a2418);
    dashM.toneMapped = false; T3.dashMat = dashM;
    for (let i = 0; i < 36; i++) {
    ...
    T3.mkGlow = mkGlow;
    {
      ...
      const mkCaveTube = (zoff) => {
        ...
        const rock = new THREE.Mesh(tube, new THREE.MeshLambertMaterial({ color: 0x171030, emissive: 0x080418, side: THREE.BackSide, flatShading: true })); // faceted violet mass, not void
        grp.add(rock);
        const wire = new THREE.Mesh(tube, new THREE.MeshBasicMaterial({ color: 0x7a55ff, wireframe: true, transparent: true, opacity: 0.022, side: THREE.BackSide, blending: THREE.AdditiveBlending, depthWrite: false }));
  ...
  step(P, dt, t, inp) {
    ...
    for (const tb of T3.caveTubes) {
      ...
      tb.userData.wireMat.opacity = 0.018 + glow * 0.03; // structure stays an event, never a flash
    }
