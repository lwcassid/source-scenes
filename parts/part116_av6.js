/* ---------- SRC-09.6 · ATTRACTOR VESPERS V6 (the organism palette) ---------- */
reg({
  id: 'SRC-09.6', family: 'SRC-09', ver: 6, title: 'Attractor Vespers V6', tech: 'CLIFFORD MAP / FLAME DENSITY',
  music: {
  ...
  tags: ['STRANGE ATTRACTOR', 'FLAME DENSITY', 'LIGHT = SOUND', 'LIVING GLOW'],
  desc: 'Four constants and a spark, rendered as a living thing. V6 throws away the color filter: the palette now comes from the FORM itself. The accumulated smoke is one warm organic body — russet into amber into bone-white where it burns — and a cold bioluminescent glow lives only in the veins the orbit is working RIGHT NOW. Hold still and the glow settles into the caustic folds; move, and the leading edge blazes alive while the abandoned smoke cools back into amber history. The sound is still the light: the synth swells, opens and sings with the measured picture.',
  interact: 'L bends constant a, R bends constant b — the figure obeys instantly, and reach outward = more: at the sphere the map rests as condensed embers, full stretch on both sides is the torn smoke at its most chaotic. Color is anatomy, not position: amber body = where the smoke has settled, cold glow = where the point is weaving right now — watch it chase your gesture. The hands tint the glow (L pulls it green-gold, R pulls it violet-blue). Brightness drives the synth\'s intensity, caustic sharpness rings the air, and the beam voice slides along the chord wherever the light\'s mass sits.',
  sound: 'ONE SYNTH, played by the picture (V5\'s engine, unchanged). The rendered frame is measured every tick — total luminous energy, caustic sharpness, and the centroid of the light. BRIGHTNESS = INTENSITY: bed, sub and the sawtooth teeth swell and open as the picture burns. SHARPNESS = RESONANCE: filaments and hard caustics raise the cathedral air\'s Q until it sings. THE BEAM: one voice glides continuously along the chord ladder following the light\'s mass, panned with it, held on lead ch1. Chord color drifts Gm(add9) → Gm9 → Cm11 → E♭maj9 every four bars behind a quiet roll; fast hands doppler the whole hum. No sequenced notes. Ableton: pad ch2 CC74 = the LIGHT, beam on lead ch1, sub on texture ch6.',
  init(P) {
    ...
    P.state = {
      ...
      amb: null, glo: null, tcurve: null, lutPeak: -1, lutMix: -1, peak: 60, fpeak: 8, dt: 0.016
    };
  },
  step(P, dt, t, inp) {
    const s = P.state;
