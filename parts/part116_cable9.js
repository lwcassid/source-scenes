/* ---------- SRC-13.9 · EVENT HORIZON V9 — the swallow's manners ---------- */
/* Three legibility fixes around the crown jewel. TELEGRAPH: while charging,
   the void's edge visibly creeps up from the bottom — the room sees it
   coming (anticipation law). RECHARGE: after a swallow the rim burns
   ember-dim and the HUD says so, so a second attempt reads "not yet"
   instead of "broken". SCAR: each swallow permanently eats ~18% of the
   stars and deepens the rim — minute 9 no longer looks like minute 1;
   a long session visibly consumes the sky (the arc law). */
reg({
  id: 'SRC-13.9', family: 'SRC-13', ver: 9, title: 'Event Horizon', tech: 'VERLET ROPE / ACCRETION',
  music: {
  ...
  interact: 'L = left arm of the conductor, R = right. Height is the crescendo: every chair you add is announced and audibly thickens the wall; the filter opens as you rise, the pulse and the booms arrive as the line tightens, the gate throbs deeper. A flick = one run that OBEYS the throw: upward flicks rise, downward flicks fall, soft lobs run low and dark, sharp snaps sit nearly an octave brighter — and the ribbons fall into the void. HOLD BOTH ARMS HIGH: the void starts CLIMBING while the riser builds — keep holding and it swallows everything but the line. Each swallow leaves a scar: fewer stars, a deeper burn, forever. While it recharges the rim runs ember-dim — wait for the fire to return.',
  ...
  step(P, dt, t, inp) {
    ...
    // recharge state, made visible: 1 right after a swallow, easing to 0
    s.cool = s.act === 0 ? clamp((s.cdT - t) / 25) : 0;
  ...
  draw(P, g, w, h, t, inp) {
    ...
    // recharging = the rim burns ember-dim; scars = it runs deeper, forever
    const rimA = (0.38 + s.tension * 0.18 + s.chg * 0.3) * (1 - s.cool * 0.45);
  ...
  audio(A, P) {
    ...
    return {
      tick(inp) {
        ...
        // run, a snap sits nearly an octave brighter.
        let ev, k = 0;
        while ((ev = s.strumQ.shift()) && k < 2) {
          k++;
          const sharp = clamp((ev.sp - 0.9) / 2.2);
          const base = Math.floor(s.tension * 4) + Math.round(sharp * 4);
