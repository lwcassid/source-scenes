/* ---------- SRC-13.12 · EVENT HORIZON V12 — land on the boom, aurora warp ---------- */
/* Lance on V11: the transition into the window went QUIET (boom, then ~1.6s
   of ducked nothing while the void receded, then the groove from scratch),
   and the warp wanted to be a different color — a screensaver flying through
   space. V12: THE DROP LANDS ON THE BOOM — the window opens AT totality, the
   kick pattern is already driving while the void recedes (you fly out of the
   black into the warp), drums and bass ignore the duck, only the chairs keep
   their staggered roll-in. And the warp is an AURORA: deep teal-black space,
   streaks colored by depth (far = deep green, near = hot cyan-white), the
   sky's ambient glow tinting cyan — while the line and rim stay burning
   ember. Orange horizon vs cyan warp: the two worlds cannot be confused. */
reg({
  id: 'SRC-13.12', family: 'SRC-13', ver: 12, title: 'Event Horizon', tech: 'VERLET ROPE / ACCRETION',
  music: {
  ...
  step(P, dt, t, inp) {
    ...
    if (s.act > 0) {
      ...
      if (actPrev < 0.5 && s.act >= 0.5) s.jamT = 47;
      if (s.act >= 1) {
        s.act = 0; s.rise = 0;
    ...
    for (const st of s.stars) {
      ...
      st.vy -= 0.75 * s.jam * throttle * st.z * dt;     // warp lift, depth-scaled
      st.vy = Math.min(st.vy, 0.55 * Math.max(hunger, 0.2));
      st.vy = Math.max(st.vy, -(0.15 + 0.65 * s.jam * throttle) * st.z);
      st.y += st.vy * dt;
      ...
      if (st.y >= cy - 0.004 && s.jam < 0.5) {
        ...
        st.vy = -(0.1 + P.rand() * 0.2) * st.z;         // bounced back out
      }
      if (st.y < -0.06) {
        if (s.jam > 0.1) {                              // reborn AT the line, rising
          st.x = P.rand(); st.y = cableYat(st.x) / h - 0.02 - P.rand() * 0.03;
          st.vy = -(0.1 + P.rand() * 0.25) * st.z; st.px = st.x; st.py = st.y;
  ...
  draw(P, g, w, h, t, inp) {
    ...
    for (const st of s.stars) {
      ...
      // warm ember sky normally; AURORA in the warp — far stars deep green,
      // near stars hot cyan-white, so depth reads as color as well as speed
      const zn = clamp((st.z - 0.45) / 0.95);
      const r = lerp(200 + sp * 55, 40 + zn * 130, jw);
      const gg = lerp(205 - sp * 60, 215 + zn * 40, jw);
      const bb = lerp(255 - sp * 120, 150 + zn * 105, jw);
      g.strokeStyle = `rgba(${r | 0},${gg | 0},${bb | 0},${Math.min(1, a)})`;
      g.lineWidth = (2.2 + sp * 1.8) * (1 + (st.z - 0.9) * jw * 0.8) * ms;
  ...
  audio(A, P) {
    ...
    return {
      tick(inp) {
        ...
        A.set(hg.gain, ((0.005 + s.tension * 0.02) * gate + idleAmt * (0.0045 * und + s.breath * 0.014)) * duckM * scarHeavy, 0.3);
        ...
        for (let j = 0; j < 6; j++) {
          ...
          const lvl = (j < 3 ? 0.045 : 0.03) * env[j] * body * gate * gv * duckM * retK;
        ...
        while ((ev = s.strumQ.shift()) && k < 2) {
          ...
          const vol0 = clamp(0.055 + ev.sp * 0.05, 0.06, 0.14) * gate * duckM;
          for (let i = 0; i < 5; i++) {
