/* ---------- SRC-42.17 · RIDGE LOOM V17 (the band learns restraint)
   Lance on V16's Ableton send: "Ridge Loom is too busy when it sends to
   Ableton, can we make it a little more subtle somehow? Maybe less
   instruments or something? It's just too intense."
   Diagnosis: at full reach FIVE pitched channels fired at once — a
   six-note bass figure every bar, the STRINGS re-sing, the held ARP,
   BELLS twice a bar, and lead accent plucks on every loop change, up
   AND down. The sound-craft law is "for intensity, gate the bed — don't
   add events"; V17 thins the ARRANGEMENT:
   - BASS: the figures halve — 1/2/3 notes a bar (was 1/3/6), the walking
     pair is the workhorse, accents softened ~15%.
   - BELLS: the answer rings ONCE a bar (st 6; st 13 dropped), ~30% softer.
   - ARP: EARNED at deep reach now (wH > 2.2, was any core past 0.8) —
     the third voice arrives when the player commits, not by default.
   - LEAD: accent plucks fire only on loops GAINED — walking away no
     longer chatters ch7 — and sit ~40% softer.
   - The shared-ring event is ONE statement: the bell alone (the doubled
     bass hit is gone).
   STRINGS keeps the long tone (the featured voice, unchanged). Nothing
   visual changes; the pocket/SUMMON, the listening layer and the exprs
   are exactly V16. Nima's scene — Lance's verdict, say so in chat.
   (V16 notes below.) -------------------------------------------------- */
/* ---------- SRC-42.16 · RIDGE LOOM V16 (the DJ is both musicians)
   Lance's DJ-set round (second wave). Ridge's own metaphor — left hand
   = rhythm section, right hand = front line — maps onto a mix with
   eerie precision, so with a booth feed live (audioIn: true):
   THE BOOTH IS A FLOOR HAND: bass widens the three left cores, the
   mid/treble blend the three right, through the existing per-core
   threshold machinery (Math.max under the hands — a player always
   out-reaches the track). The topology, ridges and halos follow free.
   THE SHADOW RULE, structural: every count and every sounding read
   (nL/nR, the loop topology, figure velocities, arp depth) moves to a
   HAND-ONLY shadow width c.wH — the audit found c.w > 0.8 at eight
   call sites gating MIDI figures and the STRINGS re-sing, and an
   audio-jittered width would machine-gun them. Audio drives the
   PICTURE; only hands write the music.
   THE KICK IS THE BREATH: each real kick pumps s.breathEnv — the one
   scalar that already moves wave amplitude, brightness AND palette
   warmth — and while the booth is live the breath's decay quickens to
   ~0.45s so it reads as rhythm, not a swelling tide. MID adds a slow
   wave-amplitude swell on top.
   THE PALETTE CROSSING BELONGS TO presV: a playing booth runs the 6s
   two-state crossing like a playing human does.
   THE DROP: one deep breath-spike (1.4) — the weave lurches and warms.
   COURTESY: the SUMMON pocket kit ducks ~85% while audio is live (the
   DJ owns the rhythm), and the resting breath's bass tones wait for a
   genuinely empty room. Counters seeded on first sight; with no
   signal this is exactly V15. Nima's scene — Lance's round, say so in
   chat. (V15 notes below.) ------------------------------------------- */
/* ---------- SRC-42.15 · RIDGE LOOM V15 (the long tone finds its bow)
   The orchestration round (Lance: draft the Nine with ALL instruments
   in consideration). Ridge's own brief always said the first lead is
   "a long BOWED tone" -- and it was playing the felt piano. It sings
   on AUGMENTED STRINGS now (ch6): the right hand's first core raises a
   bowed voice, re-sung on each loop closed, while the plucky accents
   and the answering bell stay where they were. The felt piano keeps
   only the accents -- keys stay open as the hole a live player fills.
   (V14 notes below.) ------------------------------------------------ */
...
reg({
  id: 'SRC-42.17', family: 'SRC-42', ver: 17, title: 'Ridge Loom V17', tech: 'THE LOOM LISTENS / A QUIETER BAND',
  music: {
  ...
  fx: { bloom: 0.45 },
  tags: ['LEFT HAND = BASS', 'RIGHT HAND = LEADS', 'THE DJ IS BOTH MUSICIANS', 'REST BREATHES, NEVER TICKS'],
  desc: 'V16 with the band taught restraint (Lance: "too busy when it sends to Ableton... too intense"): bass figures halved to a breathing walk, the bell answers once a bar, the ARP is earned at deep reach instead of arriving by default, accents fire only on loops gained, and every event is one statement. The long bowed tone keeps the stage. V15, given ears: the booth is a floor hand — its bass widens the left cores, its top end the right, every kick pumps the breath through the whole weave, and a structural drop is one deep lurch. Audio drives the picture only; the music stays in your hands, and the pocket kit ducks while a DJ plays. With no signal, exactly V15. audioIn: true — ' + 'V10\'s palette and line, with the resting state rebuilt. An empty frame now genuinely rests: it settles to the violet flow the card always promised and BREATHES — soft low swells at irregular intervals, each one visibly pumping the whole weave and warming the frame a shade before it cools. The six-second two-state palette crossing (violet floor / orange summits into orange floor / cyan summits) now belongs to PRESENCE: it runs while someone is playing and eases home to violet when they leave, so the breath you see at rest is the breath you hear, never a clock. First touch snaps the scene awake; letting go unravels it slowly instead of dropping it.',
  ...
  sound: 'The playing instrument is V10\'s band with dynamics: every grid voice now derives its velocity from the gesture (core width and reach speed) with downbeat accents, the arpeggio\'s density is earned (quarters at low reach, eighths near full), and the shared-ring convergence is real — arp and bell degrees are pulled toward the long tone\'s pitch as rings enclose more cores, instead of a 2-cent detune nobody could hear. The RESTING state is rebuilt to the studio\'s idle law: the bar-clock bass root is gone; below presence the grid is silent and the scene breathes instead — randomized low swells (length, depth, voice and spacing all rolled per breath, roughly one in seven landing a deeper toll) over a loom-noise floor that undulates on two incommensurate slow LFOs, never dead, never periodic. Each breath pumps the wave field so the sound visibly moves the picture. Ableton: bass = the picture-written figures, the long bowed tone sings on STRINGS (ch6), lead keeps only the accents, ARP SYNTH holds the arpeggio\'s convergence note on the grid (CC74 = earned density; the patch arpeggiates), bells = the answers; the glue bed stays browser-side. THE POCKET (V13): the summons opens a ~45s window where a dry funk kit fades in on the real pads; pad 55 = the LOOP SEAT (drop a ~96 BPM funk loop there, ch1 CC74 fades it) - kick 36 (one, and-of-2, and-of-3), side-stick 38 backbeat, ranked hats 42, shaker 51 offbeats, open-hat 46 sigh - swung scene-side, velocity-ranked, perc CC74 = the window energy. Velocities carry the dynamics.',
  ...
  init(P) {
    ...
    for (const c of CORES) {
      ...
      c.wH = 0;   // V16: hand-only shadow width — counts and music read THIS
    }
    ...
    P.state = {
      ...
      breathEnv: 0,
      // V16: the listening layer
      aPres: 0, aBass: 0, aMid: 0, aTreb: 0, presV: 0, _kN: 0, _dropN: 0, _aSeeded: false,
      waves: [
  ...
  step(P, dt, t, inp) {
    ...
    if (au) {
      ...
      // THE KICK IS THE BREATH: the one scalar that already pumps wave
      // amplitude, brightness and palette warmth, back-dated per hit
      if (au.kick && au.kick.n !== s._kN) {
        ...
        if (hit > 0.05) s.breathEnv = Math.max(s.breathEnv, 0.85 * hit * Math.exp(-2.2 * (age + 0.030)));
      }
      // THE DROP: one deep lurch through the whole weave
      if (au.drop && au.drop.n !== s._dropN) {
        s._dropN = au.drop.n;
        if (s.aPres > 0.3) s.breathEnv = 1.4;
      }
    } else { s.aBass *= Math.pow(0.5, dt); s.aMid *= Math.pow(0.5, dt); s.aTreb *= Math.pow(0.5, dt); }
    s.presV = Math.max(s.pres, s.aPres);
    // while the booth is live the breath decays fast enough to read as
    // RHYTHM (~gone between 128bpm kicks); at rest it stays the long tide
    s.breathEnv *= Math.exp(-dt / (s.aPres > 0.5 ? 0.45 : 2.2));
    const HV = [clamp(inp.L), clamp(inp.R)];
    // V16: THE BOOTH IS A FLOOR HAND — bass under the left cores (the
    // rhythm section), the mid/treble blend under the right (the front
    // line), through the same per-core thresholds. Math.max: a player
    // always out-reaches the track.
    const HVa = [s.aBass * s.aPres, (0.45 * s.aMid + 0.55 * s.aTreb) * s.aPres];
    const TH = [[0, 0.28, 0.9], [0.32, 0.4, 0], [0.64, 0.36, 0]];
    let liveN = 0, nL = 0, nR = 0;
    for (const c of s.cores) {
      const x = HV[c.side], xa = HVa[c.side];
      const th = TH[c.slot];
      const wantOf = xx => (c.slot === 0 ? clamp(th[1] + xx * th[2]) : clamp((xx - th[0]) / th[1])) * s.wMax;
      const wantH = wantOf(x);
      const want = Math.max(wantH, wantOf(xa));
      const pwH = c.wH;
      // grow fast, shrink slower — the walk-away is an unravel, not a collapse
      c.w += (want - c.w) * Math.min(1, dt * (want > c.w ? 4.5 : 2.2));
      // the hand-only shadow: counts, topology and every sounding velocity
      // read THIS — an audio-jittered width must never write the music
      c.wH += (wantH - c.wH) * Math.min(1, dt * (wantH > c.wH ? 4.5 : 2.2));
      // reach speed, smoothed, HAND-only: the audio strikes harder for a
      // fast gesture, and the booth is not a gesture
      c.spd += (Math.abs(c.wH - pwH) / Math.max(dt, 1e-3) - c.spd) * Math.min(1, dt * 3);
      c.ph += dt * 0.3;
      c.x = c.hx * w + Math.sin(c.ph) * w * 0.012;
      c.y = c.hy * h + Math.cos(c.ph * 0.8) * h * 0.02;
      if (c.wH > 0.8) { liveN++; if (c.side) nR++; else nL++; }
    }
    ...
    // V16: the crossing belongs to presV — a playing booth runs it too
    s.mix = clamp(cross * s.presV + (1 - s.presV) * s.breathEnv * 0.22);
    for (let e = 0; e < 2; e++) {
    ...
    const wamp = 1 + s.breathEnv * 0.6 + 0.45 * s.aMid * s.aPres;   // V16: mid swells the waves
    ...
    for (let i = 0; i < 6; i++) {
      const ci = C[i];
      // V16: topology counts read the HAND-ONLY shadow width — loops write
      // the music (the long lead re-sings on a count change)
      if (ci.wH < 0.8) { if (ci.loops !== 0) { s.evq.push({ core: i, n: 0, up: false }); ci.loops = 0; } continue; }
      let sad = 0;
      for (let j = 0; j < 6; j++) {
        if (i === j) continue;
        const cj = C[j];
        if (cj.wH < 0.8) continue;
        const dx = cj.x - ci.x, dy = cj.y - ci.y;
        const v2 = (ci.wH + cj.wH) * Math.exp(-((dx * dx + dy * dy) / 4) / sig2);
        if (v2 > sad) sad = v2;
  ...
  draw(P, g, w, h, t, inp) {
    ...
    const bright = (0.5 + (s.presV || s.pres) * 0.5) * s.dens * (1 + s.breathEnv * 0.25);
    ...
    g.fillText('BASS ' + (resting ? '—' : BASSN[s.nL] + ' (' + s.nL + ')') +
      ...
      (resting ? '   · RESTING (BREATHES)' : '') +
      (s.aPres > 0.3 ? '   ♪ AUD b' + Math.round(s.aBass * 100) + ' t' + Math.round(s.aTreb * 100) + ' breath ' + Math.round(s.breathEnv * 100) : ''), 10, h - 10);
  },
  ...
  audio(A, P) {
    ...
    // V17: the figures HALVED (Lance: "too busy... too intense") — the
    // walking pair is the workhorse, three notes is the ceiling, accents
    // softened. The left hand still gets busier per core; it just breathes.
    const BASS = [
      [],
      [[0, 0, 1.1]],
      [[0, 0, 1.15], [8, 1, 0.95]],
      [[0, 0, 1.15], [6, 1, 0.9], [10, 0, 0.85]]
    ];
    ...
    return {
      tick(inp, dt) {
        ...
        /* ---- the resting breath: randomized swells, never a metronome --
           V16: waits for a genuinely EMPTY room — while the booth is live
           the visual breath comes from the real kicks instead, and the
           scene must not play bass tones over the DJ's track. */
        if (s.pres < 0.3 && s.aPres < 0.3) {
          if (now >= nextBreath) {
        ...
        // never starts or stops (the Rain gate law). V16: the pocket ducks
        // ~85% while the booth is live — the DJ owns the rhythm.
        jam += (sumA * (1 - 0.85 * s.aPres) - jam) * Math.min(1, dt * (sumA > jam ? 1.2 : 0.35));
        ...
        while (nextT < horizon && guard++ < 24) {
          ...
          if (gridOn) {
            ...
            for (const [step, coreI, acc] of fig) {
              ...
              if (!c || c.wH < 0.8) continue;   // V16: sounding reads = hand shadow
              const dur = s.nL > 2 ? 0.5 : s.nL > 1 ? 0.9 : 2.4;
              A.bassNote(bassToneOf(coreI), {
                at: nextT, vol: (0.1 + clamp(c.wH / 5) * 0.06) * acc * spdAcc(c) * gate, dur
              });
            }
          }
          ...
          const c4 = s.cores[4];
          if (gridOn && c4 && c4.wH > 0.8 && st % 2 === 0) {
            // density is EARNED: quarters at low reach, eighths near full
            const depth = clamp((c4.wH - 0.8) / (s.wMax - 0.8));
          ...
          const c5 = s.cores[5];
          // V17: the answer rings ONCE a bar, softer (st 13 dropped)
          if (gridOn && c5 && c5.wH > 0.8 && st === 6) {
            const own = c5.deg + c5.loops + 4;
            const deg = Math.round(own + (tgt - own) * conv * 0.7);
            const acc = spdAcc(c5);
            A.bell(H.chordTone(deg, 1), {
              at: nextT, vol: (0.014 + clamp(c5.wH / 5) * 0.011) * acc * gate, dur: 2.4, pan: 0.25, rev: 0.75
            });
          }
        ...
        if (typeof MOut !== 'undefined') {
          const c4a = s.cores[4];
          // V17: the ARP is EARNED at deep reach (was any core past 0.8) —
          // the third voice arrives when the player commits, not by default
          const arpOn = gridOn && c4a && c4a.wH > 2.2;
          if (arpOn) {
            const depth = clamp((c4a.wH - 2.2) / (s.wMax - 2.2));
        ...
        if (gridOn && c3 && c3.wH > 0.8 && c3.loops !== lastLoops[3]) {
          lastLoops[3] = c3.loops;
          const f = toneOf(3, 0);
          const at = T.next(0.5);
          A.tone(f, {
            at, vol: (0.05 + clamp(c3.wH / 5) * 0.025) * spdAcc(c3) * gate,
            dur: 3.4, attack: 0.5, type: 'triangle',
            pan: 0.5, rev: 0.6, del: 0.1, role: 'strings'
          });
        ...
        while ((ev = s.evq.shift()) && k < 4) {
          ...
          // V17: accents fire only on loops GAINED — walking away no longer
          // chatters the rack — and each event is ONE statement.
          if (!ev.up) continue;
          if (ev.core < 0) {
            // rings that enclose more than one core: the bell alone now
            // (the doubled bass hit was two instruments on one event)
            A.bell(H.chordTone(ev.n + 2, 1), { at, vol: 0.038 * gate, dur: 3.2, pan: 0, rev: 0.78 });
            continue;
          }
          const c = s.cores[ev.core];
          if (c.side === 0) {
            // the floor moved
            A.bassNote(bassToneOf(ev.core), { at, vol: 0.07 * spdAcc(c) * gate, dur: 1.4 });
          } else if (ev.core !== 3) {
            A.pluck2(toneOf(ev.core, 0), {
              at, vol: 0.027 * spdAcc(c) * gate, dur: 1.1,
