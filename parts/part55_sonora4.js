/* ---------- SRC-32.4 · SONORA V4 (stone pan) ---------- */
/* The stone bed comes back, but it is not the tumbled gem shop of V2.
   These are field stones — light grey, dry, near-monochrome, each one a
   different geology. The pressure fronts and the relaxation solver are
   V3's, unchanged: the pack still grinds rather than knocks. What is new
   is HEAT. Every stone carries an excitation that rises as it is worked
   and cools over seconds, and heat is what you hear: a cold stone knocks
   like a rock, a worked one has warmed into a handpan. */
reg({
  id: 'SRC-32.4', family: 'SRC-32', ver: 4, title: 'Sonora', tech: 'STONE PAN / SIDE PRESSURE',
  music: { bpm: 60, root: 48, mode: 'ionian', prog: [0, 5, 3, 4], chordBars: 4 },
  fx: { bloom: 0.32, edge: true },
  tags: ['GREY STONE / FOUR GEOLOGIES', 'INVISIBLE PRESSURE FRONTS', 'EXCITATION HEAT', 'ROCK WARMS INTO HANDPAN'],
  ...
  interact: 'The stones roam, drift, and touch. Your hands are two pressure fronts you never see: raise L and the west front advances, raise R and the east answers, and the bed is herded between them. Each front runs 40% of the way in, so at full squeeze they have claimed 80% of a tank the stones cannot possibly fit inside — they pile, bulge back out through the fronts, and tremble there. That trembling is the instrument. Sustained pressure does not produce a faster clatter, it produces GRIND: stone sliding on stone, a friction bed under everything, and every contact feeding heat into the stones it touches. Squeeze and hold, and you can hear the bed warm up — dry at first, then ringing, then singing. Let go and it cools. The slow play is the sympathy: a stone in tune with something already sounding stays warm long after you stopped working it, so the field holds its brightness in patches you did not touch.',
  ...
  init(P) {
    ...
    /* ---- C MAJOR PENTATONIC · degree carries geology, value and name ----
       ...
       This is the same discipline the dye bowls used with Werner's chips —
       the look tells you the note — with rock standing in for pigment. */
  ...
  step(P, dt, t, inp) {
    ...
    /* ---- retune. The pentatonic is tied to the harmony engine, so when H
           modulates the whole bed shifts. A handpan cannot do this; we do it
           anyway, and we make it audible — the stones sound their new pitches
           in size order over about a second, low to high, so you hear the pan
           being tuned rather than just noticing it already happened. ---- */
    ...
    /* Two pressure fronts, invisible, one per hand. Not barriers: each runs
       40% of the way in, and at full squeeze they have claimed 80% of a tank
       the stones cannot fit inside. They pile, bulge back through, tremble. */
    const TRAVEL = 0.40;
    ...
    for (let i = 0; i < s.rocks.length; i++) {
      for (let j = i + 1; j < s.rocks.length; j++) {
        ...
        if (d < min && d > 0.01) {
          ...
          if (rel < 0) {
            /* Mostly INELASTIC. These are heavy stones meeting in a pile:
               they touch, they lean, they part. Nothing bounces off. Under
               sustained pressure the pack grinds — which is what the friction
               bed in audio() is carrying. */
            const imp = -rel * 0.30 + 3;
            ...
            const speed = -rel;
            /* WORK. Every contact loads the stones it touches, including the
               ones far too soft to sound — a bed trembling under the fronts is
               being worked hard even while it goes comparatively quiet. This is
               a LOAD, not a heat: it is summed over the frame and then drives
               heat through a time constant below, so warming takes the seconds
               it should instead of depending on how many contacts a frame
               happens to contain. */
            ...
            if (speed > SOUND_FLOOR) {
              ...
              // a handful at once — the grind does not, it just dusts
              ...
              for (const st of [a, b]) {
                ...
                /* An enforced rest per stone, so a trembling pack reads as the
                   ...
                   with heat, and it has to: a cold stone's knock is gone in a
                   fifth of a second, but a hot one rings for four, and striking
                   it again underneath its own tail is what turns an instrument
                   back into noise. Warm stones speak less often and say more. */
    ...
    /* ---- HEAT. A first-order lag on how hard the stone is actually being
           worked right now, so it is framerate-independent and the arc takes
           the seconds it should: about five to warm through, four to cool, and
           as much as fourteen if something is singing to it. That last term is
           the whole sympathy payoff — a stone in tune with the bed holds its
           brightness long after you stopped touching it, so the field keeps
           warm patches you never worked. It cannot run away: sympathy never
           fires a note, so nothing feeds back into the bus. ---- */
    for (const st of s.rocks) {
  ...
  draw(P, g, w, h, t, inp) {
    ...
    /* MINERAL GRAIN — one seamless tile, built once, shared by every stone.
       Coarser and drier than the pigment tile the dye bowls used: rock is
       granular at every scale, and the tooth is what stops a grey disc from
       reading as a grey disc. */
    if (!s.grain) {
      const SZ = 128;
    ...
    /* Bake each stone's surface once, in its own local frame. It is drawn
       rotated afterwards, so the geology turns with the stone the way a real
       surface does — which is the whole reason these are baked and not
       procedurally redrawn every frame. */
    const bake = (st) => {
      ...
      /* FORM. Everything above is surface; this is what makes it a solid.
         ...
         lower right, and a dark inner rim so the silhouette has thickness. */
      ...
      dg.addColorStop(0.5, 'rgba(6,6,9,0.2)');
      dg.addColorStop(1, 'rgba(4,4,7,0.62)');
      q.fillStyle = dg;
      q.fillRect(-size, -size, size * 2, size * 2);

      // inner rim — dark all round, brightest catch on the upper left edge
      q.lineWidth = Math.max(1, u * 0.10);
      q.strokeStyle = 'rgba(6,6,9,0.5)';
      rockPath(q, sh, u, 1); q.stroke();
      q.restore();
    ...
    /* Contact rings — a dry shock through the bed, not a ripple in water.
       Kept very faint and very short: under a full crush there are a dozen
       contacts a second, and at any real opacity the expanding circles stack
       into moiré that reads as a CRT artefact rather than as stone. */
    ...
    for (const st of s.rocks) {
      ...
      if (heat > 0.02 || sym > 0.05 || st.shock > 0.03) {
        ...
        /* Everything below is INSIDE the stone. Nothing is allowed to draw a
           halo around the silhouette — an outline is the single loudest tell
           that a thing is a shape rather than an object, and a bed of stones
           each wearing a bright rim reads as stickers on black. */
        ...
        // heat — the stone glows along its own edge, in its own shape, and
        // carries a standing bloom that follows the silhouette rather than a
        // circle, because a circle inside a rock reads as a drawn circle
        if (heat > 0.02) {
        ...
        /* The shock of a strike, travelling out from the core. Squared falloff
           and a thin line on purpose: under a full crush most of the bed has
           been struck in the last half second, and at any real weight these
           become a contour map drawn over every stone. */
        if (st.shock > 0.06) {
  ...
  audio(A, P) {
    ...
    /* GRIND. Under sustained pressure the stones are not knocking, they are
       ...
       a dry rasp of grit between them. This carries the intensity so the
       scene does not have to fire more discrete notes at it. */
    const rum = v.noise();
    ...
    return {
      tick(inp) {
        ...
        /* Publish the drone so other scenes' resonators can hear this one —
           but an OCTAVE BELOW the lowest stone, and quietly. Pushed at the
           root itself it lands in unison with the bottom stone and pins that
           stone's sympathy at full for as long as the scene is open, which
           reads as a permanent haze over a bed nobody is touching. An octave
           down couples at half, which is audible without pinning anything. */
        const now = A.t();
