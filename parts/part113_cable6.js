/* ---------- SRC-13.6 · EVENT HORIZON V6 ---------- */
/* Lance's V5 clarification: the gradient he wanted longer was the RIM BELOW
   the line (V5 brightened the sky above instead). The dying red bleed into
   the void now reaches ~2.5x deeper, and stretches further as the rope
   rises. */
reg({
  id: 'SRC-13.6', family: 'SRC-13', ver: 6, title: 'Event Horizon', tech: 'VERLET ROPE / ACCRETION',
  music: {
  ...
  draw(P, g, w, h, t, inp) {
    ...
    const gpulse = s.gate;
    const glow = (0.10 + s.tension * 0.28 + gpulse * 0.10 + s.chg * 0.25) * (0.4 + s.pres * 0.6);
    // a high, taut rope LIGHTS THE SPACE: the glow reaches further and holds
    // instead of dissipating — at full it washes the whole sky
    const reach = 0.38 + s.tension * 0.5;
    const skyBot = Math.min(h, topY + h * 0.15);
    const sky = g.createLinearGradient(0, topY - h * reach, 0, skyBot);
    sky.addColorStop(0, `rgba(255,80,90,${glow * 0.18 * s.tension})`);
    sky.addColorStop(0.72, `rgba(255,110,55,${glow * (0.5 + s.tension * 0.4)})`);
    ...
    for (let k = 4; k >= 1; k--) {
      ridge(-k * 7 * ms, `rgba(255,140,70,${glow * 0.28 * (1 + s.tension * 0.5) * (1 - k / 4.6)})`, 10 * ms);
    }
    // the rim below the line — LONG. The red bleeds deep into the void and
    // reaches deeper still as the rope rises.
    const rimA = 0.24 + s.tension * 0.14 + s.chg * 0.3;
    const NL2 = 16, rstep = (9 + s.tension * 9) * ms;
    for (let k = NL2; k >= 1; k--) {
      const f = k / (NL2 + 1);
      // stroke width rides the step so the layers always overlap — a gap
      // between them reads as stripes, and stripes are the old bar chart
      ridge(k * rstep, `rgba(${255 - f * 130 | 0},${42 - f * 30 | 0},20,${rimA * Math.pow(1 - f, 1.5)})`, rstep * 1.5);
    }
