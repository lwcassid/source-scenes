/* ---------- SRC-65 · L-FLOWER STUDY (after Dan Hoopert's "L-Systems Flower") ----------
...
(() => {
  ...
  const RIB_VS = GLSL_SWAY + GLSL_RAMP + GLSL_COC + `
    ...
    attribute float aW; attribute float aK; attribute float aJ;
    ...
    void main(){
      ...
      vCol = c;
      vA = uFade * (aK < 0.5 ? 0.85 : 0.38) * clamp(base / wpx, 0.06, 1.0);
      vSide = aC.y;
    }`;
  const RIB_FS = `
    varying vec3 vCol; varying float vA; varying float vSide;
    void main(){
      ...
      gl_FragColor = vec4(vCol * vA * e, 1.0);
    }`;
  ...
  const PT_VS = GLSL_SWAY + GLSL_RAMP + GLSL_COC + `
    ...
    varying vec3 vCol; varying float vA; varying float vBlur; varying float vK; varying float vRot;
    void main(){
      float age = uG - aT;
      ...
      float base = min(26.0, max(1.2, aS * uProj / z * uPt)) * (0.35 + 0.65 * pop) * (1.0 + 0.6 * flash);
      float coc = cocPx(z);
      float s = base + coc;
      gl_PointSize = s;
      float a = age / uSpan + aJ * 0.12;
      vec3 c = aK > 1.5 ? vec3(1.0, 0.97, 0.90) : (aK > 0.5 ? mix(ramp(a), vec3(1.0), 0.45) : ramp(a));
      c = mix(c, vec3(1.0, 0.98, 0.9), flash * 0.8);
      vCol = c;
      float energy = clamp((base * base) / (s * s), 0.02, 1.0);
      float k = (aK > 1.5 ? 0.55 : (aK > 0.5 ? 0.6 : 0.20)) * uPtGain;
      vA = uFade * k * pop * energy * (1.0 + 2.0 * flash) * mix(1.0, 1.9, smoothstep(2.0, 14.0, coc));
      vA *= pow(clamp(uFocus / z, 0.45, 1.25), 1.6) * smoothstep(0.12, 0.45, z);   // shade behind, and nothing smears the lens            // the far side of the cloud falls into shade
      vBlur = clamp(coc / max(base, 1.0) / 3.0, 0.0, 1.0);
      vK = aK; vRot = aJ * 6.2831;
    }`;
  const PT_FS = `
    varying vec3 vCol; varying float vA; varying float vBlur; varying float vK; varying float vRot;
    void main(){
      ...
      if (vK > 1.5) {                              // glyph: hollow square
        ...
        m *= 1.0 + vBlur * 0.55 * smoothstep(0.55, 0.92, d);  // bokeh rim
        m *= mix(0.75 + 0.25 * smoothstep(0.5, 0.0, d), 1.0, vBlur); // sharp: brighter heart
      }
      if (m <= 0.002) discard;
  ...
  // ---------- lens pass ----------
  const LENS = {
    uniforms: { tDiffuse: { value: null }, uT: { value: 0 }, uCA: { value: 0.0035 }, uExpo: { value: 1.35 },
  ...
  function buildGeo(plant) {
    ...
    const aW = new Float32Array(NS * 4), aK = new Float32Array(NS * 4), aJ = new Float32Array(NS * 4);
    ...
    const pos = new Float32Array(NP * 3), pt = new Float32Array(NP), ps = new Float32Array(NP),
          pj = new Float32Array(NP), pk = new Float32Array(NP);
    for (let i = 0; i < NP; i++) {
      const p = Pp[i];
      pos.set(p.p, i * 3); pt[i] = p.t; ps[i] = p.s; pj[i] = p.j; pk[i] = p.k;
    }
  ...
  function newPlant(P, T3) {
    const s = P.state;
    if (T3.rib) { T3.scene.remove(T3.rib); T3.rib.geometry.dispose(); }
    if (T3.pts) { T3.scene.remove(T3.pts); T3.pts.geometry.dispose(); }
    const plant = growPlant(P.rand);
    const geo = buildGeo(plant);
    T3.rib = new THREE.Mesh(geo.rg, T3.ribMat); T3.rib.frustumCulled = false;
    T3.pts = new THREE.Points(geo.pg, T3.ptMat); T3.pts.frustumCulled = false;
    T3.scene.add(T3.rib); T3.scene.add(T3.pts);
  ...
  reg({
    id: 'SRC-65', family: 'SRC-65', ver: 1,
    title: 'L-Flower Study', tech: 'WEBGL / GPU GROWTH REVEAL + SCATTER DEPTH OF FIELD',
    tags: ['STUDY', 'LOOK-DEV', 'WEBGL', 'DEPTH OF FIELD', 'AFTER DAN HOOPERT'],
    desc: 'A look-development study after Dan Hoopert\'s "L-Systems Flower": one green stem rises from below the frame, opens an umbel at its top, and then a cloud of thousands of florets branches out beneath it and settles, their colour drifting with age from bud green to cream, pink, lilac and finally blue. Everything is seen through a lens: each floret and filament blurs by its own distance from the focal plane, so the far side of the cloud dissolves into bokeh discs while the near side stays sharp, and the focus breathes, hunts and racks on its own. When the plant is full it holds, dissolves, and a new one grows from a new seed.',
    ...
    init(P) {
      ...
      T3.u = common;
      const mk = (vs, fs) => new THREE.ShaderMaterial({
        uniforms: common, vertexShader: vs, fragmentShader: fs,
        transparent: true, depthTest: false, depthWrite: false,
        blending: THREE.AdditiveBlending, side: THREE.DoubleSide
      });
      T3.ribMat = mk(RIB_VS, RIB_FS);
      T3.ptMat = mk(PT_VS, PT_FS);
      ...
      T3.bloom = new THREE.UnrealBloomPass(new THREE.Vector2(T3.rw, T3.rh), 0.55, 0.6, 0.55);
    ...
    draw(P, g, w, h, t, inp) {
      ...
      u.uPtGain.value = 1.0 - 0.62 * ze;          // exposure: inside the cloud there is far more of it on screen
