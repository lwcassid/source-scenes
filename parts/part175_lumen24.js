...
const LUMEN24_STOPCACHE = new Map();
reg({
  id: 'SRC-38.24', family: 'SRC-38', ver: 24, title: 'Lumen Film V24', tech: 'GEM FILM / APERTURE ORGAN',
  music: {
  ...
  draw(P, g, w, h, t, inp) {
    ...
    const addSvgStops = (grad, a) => {
      let set = LUMEN24_STOPCACHE.get(a);
      if (!set) {
        set = SVGSTOPS.map(([t, rgb]) => [t, `rgba(${rgb},${a})`]);
        LUMEN24_STOPCACHE.set(a, set);
      }
        ...
        if (s.pres < 0.25) {
          if (now >= nextBreath) {
            ...
            if (deep) {
              ...
              MOut.evNote('shrine', H.chordTone(Math.random() < 0.5 ? 0 : 2, 0), 0.05 + Math.random() * 0.04, now + 0.05, 1.2);
            } else if (pick < 0.7) {
              // a barely-there choir root
              MOut.evNote('choir', H.rootFreq(0), 0.04 + Math.random() * 0.03, now + 0.05, 3.5);
            } else {
