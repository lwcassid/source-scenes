/* ---------- SRC-10.11 · WEATHER STATION V11 (one great wrapping form) ---------- */
reg({
  id: 'SRC-10.11', family: 'SRC-10', ver: 12,
  title: 'Weather Station V11', tech: 'WIND + GROWING ATTRACTOR / ENGULFING CHAOS',
  music: { bpm: 60, root: 43, mode: 'aeolian', prog: [0, 5], chordBars: 4 },
  fx: { bloom: 0.7, edge: true },
  tags: ['ATTRACTOR GROWS WITH THE HAND', 'LAYERED ORBITS', 'ENGULFING AT PEAK', 'WARM AT PEAK CHAOS'],
  desc: 'Two poles drift inside the wind, and every thread near one settles into its own orbit — some tight, some wide, layered like contour lines wrapping a form that is not quite there. At rest the poles are small, barely felt. Raise your right hand and they swell — not gently, hard — until at full reach they own most of the visible sky, and nearly every thread in the field is caught in the same great wrapping motion. Cool white and violet is home; at the very peak the core of the weave catches fire, coral and gold.',
  interact: 'L = heading, same as always — the field snaps to a new direction instantly. R = the poles themselves: small and easy to ignore at rest, ballooning fast as you raise the hand, until at full reach the whole sky is engulfed in one wrapping, layered, orbiting mass. Low R is still just a scatter of tiny drifting marks.',
  ...
  _spawnGust(P, w, h) {
    return {
      ...
      baseR: Math.sqrt(w * h) * (0.12 + P.rand() * 0.08),
      r: 0,
      life: 0, maxLife: 7 + P.rand() * 9,
      wander: P.rand() * TAU, wSpeed: 12 + P.rand() * 26, str: 0,
      streamAngle: (P.rand() - 0.5) * 0.8, wobPh: P.rand() * TAU, wobFr: 0.08 + P.rand() * 0.18, wobAmp: 0.15 + P.rand() * 0.2
    };
  },
  _hash(a, b) {
  ...
  init(P) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      mass.push({
        ...
        warmJ: (P.rand() - 0.5) * 70, ph: P.rand() * TAU, seed: P.rand() * 1000, orbitFrac: 0.12 + P.rand() * 0.6,
        curlPh: P.rand() * TAU, curlFr: 0.4 + P.rand() * 1.7, curlAmp: 0.5 + P.rand() * 1.3, boost: 0
      });
    }
    const NG = 2, gusts = [];
    for (let i = 0; i < NG; i++) { const gu = this._spawnGust(P, w, h); gu.life = P.rand() * gu.maxLife; gusts.push(gu); }
    P.state = { mass, gusts, head: 0, spin: 0, turnPulse: 0, hue: 200, dens: 0, gust: 0, warmT: 0, pres: 0 };
  },
  step(P, dt, t, inp) {
    ...
    for (let i = 0; i < s.gusts.length; i++) {
      ...
      const envelope = Math.sin(Math.min(1, k) * Math.PI);
      gu.str = Math.max(envelope, chaos * 0.7); // never lets peak chaos go dormant, even between blooms
      gu.r = gu.baseR * (1 + growT * 6.5); // small and easy to miss at rest, can exceed the frame at full reach
      gu.wobPh += dt * gu.wobFr;
      ...
      if (gu.life >= gu.maxLife) { const nx = gu.x, ny = gu.y; Object.assign(gu, this._spawnGust(P, w, h)); gu.x = nx; gu.y = ny; }
    }

    const base = clamp(dens * 1.05); // right hand is the only thing that lets lines start growing
    for (const m of s.mass) {
      // which pole (if any) dominates this thread, how deep inside it, and the layered orbital pull it exerts
      let boost = 0, angBias = 0, pullX = 0, pullY = 0;
      for (const gu of s.gusts) {
        const dx = gu.x - m.x, dy = gu.y - m.y, d = Math.sqrt(dx * dx + dy * dy) || 1;
        if (d < gu.r) {
          const gb = gu.str * (1 - d / gu.r);
          if (gb > boost) { boost = gb; angBias = gu.streamAngle + Math.sin(gu.wobPh) * gu.wobAmp; }
          const nx = dx / d, ny = dy / d;
          const orbitR = gu.r * m.orbitFrac; // each thread settles onto its own ring — layered, not one flat spiral
          const radial = (d - orbitR) * gb * chaos * 3.2;
          const tangential = gb * chaos * spd0 * 1.4;
          pullX += nx * radial - ny * tangential;
          pullY += ny * radial + nx * tangential;
        }
      }
      m.boost = boost;
      ...
      // independent per-thread wobble — texture, not the dominant motion
      m.curlPh += dt * m.curlFr * (1 + chaos * 2.2);
      const curl = Math.sin(m.curlPh) * m.curlAmp + Math.sin(m.curlPh * 1.8 + 1.7) * m.curlAmp * 0.5;

      const a = heading + angBias * boost + turb * curl * 0.4
        + 0.5 * turnCoherence * Math.sin(m.x * 0.01 + t * 0.6 + m.ph)
        + 0.5 * turnCoherence * Math.cos(m.y * 0.012 - t * 0.5 + m.ph);
      m.vx += (Math.cos(a) * spd0 - m.vx) * Math.min(1, dt * 8) + pullX * dt;
      m.vy += (Math.sin(a) * spd0 - m.vy) * Math.min(1, dt * 8) + pullY * dt;
  ...
  draw(P, g, w, h, t) {
    ...
    for (const m of s.mass) {
      ...
      const baseA = (0.24 + lenFrac * 0.5 + m.boost * 0.32) * pres;
      const baseW = (0.55 + m.widP * 0.55 + m.boost * 0.4) * ms;
      const gr = g.createLinearGradient(trail[0].x, trail[0].y, trail[N - 1].x, trail[N - 1].y);
      gr.addColorStop(0, `hsla(${hue},${sat}%,${42 + warmMix * 8}%,0)`);
      gr.addColorStop(0.6, `hsla(${hue},${sat}%,${76 - warmMix * 16}%,${baseA})`);
      gr.addColorStop(1, `hsla(${hue},${Math.max(0, sat - 8)}%,${92 + m.boost * 5 - warmMix * 26}%,${Math.min(1, baseA * 1.3)})`);
      g.strokeStyle = gr;
      g.lineCap = 'round';
