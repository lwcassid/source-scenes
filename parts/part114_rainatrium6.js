/* ---------- SRC-16.6 · RAIN ATRIUM V6 (sunset-storm line) ---------- */
reg({
  id: 'SRC-16.6', family: 'SRC-16', ver: 6, title: 'Rain Atrium V6', tech: 'TWO-FIELD WATER / NOTE ECONOMY',
  music: {
  ...
  tags: ['WAVE INTERFERENCE', 'FELT PIANO', 'FLICK = PLACED DROP', 'GUSTS & LULLS', 'EARNED GROOVE'],
  desc: 'The pool learns restraint. Most rain is just water now — splash, rings, the hush of the bed — and a piano note is something a drop EARNS: the big ones, the gust peaks, a couple a second at the very most, each dry enough to end before the next begins. Gusts finally include their opposite: real lulls where a side goes almost quiet and the room can breathe. FLICK a hand outward to place a drop yourself — bigger, lower, louder, exactly where your side\'s storm cell sits — so between the weather\'s phrases you can answer with your own. The two ring systems wander toward the middle more often, arguing in rose; the palette drifts a little further with each chord.',
  ...
  sound: 'Felt piano with a note budget. Drops mostly sound as water (granulated bed, no synth); a note is earned by size or luck, capped globally near two a second, register centered on one mid-high octave with rare highs, big drops anchoring low. Small notes are nearly DRY — the long reverb is saved for chord rolls, thunder and petrichor, so those read as moments. Past ~three-quarter downpour the piano yields almost fully to weather: bed + groove carry it, which is what a downpour is. The earned groove now moves — bass walks to a color tone every fourth bar, the backbeat kick breathes, a turnaround tick at bar\'s end — still gone entirely in the small state. Flick = one placed note that cuts through. Lulls duck the patter so quiet is really quiet. Pinned key, empty mids, D pedal underneath.',
  ...
  step(P, dt, t, inp) {
    ...
    const tryDrop = (side, dens) => {
      ...
      if (big) s.lastBass = t;
      const xh = (xf - lo) / (hi - lo);                              // position within the half
      const deg = 2 + Math.floor(xh * 6) + (P.rand() < 0.15 ? 3 : 0); // one home octave, rare highs
      P.ping(A => {
        ...
        P.def._felt(A, big ? H.chordTone(deg % 4, 0) : H.chordTone(deg, 1), {
          at, vol: big ? vol * 1.6 : vol, pan: xf * 2 - 1, dur: big ? 2.4 : 1.3, big
        });
      });
    };
    ...
    for (const [side, flick] of [[0, flickL], [1, flickR]]) {
      ...
      P.ping(A => {
        P.def._felt(A, H.chordTone(1 + (P.rand() * 3 | 0), 0), {
        ...
        while (nextT < horizon) {
          ...
          if (idx > lastIdx) {
            ...
            if (gate > 0.03) {
              ...
              if (st === 0) {
                A.kick(at, (0.06 + 0.13 * gate) * (0.9 + Math.random() * 0.2));
                A.bassNote(H.chordTone(bar % 4 === 3 ? 1 : 0, -1), { at, vol: 0.05 + 0.09 * gate, dur: 1.6 });
              }
