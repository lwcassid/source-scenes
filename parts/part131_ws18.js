/* ---------- SRC-10.18 · WEATHER STATION V18 (the form learns articulation) ---------- */
/* Self-critique round on V17 against the five listening tests:
   1. ARTICULATION - the voice only glided; a melody with no attack is
      mush. Each ladder step the form crosses now lands a soft pluck
      (cooldown 0.15s, velocity from density) over the continuing glide -
      you FEEL every note you fly. Real notes on lead ch1 for Ableton.
   2. THE COMB WAS OUT OF TUNE - V17 pulled rings halfway to the lead
      (between chord tones = off-key smear). Now full unison, and the
      wind dips while the field is straight - less visible turbulence,
      less audible turbulence.
   3. MIX - the wind buried the voice at gale (featured-layer law). Wind
      trimmed, voice raised.
   4. UNDULATION - was a 3Hz flutter at full gust; now a 0.3-0.9Hz breath.
   5. Chord changes get the quiet low-to-high roll when played. */
reg({
  id: 'SRC-10.18', family: 'SRC-10', ver: 19,
  title: 'Weather Station V18', tech: 'DENSITY SMOKE / THE SINGING FORM',
  music: {
  ...
  audio(A, P) {
    ...
    v.fadeIn(1, 1);
    let comb = 0, lastNow = A.t(), lastDeg = -1, lastPluck = -9;
    ...
    return {
      tick() {
        ...
        // the bow
        A.set(ng.gain, ((0.012 + dens * 0.055 + tp * 0.02) * pres + 0.03 * idleAmb) * (1 - comb * 0.3), 0.25);
        ...
        if (deg !== lastDeg && pres > 0.2 && now - lastPluck > 0.15) {
          lastPluck = now;
          A.pluck2(vf0, {
            vol: (0.028 + dens * 0.05) * (0.8 + Math.random() * 0.4),
