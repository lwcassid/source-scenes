/* ---------- SRC-15.12 · FERRO BLOOM V12 (warm↔cool mood sequencing, richer per-flower gradients) ---------- */
reg({
  id: 'SRC-15.12', family: 'SRC-15', ver: 12,
  title: 'Ferro Bloom V12', tech: 'SHADER FIELD / MOTION ILLUSION',
  music: {
  ...
  desc: 'Color no longer splits the frame by hand — every flower can be any temperature at once, mixed throughout the composition. What ties them together is a slow shared mood: the whole garden breathes from warm toward cool and back over several minutes, and each flower\'s own hue jitters around that shared center, so the palette always reads as "the garden right now" rather than a scatter of unrelated colors. Within a single flower, hue drifts across its rings again — the richer, more painterly gradient banding V9 had — instead of holding one flat tone. Ember orange and blush pink anchor the warm end, teal/lavender/electric blue the cool end, chartreuse the pivot between them. Hero keeps its clean six-petal silhouette from V10; dots stay gone. Music unchanged: a three-layer soundscape — C-minor pedal drone whose color shifts, a quantized groove that only exists once the garden is thrown wide open, and a reactive layer where every willed bloom, flick, and stillness sounds back at you.',
  ...
  init(P) {
    ...
    const mat = new THREE.ShaderMaterial({
      ...
      fragmentShader: [
        ...
        '}',
        // V12: the whole garden shares one slow "mood" — a warm↔cool center
        // that breathes over several minutes, independent of hand position.
        // Every flower's hue jitters around THIS shared center, so the
        // palette stays mixed moment to moment but the overall feel drifts
        // together instead of splitting the frame by side.
  ...
  draw(P, g, w, h, t, inp) {
    ...
    if (s.noGL || !P._three) {
      ...
      g.textAlign = 'center';
      g.fillText('FERRO BLOOM V12 · MOOD SEQUENCE', w / 2, h / 2 - 12);
