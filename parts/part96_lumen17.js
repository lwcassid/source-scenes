/* ---------- SRC-38.17 · LUMEN FIELD (density holds at any shape) ---------- */
...
const LUMEN17_STOPCACHE = new Map();
reg({
  id: 'SRC-38.17', family: 'SRC-38', ver: 17, title: 'Lumen Film', tech: 'GEM FILM / APERTURE ORGAN',
  music: {
  ...
  init(P) {
    ...
    const R_LO = 0.0153, R_HI = 0.0632, RD = R_HI - R_LO;
  ...
  draw(P, g, w, h, t, inp) {
    ...
    const addSvgStops = (grad, a) => {
      let set = LUMEN17_STOPCACHE.get(a);
      if (!set) {
        set = SVGSTOPS.map(([t, rgb]) => [t, `rgba(${rgb},${a})`]);
        LUMEN17_STOPCACHE.set(a, set);
      }
