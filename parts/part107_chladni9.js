/* ---------- SRC-28.9 · CHLADNI COURT V9 ---------- */
/* V8's two-square layout (fills the 16:10 frame, two full-height squares meeting
   at a mirror seam), but the grains read SHARP and grainy instead of blurry:
   bloom pulled back, the fade-trail dropped (full clear each frame, so a moving
   grain is a crisp square, not a smeared streak), the additive glow pass
   removed, grains drawn more opaque, and pixel-crisp blits. */
reg({
  id: 'SRC-28.9', family: 'SRC-28', ver: 9, title: 'Chladni Court', tech: 'STANDING WAVES / SAND ON A PLATE',
  ...
  draw(P, g, w, h, t, inp) {
    ...
    const sz = Math.max(2.6, m * 0.0044) * (1 + warm * 0.4);
