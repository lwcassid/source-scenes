/* ---------- SRC-42.4 · RIDGE LOOM V4 (one line weight, elevation is the colour) ---------- */
reg({
  id: 'SRC-42.4', family: 'SRC-42', ver: 4, title: 'Ridge Loom V4', tech: 'CONTOUR HEAT / VIOLET FLOOR, ORANGE SUMMITS',
  music: {
  ...
  fx: { bloom: 0.45 },
  tags: ['LEFT HAND = BASS', 'RIGHT HAND = LEADS', 'HEIGHT IS THE COLOUR', 'ONE LINE WEIGHT'],
  desc: 'V3\'s loom exactly — same flow, same six cores, same music — rebuilt as a proper contour map. Every ridge is now one hairline weight, and its colour says only one thing: how high it is. The low ground and the outermost rings are deep violet (#4900ff); climb the ladder and the lines run up through purple and magenta until the summit rings sit on hot orange (#f36c3b). At rest, two focal points push shallow magenta knots into a violet sea. Raise a hand and a second core surfaces on that side, then a third, and each one heats from violet to orange as you reach — so the frame reads as a topographic map of your own hands, and you can see the altitude of a peak from across the room without counting anything.',
  ...
  init(P) {
    ...
    P.state = {
      ...
      // ONE BUCKET PER CONTOUR LEVEL — the level index IS the colour index now,
      // where V3 bucketed by which core owned the ground.
      seg: [], NB: NL,
    ...
    const s = P.state;
    // gamma-space blend: a straight RGB lerp between these two runs through a
    // dead grey-plum in the middle; blending on the squares keeps the mid
    // magenta saturated and luminous, which is what survives the scrim.
    const LOW = [0x49, 0x00, 0xff], HIGH = [0xf3, 0x6c, 0x3b];
    s.ramp = u => {
      const t = clamp(u);
      return [
        Math.sqrt(LOW[0] * LOW[0] * (1 - t) + HIGH[0] * HIGH[0] * t) | 0,
        Math.sqrt(LOW[1] * LOW[1] * (1 - t) + HIGH[1] * HIGH[1] * t) | 0,
        Math.sqrt(LOW[2] * LOW[2] * (1 - t) + HIGH[2] * HIGH[2] * t) | 0
      ];
    };
    for (let k = 0; k < NL; k++) {
  ...
  step(P, dt, t, inp) {
    ...
    // ---- the field --------------------------------------------------------
    const GX = s.GX, GY = s.GY, f = s.f, fs = s.fs;
    ...
    for (let j = 0; j <= GY; j++) {
      ...
      for (let i = 0; i <= GX; i++) {
        ...
        for (let q = 0; q < 6; q++) {
          ...
          val += c.w * Math.exp(-(dx * dx + dy * dy) / sig2);
        }
        f[j * (GX + 1) + i] = val;
      }
    }
    ...
    // ---- march it into ridges, one bucket per elevation --------------------
  ...
  draw(P, g, w, h, t, inp) {
    ...
    // ONE WEIGHT for every ridge — V3's thinnest stroke, applied to all of them.
    const lw = Math.max(3.4 * ms, Math.min(w, h) * 0.011) * 0.8;
    ...
    for (let k = 0; k < s.NB; k++) {
      ...
      const c = s.lut[k];
      // near-flat alpha on purpose: the ramp already runs dark→bright on its
      // own (violet is a low-luminance colour, orange a high one), so fading
      // the bottom would just wash the stated violet out to a dark navy.
      const a = (0.85 + s.lutT[k] * 0.15) * bright;
