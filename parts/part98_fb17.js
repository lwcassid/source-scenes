/* ---------- SRC-15.17 · FERRO BLOOM V17 (three scenes, V9's vivid ring-stepped color) ---------- */
reg({
  id: 'SRC-15.17', family: 'SRC-15', ver: 17,
  title: 'Ferro Bloom V17', tech: 'SHADER FIELD / MOTION ILLUSION',
  music: {
  ...
  desc: 'V16\'s three scenes with V9\'s color back in them. What made V9 vivid was never the palette on its own — it was that each successive RING inside a flower steps to the next saturated hue, so a bloom stacks contrasting colors outward instead of shading through one. V16 interpolated inside a narrow ramp and went chalky; this restores the ring stepping exactly, four saturated stops per scene cycling the way V9 cycled its seven. WARM stacks hot pink, red and ember orange with light blue as the counterpoint. GREEN stacks chartreuse, teal and lime. BLUE stacks electric blue, lavender, light blue and teal. Each scene still owns the whole field with nothing from the other two leaking in, and the crossfade between them is steep on purpose — a scene holds at full saturation for most of its span so the necessarily duller blended moment between two schemes passes quickly. Everything else is V14: right grows the bigger half, the cluster rerolls its shape and center size each time, and the color runs on its own clock, untouched by the hands. Music unchanged: a three-layer soundscape — C-minor pedal drone whose color shifts, a quantized groove that only exists once the garden is thrown wide open, and a reactive layer where every willed bloom, flick, and stillness sounds back at you.',
  ...
  init(P) {
    ...
    const mat = new THREE.ShaderMaterial({
      ...
      fragmentShader: [
        ...
        '}',
        // V17: back to V9\'s COLOR CONSTRUCTION — the thing that made V9 vivid
        // was not the palette alone, it was that consecutive RINGS step to the
        // next saturated hue, so a flower stacks contrasting colors instead of
        // shading within one. V16 interpolated inside a narrow ramp, which is
        // what washed everything out. Each scene is now 4 saturated stops that
        // the rings cycle through, exactly the way V9 cycled its 7.
        'vec3 warmStop(float i){',
        '  float h = mod(i, 4.0);',
        '  if (h < 1.0) return vec3(1.0, 0.341, 0.831);',  // hot pink / orchid
        '  if (h < 2.0) return vec3(1.0, 0.16, 0.13);',    // red
        '  if (h < 3.0) return vec3(0.98, 0.62, 0.34);',   // ember orange
        '  return vec3(0.45, 0.82, 1.0);',                  // light blue — the counterpoint
        '}',
        'vec3 greenStop(float i){',
        '  float h = mod(i, 4.0);',
        '  if (h < 1.0) return vec3(0.878, 1.0, 0.161);',  // acid chartreuse
        '  if (h < 2.0) return vec3(0.16, 0.55, 0.58);',   // teal
        '  if (h < 3.0) return vec3(0.55, 0.92, 0.45);',   // lime
        '  return vec3(0.45, 0.82, 1.0);',                  // light blue
        '}',
        // The cool family sits in a narrow hue band, so ring stepping alone
        // gives it less to contrast than warm/green get. These four are pushed
        // apart in VALUE as well as hue — deep indigo against bright cyan —
        // so the scene reads as banded rather than one blue wash.
        'vec3 blueStop(float i){',
        '  float h = mod(i, 4.0);',
        '  if (h < 1.0) return vec3(0.0, 0.263, 1.0);',    // electric blue
        '  if (h < 2.0) return vec3(0.35, 0.95, 1.0);',    // bright cyan
        '  if (h < 3.0) return vec3(0.09, 0.10, 0.42);',   // deep indigo
        '  return vec3(0.58, 0.56, 0.90);',                 // lavender
        '}',
        // WARM <-> GREEN <-> BLUE and back, green bridging the two. The
        // crossfade is deliberately steep (0.30..0.70) so each scene holds at
        // full saturation for most of its span and the blended, necessarily
        // duller moment between two schemes stays brief.
        'vec3 sceneStop(float i, float scenePos){',
        '  if (scenePos < 0.5) return mix(warmStop(i), greenStop(i), smoothstep(0.30, 0.70, scenePos * 2.0));',
        '  return mix(greenStop(i), blueStop(i), smoothstep(0.30, 0.70, (scenePos - 0.5) * 2.0));',
        '}',
        ...
        '    float w = exp(-f / k);',
        '    acc += w;',
        // V9\'s exact per-bloom hue seed — becomes an integer slot offset below,
        // rotating which stop each flower starts its ring cycle on.
        ...
        // tour.
        // V9's ring stepping, verbatim: rif advances with phase, so moving
        // outward through a flower walks the scene's stops one after another —
        // contrasting saturated bands, not a shade of one colour.
        '  float rif = phase + 0.5 + hueOff;',
        '  vec3 tint = mix(sceneStop(floor(rif), mood), sceneStop(floor(rif) + 1.0, mood), smoothstep(0.0, 1.0, fract(rif)));',
        ...
        '    col += mix(deep, sceneStop(floor(af * 3.0 + fj), mood), aband) * ain * 0.34 * uAmb;',
  ...
  draw(P, g, w, h, t, inp) {
    ...
    if (s.noGL || !P._three) {
      ...
      g.textAlign = 'center';
      g.fillText('FERRO BLOOM V17 · VIVID SCENES', w / 2, h / 2 - 12);
