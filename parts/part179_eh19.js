...
reg({
  id: 'SRC-13.19', family: 'SRC-13', ver: 19, title: 'Event Horizon V19', tech: 'VERLET ROPE / ACCRETION',
  music: {
      ...
      tick(inp) {
        ...
        for (let j = 0; j < 6; j++) {
          ...
          if (want > 0.5 && !was[j]) {
            ...
            A.tone(f, { at: T.next(0.5), vol: 0.045 * gate * duck, dur: 1.8, attack: 0.1, type: 'triangle', pan: j < 3 ? -0.4 : 0.4, rev: 0.55, midi: false });
        ...
        if (s.dropBeat !== 0 && !boomed) {
          ...
          if (typeof MOut !== 'undefined' && s.dropBeat > 0) {
            MOut.evNote('drums2', 246.94, 0.24, Math.max(now + 0.01, at - T.beat * 8), T.beat * 8);
          }
          ...
          // THE JAM LOOP LANDS ON THE BOOM (V19): his pop drums (ch2 pad
          // 56), struck once on the drop's own instant, held all scene;
          // ch2 CC74 (jam energy, streamed below) does every fade
          if (typeof MOut !== 'undefined' && !s._loopHeld) {
            s._loopHeld = true;
            MOut.evNote('drums2', 207.65, 0.22, at, 3600);
          }
        }
