...
const LUMEN26_STOPCACHE = new Map();
reg({
  id: 'SRC-38.26', family: 'SRC-38', ver: 26, title: 'Lumen Film V26', tech: 'GEM FILM / APERTURE ORGAN',
  music: {
  ...
  step(P, dt, t, inp) {
    ...
    // tease?") -- a tease that cannot be seen is no tease.
    if (s.sparkle > 0) {
  ...
  draw(P, g, w, h, t, inp) {
    ...
    const addSvgStops = (grad, a) => {
      let set = LUMEN26_STOPCACHE.get(a);
      if (!set) {
        set = SVGSTOPS.map(([t, rgb]) => [t, `rgba(${rgb},${a})`]);
        LUMEN26_STOPCACHE.set(a, set);
      }
