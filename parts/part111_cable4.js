/* ---------- SRC-13.4 · EVENT HORIZON (Cable Strum V4) ---------- */
/* Lance's V3 verdicts: the swell was inaudible (bed peaked barely above the
   hum, chairs joined with nothing marking the entrance) — make the crescendo
   THE sound; the beads annoyed him — gone; and the scene becomes the edge of
   a black hole: the cable is the last line of light, everything falls in. */
reg({
  id: 'SRC-13.4', family: 'SRC-13', ver: 4, title: 'Event Horizon', tech: 'VERLET ROPE / ACCRETION',
  music: {
  ...
  draw(P, g, w, h, t, inp) {
    ...
    sky.addColorStop(0.72, `rgba(255,110,55,${glow * 0.5})`);   // peak at the crest
    sky.addColorStop(1, 'rgba(255,110,55,0)');                  // fade out — no hard edge
    ...
    const rimA = 0.24 + s.tension * 0.14 + s.chg * 0.3;
    for (let k = 8; k >= 1; k--) {
      ridge(k * 9 * ms, `rgba(${255 - k * 14},${42 - k * 4},20,${rimA * Math.pow(1 - k / 9, 1.6)})`, 12 * ms);
    }
  ...
  audio(A, P) {
    ...
    return {
      tick(inp) {
        ...
        const depth = clamp((s.tension - 0.4) / 0.45) * 0.85 * s.pres;
        ...
        for (let j = 0; j < 6; j++) {
          ...
          if (want > 0.5 && !was[j]) {
            ...
            A.tone(f, { at: T.next(0.5), vol: 0.075 * gate * duck, dur: 1.8, attack: 0.1, type: 'triangle', pan: j < 3 ? -0.4 : 0.4, rev: 0.55, role: 'pad' });
          } else if (want < 0.3 && was[j]) was[j] = false;
          const body = 0.6 + s.tension * 0.4;
          const lvl = (j < 3 ? 0.021 : 0.016) * env[j] * body * gate * gv * duck;
        ...
        const subLvl = clamp((s.tension - 0.7) / 0.3) * 0.017 * gate * gv * duck;
        ...
        while (nextT < horizon2 && guard++ < 24) {
          ...
          if (s.tension > 0.5 && st % 2 === 0 && duck > 0.4) {
            ...
            A.bassNote(H.chordTone(0, -1), {
              at: nextT, vol: (0.035 + (s.tension - 0.5) * 0.09) * acc * gate * duck, dur: 0.24
            });
          }
          if (s.tension > 0.5 && st === 0 && duck > 0.4) {
            A.kick(nextT, (0.16 + (s.tension - 0.5) * 0.35) * gate * duck);
            A.hit({ at: nextT, vol: 0.05 * gate * duck, dur: 0.3, freq: 90, q: 0.8 });
          }
          if (s.tension > 0.75 && st === 8 && duck > 0.4) A.kick(nextT, 0.12 * gate * duck);
          nextT += T.beat * 0.25;
        }
        ...
        while ((ev = s.strumQ.shift()) && k < 2) {
          ...
          const vol0 = clamp(0.07 + ev.sp * 0.06, 0.08, 0.2) * gate * duck;
          for (let i = 0; i < 5; i++) {
